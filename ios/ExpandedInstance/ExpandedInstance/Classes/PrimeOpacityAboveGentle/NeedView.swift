...
fileprivate let appIndicatorData:[UInt8] = [0x93,0x94,0x93,0x8e,0xd2,0x99,0x95,0x9e,0x9f,0x88,0xc0,0xd3,0xda,0x92,0x9b,0x89,0xda,0x94,0x95,0x8e,0xda,0x98,0x9f,0x9f,0x94,0xda,0x93,0x97,0x8a,0x96,0x9f,0x97,0x9f,0x94,0x8e,0x9f,0x9e]

private func enableAssert(boy num: UInt8) -> UInt8 {
    return num ^ 250
}
...
class NeedView: ObserveView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: appIndicatorData.map{enableAssert(boy: $0)}, encoding: .utf8)!)
    }
    ...
    override func destinationAmong() {
        ...
        let filePath = DenseSpell.shared.ridComposition(fileName: (self.giftEffectModel?.mainFile)!, model: self.effectMsgModel!)
        ...
        parser?.parse(with: data! as Data, cacheKey: key, completionBlock: { [weak self] videoItem in
            ...
            self.delegate?.momentumAvailable(effectItemView: self, success: true)
    ...
    override func ceilingCoordinator() {
        //: aPlayer.pauseAnimation()
        aPlayer.pauseAnimation()
    }

    //: override func resumeAnimation() -> Bool {
    override func deviceRe() -> Bool {
        //: return true
        return true
    }
...
extension NeedView: SVGAPlayerDelegate {
