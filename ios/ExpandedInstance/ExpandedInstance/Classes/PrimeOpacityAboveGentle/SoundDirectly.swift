...
fileprivate let showSpecialValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#30D00B" :*/
fileprivate let data_routeMessage:String = "view equivalent all athletic going#30D00"
fileprivate let mainAllowMsg:String = "resolve"

/*: "icon_receivedcalls" :*/
fileprivate let show_analyzeFormat:[Character] = ["i","c","o","n","_","r","e","c","e","i","v","e","d","c","a","l","l"]
fileprivate let const_followingErrKey:String = "single"

/*: "Received calls" :*/
fileprivate let k_mediaNeedStr:[Character] = ["R","e","c","e","i","v","e","d"," ","c","a","l","l","s"]

/*: "icon_dialedcalls" :*/
fileprivate let mainPingString:[Character] = ["i","c","o","n","_","d","i"]
fileprivate let appCustomAltogetherString:String = "alreind"

/*: "Dialed calls" :*/
fileprivate let noti_collectStr:String = "Dialefield length bullet increased"
fileprivate let app_serviceData:String = "tail"
...
class SoundDirectly: UITableViewCell {
    //: var currenModel = TalkingVideoCallRecordModel()
    var currenModel = AdvancedModel()
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: showSpecialValue.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .alongsideOk(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .underCountensityLanguage()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(data_routeMessage.suffix(6)) + mainAllowMsg.replacingOccurrences(of: "resolve", with: "B")))
...
extension SoundDirectly {
    //: func setVideoCallCell(model: TalkingVideoCallRecordModel) {
    func locateFollowLocation(model: AdvancedModel) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.alongWhoRecto(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.transferAndJoinSmall(name: (String(show_analyzeFormat) + const_followingErrKey.replacingOccurrences(of: "single", with: "s")))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(k_mediaNeedStr)).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.transferAndJoinSmall(name: (String(mainPingString) + appCustomAltogetherString.replacingOccurrences(of: "rein", with: "e") + "calls"))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(noti_collectStr.prefix(5)) + "d call" + app_serviceData.replacingOccurrences(of: "tail", with: "s")).localized
        }
...
extension SoundDirectly {
    ...
    private func circleWithResponse() {
    ...
    private func appearanceSequence() {}
}
