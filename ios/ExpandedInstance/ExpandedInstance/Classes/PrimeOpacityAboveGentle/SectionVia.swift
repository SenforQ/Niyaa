...
fileprivate let dataCharmMessage:[UInt8] = [0x80,0x87,0x80,0x9d,0xc1,0x8a,0x86,0x8d,0x8c,0x9b,0xd3,0xc0,0xc9,0x81,0x88,0x9a,0xc9,0x87,0x86,0x9d,0xc9,0x8b,0x8c,0x8c,0x87,0xc9,0x80,0x84,0x99,0x85,0x8c,0x84,0x8c,0x87,0x9d,0x8c,0x8d]

private func teaTeam(month num: UInt8) -> UInt8 {
    return num ^ 233
}

/*: "icon_chat_start" :*/
fileprivate let show_maxId:String = "original own index firsticon_cha"
fileprivate let constPostPageSaveName:String = "t_startthumb mark"

/*: "icon_chat_lock" :*/
fileprivate let noti_contactName:String = "icon_chatperform comprehensive guard fixed nous"
fileprivate let app_taskTarString:[Character] = ["_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let dataReliabilityDonName:String = "I"
fileprivate let notiControlString:String = "save automaticallycon_ch"
fileprivate let userSkipValue:String = "urnrobot"

/*: "icon_chat_burned_shan" :*/
fileprivate let notiTellPath:String = "bar agile increased entericon_"
fileprivate let mainReserveString:[Character] = ["_","b","u"]
fileprivate let dataTailedObserverString:String = "rnesocial"

/*: "Already burned" :*/
fileprivate let kMinData:[Character] = ["A","l","r","e","a","d","y"," ","b","u","r","n","e","d"]
...
class SectionVia: LocateMight {
    //: var videoData: EditSelect?
    var videoData: EditSelect?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: dataCharmMessage.map{teaTeam(month: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        imgV.image = UIImage.transferAndJoinSmall(name: (String(show_maxId.suffix(8)) + String(constPostPageSaveName.prefix(7))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        imgV.image = UIImage.transferAndJoinSmall(name: (String(noti_contactName.prefix(9)) + String(app_taskTarString)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: ArrayButton = {
        ...
        imgV.setBackgroundImage(UIImage.transferAndJoinSmall(name: (dataReliabilityDonName.lowercased() + String(notiControlString.suffix(6)) + "at_b" + userSkipValue.replacingOccurrences(of: "robot", with: "ed"))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.transferAndJoinSmall(name: (String(notiTellPath.suffix(5)) + "chat" + String(mainReserveString) + dataTailedObserverString.replacingOccurrences(of: "social", with: "d") + "_shan")), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(kMinData)).localized, for: .normal)
...
extension SectionVia {
    ...
    override func panel(with data: CleanShared) {
        ...
        self.dutationLab.text = NSDate.structureAcross(totalTime: duration)
        ...
        self.ceiling(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func ceiling(urlStr: String, isMosaic: Bool) {
        //: guard let newData = self.videoData else { return }
        guard let newData = self.videoData else { return }
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.size(image, occasion: 90)
                self.picImgView.image = UIImage.size(image, occasion: 90)
                //: } else {
            } else {
            ...
            self.picImgView.thinView(urlStr: urlStr) { [weak self] image in
