...
/*: "DBUserInfoTable" :*/
fileprivate let mainCooperativeKey:String = "Dcolonial"
fileprivate let dataRelationInsideFormat:[Character] = ["U","s","e","r","I","n","f","o","T","a","b","l","e"]

/*: "WCDB表 :*/
fileprivate let app_talkFormat:[Character] = ["W","C","D","B","\u{8868}"]

/*: : 更新数据失败。error： :*/
fileprivate let constTickName:String = ": 更新\u{6570}据"
fileprivate let dataBalanceId:[Character] = ["\u{5931}","败","。","e","r","r","o","r","："]

/*: : 批量插入数据失败。error： :*/
fileprivate let kManualStr:String = ": \u{6279}量"
fileprivate let dataFillAltogetherPath:String = "失败\u{3002}e"
...
let app_markName = (mainCooperativeKey.replacingOccurrences(of: "colonial", with: "B") + String(dataRelationInsideFormat))
//: final class WCDBUserInfoTable: TableCodable {
final class OutsideMode: TableCodable {
    /// 用户Id【主键】
    //: var uid: String = ""
    var uid: String = ""
    /// 昵称
...
// MARK: - BetweenInput和WCDBUserInfoTable转换

//: extension WCDBUserInfoTable {
extension OutsideMode {
    /// BetweenInput  =>  OutsideMode
    //: private class func mapToDBUserInfoData(_ userInfo: BetweenInput) -> WCDBUserInfoTable {
    private class func crushRefresh(_ userInfo: BetweenInput) -> OutsideMode {
        //: let dbData = WCDBUserInfoTable()
        let dbData = OutsideMode()
    ...
    /// [OutsideMode] => [BetweenInput]
    //: private class func mapToUserInfoModels(_ dbDatas: [WCDBUserInfoTable]) -> [BetweenInput] {
    private class func operateBold(_ dbDatas: [OutsideMode]) -> [BetweenInput] {
        //: return dbDatas.map { data in
        return dbDatas.map { data in
            //: let userInfo = BetweenInput()
            let userInfo = BetweenInput()
...
extension OutsideMode {
    ...
    class func scanByProgram(with userId: String) {
        ...
        try? SkirtPad.shared.database?.run(transaction: { _ in
            ...
            let condition = OutsideMode.Properties.uid.in(userId)
            //: try? WCDBManager.shared.database?.delete(fromTable: WCDBUserInfoTableName,
            try? SkirtPad.shared.database?.delete(fromTable: app_markName,
                                                     //: where: condition)
                                                     where: condition)
            //: })
        })
    ...
    /// - Parameter userId: 用户id
    /// - Returns: 模型
    //: class func db_getUserInfo(with userId: String) -> BetweenInput? {
    class func chapterActivity(with userId: String) -> BetweenInput? {
        //: let userInfos = WCDBUserInfoTable.db_getUserInfos(with: [userId])
        let userInfos = OutsideMode.findUsed(with: [userId])
    ...
    /// - Returns: 数据数组
    //: class func db_getUserInfos(with userIds: [String]) -> [BetweenInput] {
    class func findUsed(with userIds: [String]) -> [BetweenInput] {
        //: guard userIds.count > 0 else { return [] }
        guard userIds.count > 0 else { return [] }
        //: do {
        do {
            //: let condition = WCDBUserInfoTable.Properties.uid.in(userIds)
            let condition = OutsideMode.Properties.uid.in(userIds)
            ...
            let userInfos: [OutsideMode]? = try SkirtPad.shared.database?.getObjects(on: OutsideMode.Properties.all, fromTable: app_markName, where: condition)
            //: if userInfos?.isEmpty == false {
            if userInfos?.isEmpty == false {
                //: let models = WCDBUserInfoTable.mapToUserInfoModels(userInfos!)
                let models = OutsideMode.operateBold(userInfos!)
                //: return models
                return models
            }
            //: } catch {
    ...
    //: class func db_updateUserInfo(_ userInfo: BetweenInput) {
    class func cosBelow(_ userInfo: BetweenInput) {
        //: let dbData = WCDBUserInfoTable.mapToDBUserInfoData(userInfo)
        let dbData = OutsideMode.crushRefresh(userInfo)
        //: try? WCDBManager.shared.database?.run(transaction: { _ in
        try? SkirtPad.shared.database?.run(transaction: { _ in
            // 数据库中已存在，则更新数据
            //: if WCDBUserInfoTable.db_getUserInfo(with: dbData.uid) != nil {
            if OutsideMode.chapterActivity(with: dbData.uid) != nil {
                //: do {
                do {
                    //: let condition = WCDBUserInfoTable.Properties.uid == dbData.uid
                    let condition = OutsideMode.Properties.uid == dbData.uid
                    //: try WCDBManager.shared.database?.update(table: WCDBUserInfoTableName,
                    try SkirtPad.shared.database?.update(table: app_markName,
                                                            //: on: WCDBUserInfoTable.Properties.all,
                                                            on: OutsideMode.Properties.all,
                                                            //: with: dbData,
                                                            with: dbData,
                                                            //: where: condition)
                                                            where: condition)
                    ...
                    MaximumMaster.circleStat(msg: (String(app_talkFormat)) + "\(app_markName)" + (constTickName + String(dataBalanceId)) + "\(error).")
                }
                //: } else {
            } else {
                // 数据库中不存在，则插入数据
                //: WCDBUserInfoTable.db_insertUserInfo(dbData)
                OutsideMode.transitionInPop(dbData)
            }
            //: })
        })
    }
    ...
    private class func transitionInPop(_ userInfo: OutsideMode) {
        //: WCDBUserInfoTable.db_insertUserInfos([userInfo])
        OutsideMode.thread([userInfo])
    }

    /// 插入多条数据
    /// - Parameter userInfos: 数据数组
    //: private class func db_insertUserInfos(_ userInfos: [WCDBUserInfoTable]) {
    private class func thread(_ userInfos: [OutsideMode]) {
        //: try? WCDBManager.shared.database?.run(transaction: { _ in
        try? SkirtPad.shared.database?.run(transaction: { _ in
            //: do {
            do {
                //: try WCDBManager.shared.database?.insert(userInfos, intoTable: WCDBUserInfoTableName)
                try SkirtPad.shared.database?.insert(userInfos, intoTable: app_markName)
                //: } catch {
            } catch {
                //: UploadLogTool.writeLog(msg: "WCDB表\(WCDBUserInfoTableName): 批量插入数据失败。error：\(error).")
                MaximumMaster.circleStat(msg: (String(app_talkFormat)) + "\(app_markName)" + (kManualStr + "插入\u{6570}据" + dataFillAltogetherPath + "rror：") + "\(error).")
            }
            //: })
        })
    }
