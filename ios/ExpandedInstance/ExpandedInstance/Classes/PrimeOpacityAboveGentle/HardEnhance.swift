...
/*: "Search" :*/
fileprivate let kMaxName:[Character] = ["S","e","a","r","c","h"]

/*: "#F5F5F5" :*/
fileprivate let constApplicationValue:String = "#F5F5it void storage concrete"
fileprivate let appPhoneBotData:String = "background5"

/*: "Search by UID" :*/
fileprivate let noti_inviteMsg:String = "let yetSearc"
fileprivate let userLightKingUrl:String = "close"

/*: "icon_search_sea" :*/
fileprivate let user_petKey:String = "wait efficiencyicon_s"
fileprivate let user_yesterdayValue:[Character] = ["_","s","e","a"]

/*: "Can’t find~" :*/
fileprivate let userStageImplementUrl:String = "super time lay keyCan’"

/*: "icon_kong_kong_default" :*/
fileprivate let const_atThereId:String = "ICON"
fileprivate let noti_refuseName:String = "construct expected ex_kon"
fileprivate let const_pureSpringTitle:String = "equallylt"

/*: "Please enter the query UID" :*/
fileprivate let constVeryStr:[Character] = ["P","l","e","a","s","e"," ","e","n","t"]
fileprivate let noti_statDepartmentTitle:String = "er thenot recording flex lay version"
fileprivate let mainYourId:String = "entity locate return refuse rolery UID"
...
class HardEnhance: ScaleViewController {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(kMaxName)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.doEmbrace(colors: UIColor.monthForMomentum(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(slideRate), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.backgroundColor = UIColor(hex: (String(constApplicationValue.prefix(5)) + appPhoneBotData.replacingOccurrences(of: "background", with: "F")))
        //: tf.textColor = UIColor.appTitleColor()
        tf.textColor = UIColor.thinOf()
        //: tf.font = UIFont.pingfangRugularFont(fontSize: 16)
        tf.font = UIFont.botLocal(fontSize: 16)
        ...
        tf.placeholder = (String(noti_inviteMsg.suffix(5)) + "h by UI" + userLightKingUrl.replacingOccurrences(of: "close", with: "D")).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.transferAndJoinSmall(name: (String(user_petKey.suffix(6)) + "earch" + String(user_yesterdayValue)))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(PutCollection.self, forCellReuseIdentifier: PutCollection.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(userStageImplementUrl.suffix(4)) + "t find~").localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (const_atThereId.lowercased() + "_kong" + String(noti_refuseName.suffix(4)) + "g_def" + const_pureSpringTitle.replacingOccurrences(of: "equally", with: "au"))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + main_measureString, width: dataTabUrl, height: const_rebuildWithinId - (14 + main_measureString + app_whenAppearanceData)), style: style)
    ...
    private lazy var reqManager: PerNativeOver = //: return PerNativeOver()
        .init()
    //: }()
}
...
extension HardEnhance {
    // 根据uid查询用户
    //: private func req_searchUid() {
    private func manageAt() {
        //: self.view.endEditing(true)
        self.view.endEditing(true)
        //: guard searchTF.text?.count ?? 0 > 0 else {
        guard searchTF.text?.count ?? 0 > 0 else {
            //: self.func__showStatusBarErrorMsg(showMsg: "Please enter the query UID".localized)
            self.paraDown(showMsg: (String(constVeryStr) + String(noti_statDepartmentTitle.prefix(6)) + " que" + String(mainYourId.suffix(6))).localized)
            //: return
            return
        }
        ...
        reqManager.shapeSpot(uid: searchTF.text ?? "") { [weak self] succeed, result, errorModel in
            //: ProgressHUD.dismiss()
...
extension HardEnhance: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: PutCollection.className(), for: indexPath) as! PutCollection
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.nous(model: model)
...
extension HardEnhance: UITextFieldDelegate {
...
extension HardEnhance: JXPagingViewListViewDelegate {
...
extension HardEnhance {
    ...
    private func botTipReinBorderAvailable() {
