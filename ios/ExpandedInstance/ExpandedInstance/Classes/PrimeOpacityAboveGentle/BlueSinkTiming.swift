...
fileprivate let const_personalCountensityName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Select mute duration" :*/
fileprivate let user_sustainableMsg:String = "Selelocal nothing king"
fileprivate let userBrownDistanceUrl:String = "transfer aleute d"
fileprivate let user_personFormat:String = "URATION"

/*: "Cirform" :*/
fileprivate let main_chainFormat:String = "link consequence pant taCirform"

/*: "Mute success" :*/
fileprivate let dataActorData:String = "upper owner manageMute s"
fileprivate let notiModifyUrl:String = "tea"

/*: "day" :*/
fileprivate let show_hiString:String = "dablue"

/*: "days" :*/
fileprivate let show_embraceObserverValue:String = "dayreceiver"
...
class BlueSinkTiming: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: const_personalCountensityName.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(flexCancel), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(user_sustainableMsg.prefix(4)) + "ct m" + String(userBrownDistanceUrl.suffix(5)) + user_personFormat.lowercased()).localized
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
        //: return lb
        return lb
    ...
    lazy var commitBtn: UIButton = {
        ...
        btn.setTitle((String(main_chainFormat.suffix(7))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(lineSufficientOutput), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension BlueSinkTiming {
    /// 消除视图按钮
    //: @objc private func bgDismissButtonClick() {
    @objc private func flexCancel() {
        //: dismiss()
        common()
    }
    ...
    func amongProvide() {
        ...
        self.contentView.frame = CGRect(x: (dataTabUrl - contentV_Size.width) / 2, y: const_rebuildWithinId, width: contentV_Size.width, height: contentV_Size.height)

        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = (ScreenHeight-self.contentV_Size.height)/2
            self.contentView.frame.origin.y = (const_rebuildWithinId - self.contentV_Size.height) / 2
        }
    }
    ...
    func common() {
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.contentView.frame.origin.y = const_rebuildWithinId
...
extension BlueSinkTiming {
    ...
    @objc private func capPhase(btn: UIButton) {
    ...
    @objc private func lineSufficientOutput() {
        ...
        if self.viewType == .live {
            ...
            SkipOriginal().untilModel(targetUid: targetUid, duration: index) { succeed, result, errorModel in
                //: ProgressHUD.dismiss()
                HandleEquipment.element()
                //: if succeed {
                if succeed {
                    //: self.func__showStatusBarSuccessMsg(showMsg: "Mute success".localized)
                    self.outsideFeature(showMsg: (String(dataActorData.suffix(6)) + "ucce" + notiModifyUrl.replacingOccurrences(of: "tea", with: "ss")).localized)
                }
                //: self.dismiss()
                self.common()
            }
...
extension BlueSinkTiming {
    ...
    private func accept() {
        ...
        for i in 0 ..< 3 {
            ...
            btn.graphChain(color: UIColor.multiSince(), forState: .selected)
            ...
            let dayStr = (title == "1") ? (show_hiString.replacingOccurrences(of: "blue", with: "y")).localized : (show_embraceObserverValue.replacingOccurrences(of: "receiver", with: "s")).localized
            //: btn.setTitle("\(title) " + dayStr, for: .normal)
            btn.setTitle("\(title) " + dayStr, for: .normal)
            //: btn.tag = 100+i
            btn.tag = 100 + i
            //: btn.addTarget(self, action: #selector(muteBtnClick), for: .touchUpInside)
            btn.addTarget(self, action: #selector(capPhase), for: .touchUpInside)
