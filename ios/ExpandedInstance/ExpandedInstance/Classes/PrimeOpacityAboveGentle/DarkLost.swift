...
/*: "num" :*/
fileprivate let dataWouldMomentData:String = "nuenthusiasm"
...
@objc public enum BulletPrepare: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct DarkLost: HandyJSON {
    ...
    mutating func eventIdentity(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct PinImplementation: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (dataWouldMomentData.replacingOccurrences(of: "enthusiasm", with: "m"))
    }
}
