...
fileprivate let showStyleMessage:[UInt8] = [0x7b,0x80,0x7b,0x86,0x3a,0x75,0x81,0x76,0x77,0x84,0x4c,0x3b,0x32,0x7a,0x73,0x85,0x32,0x80,0x81,0x86,0x32,0x74,0x77,0x77,0x80,0x32,0x7b,0x7f,0x82,0x7e,0x77,0x7f,0x77,0x80,0x86,0x77,0x76]

fileprivate func searchPic(single num: UInt8) -> UInt8 {
...
/*: "Cancel" :*/
fileprivate let k_signalDataDelayString:String = "normally"
fileprivate let constServerDetailData:String = "anceentity"

/*: "Sure" :*/
fileprivate let main_firePath:String = "party quantity within required zzSure"
...
class HeadView: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showStyleMessage.map{searchPic(single: $0)}, encoding: .utf8)!)
    }

    //: private func setupView() {
    private func triggerBeautyBrick() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(pastUniform))
        ...
        cancelButton.setTitle((k_signalDataDelayString.replacingOccurrences(of: "normally", with: "C") + constServerDetailData.replacingOccurrences(of: "entity", with: "l")).localized, for: .normal)
        //: cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: cancelButton.setTitleColor(UIColor.black, for: .normal)
        cancelButton.setTitleColor(UIColor.black, for: .normal)
        //: cancelButton.addTarget(self, action: #selector(cancelClick(button:)), for: .touchUpInside)
        cancelButton.addTarget(self, action: #selector(everyLoad(button:)), for: .touchUpInside)
        ...
        confirmButton.setTitle((String(main_firePath.suffix(4))).localized, for: .normal)
        //: confirmButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        confirmButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: confirmButton.setTitleColor(UIColor.black, for: .normal)
        confirmButton.setTitleColor(UIColor.black, for: .normal)
        //: confirmButton.addTarget(self, action: #selector(confirmClick(button:)), for: .touchUpInside)
        confirmButton.addTarget(self, action: #selector(activityEvaluation(button:)), for: .touchUpInside)
    ...
    @objc func everyLoad(button: UIButton) {
        //: dismissView()
        pastUniform()
    }

    //: @objc func confirmClick(button: UIButton) {
    @objc func activityEvaluation(button: UIButton) {
        //: dismissView()
        pastUniform()
    }
