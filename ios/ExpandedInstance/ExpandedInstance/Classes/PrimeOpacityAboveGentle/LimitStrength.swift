...
fileprivate let show_itWedFunctionalTitle:String = "spend my rocket uniqueDocume"
fileprivate let mainRetiredStr:String = "bold await butterflyser/"

/*: "Documents/User/record/" :*/
fileprivate let constOccasionId:String = "would dynamics purchaseDocum"
fileprivate let dataTakeProductMsg:String = "stage already ionUser/r"

/*: "cancelled" :*/
fileprivate let user_includeMeetingMessage:String = "samplence"

/*: .wav" :*/
fileprivate let main_sceneHealthyUponString:String = ".wavless arc"
...
typealias OneKeyGreetDownloadBlock = (_ model: EvaluationModel) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol AtWill: NSObjectProtocol {
    /// 下载完成
...
extension LimitStrength {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func findListener(_ tasksArray: [EvaluationModel]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.thirdWithMid()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(show_itWedFunctionalTitle.suffix(6)) + "nts/U" + String(mainRetiredStr.suffix(4)) + "voice/")) {
                //: model.zipFilePath = "\(AppCacheDefine.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(SectionTotalerval.retired())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(constOccasionId.suffix(5)) + "ents/" + String(dataTakeProductMsg.suffix(6)) + "ecord/")) {
                //: model.zipFilePath = "\(AppCacheDefine.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(SectionTotalerval.serverAssociated())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            PanMarker.confirmToParty(msgId: model.taskId, status: "\(DivideSwitchtoPriority.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func byFinish(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = EvaluationModel.switchce(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? EvaluationModel()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(show_itWedFunctionalTitle.suffix(6)) + "nts/U" + String(mainRetiredStr.suffix(4)) + "voice/")) {
            //: taskModel.zipFilePath = "\(AppCacheDefine.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(SectionTotalerval.retired())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(constOccasionId.suffix(5)) + "ents/" + String(dataTakeProductMsg.suffix(6)) + "ecord/")) {
            //: taskModel.zipFilePath = "\(AppCacheDefine.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(SectionTotalerval.serverAssociated())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension LimitStrength: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = EvaluationModel.publish(path: taskModel.zipFilePath)
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.wrapPhone(model: taskModel)
    }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (user_includeMeetingMessage.replacingOccurrences(of: "sample", with: "ca") + "lled") {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    media(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.historySustainable(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                media(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.emptyRegulate(url: task.taskDescription ?? "")
        }
...
extension LimitStrength {
    ...
    func media(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(DivideSwitchtoPriority.pause.rawValue)" : "\(DivideSwitchtoPriority.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            PanMarker.confirmToParty(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.tickLikeCustom(model: taskModel)
            }
    ...
    func historySustainable(url: String) {
        //: guard let taskModel = self.downloadingModelDic[url] else { return }
        guard let taskModel = self.downloadingModelDic[url] else { return }
        //: self.downloadingModelDic.removeValue(forKey: url)
        self.downloadingModelDic.removeValue(forKey: url)
        ...
        case .chatRecord:
            ...
            PanMarker.towardBe(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.handle(model: taskModel)
    ...
    func emptyRegulate(url: String) {
        ...
        let size = EvaluationModel.publish(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                PanMarker.confirmToParty(msgId: taskModel.taskId, status: "\(DivideSwitchtoPriority.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(main_sceneHealthyUponString.prefix(4)))
            ...
            self.basic(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.romanBuild(model: taskModel)
            ...
            case .chatRecord:
                ...
                PanMarker.confirmToParty(msgId: taskModel.taskId, status: "\(DivideSwitchtoPriority.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.pickToSend(model: taskModel)
    ...
    func basic(at path: String, withKeyId keyId: String, withTypeId typeId: ContextTime, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(main_sceneHealthyUponString.prefix(4)))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    let unzippedData = data.discountInstall()
                    ...
                    if typeId == .chatRecord {
                        ...
                        PanMarker.confirmToParty(msgId: keyId, filePath: newPath)
                    }
