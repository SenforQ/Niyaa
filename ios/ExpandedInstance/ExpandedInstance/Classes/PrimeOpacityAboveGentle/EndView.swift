...
fileprivate let user_wordName:[UInt8] = [0x3b,0x3c,0x3b,0x26,0x7a,0x31,0x3d,0x36,0x37,0x20,0x68,0x7b,0x72,0x3a,0x33,0x21,0x72,0x3c,0x3d,0x26,0x72,0x30,0x37,0x37,0x3c,0x72,0x3b,0x3f,0x22,0x3e,0x37,0x3f,0x37,0x3c,0x26,0x37,0x36]

/*: "#171717" :*/
fileprivate let noti_blocThumbUrl:[Character] = ["#","1","7","1","7","1"]
fileprivate let showLayId:[Character] = ["7"]

/*: "icon_cameraoff" :*/
fileprivate let notiAroundTrackId:String = "icon_privilege framework judge feedback rate"
fileprivate let const_checkTitle:String = "clearlyff"
...
class EndView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.robotBy(51, 51, 51, 1)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: user_wordName.map{$0^82}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (String(noti_blocThumbUrl) + String(showLayId)))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.transferAndJoinSmall(name: (String(notiAroundTrackId.prefix(5)) + "camer" + const_checkTitle.replacingOccurrences(of: "clearly", with: "ao")))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension EndView {
    ...
    func listenBack(urlStr: String?, isOpenCamera: QueryEven) {
...
extension EndView: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func yesterdayReflect(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, dataTabUrl - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, const_rebuildWithinId - self.frame.size.height * 0.5)
    ...
    @objc private func advancedObtain() {
        //: if self.block != nil {
        if self.block != nil {
            //: self.block!()
            self.block!()
...
extension EndView {
    ...
    private func inside() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(advancedObtain))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(yesterdayReflect(pan:)))
        //: pan.delegate = self
        pan.delegate = self
