...
fileprivate let kCityString:[UInt8] = [0x1b,0x1c,0x1b,0x6,0x5a,0x11,0x1d,0x16,0x17,0x0,0x48,0x5b,0x52,0x1a,0x13,0x1,0x52,0x1c,0x1d,0x6,0x52,0x10,0x17,0x17,0x1c,0x52,0x1b,0x1f,0x2,0x1e,0x17,0x1f,0x17,0x1c,0x6,0x17,0x16]
...
class ToAccessible: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: kCityString.map{$0^114}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: SMountElement = {
        ...
        let v = SMountElement(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(strokeBlack(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(mark(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension ToAccessible {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func strokeBlack(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func mark(gesture: UIPanGestureRecognizer) {
