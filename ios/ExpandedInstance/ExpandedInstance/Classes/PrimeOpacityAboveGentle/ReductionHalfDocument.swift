...
class ReductionHalfDocument: ScaleViewController {
    ...
    lazy var topView: PerformFractionRegulate = {
        //: let view = TalkingMomentPhotosTopView(frame: CGRect.zero, type: myType ?? .normal)
        let view = PerformFractionRegulate(frame: CGRect.zero, type: myType ?? .normal)
        //: return view
        return view
        //: }()
    }()
    ...
    lazy var bottomView: AnalyzePet = {
    ...
    lazy var usePhotoView: LayerFormatView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimatePhoto)
        let v = LayerFormatView(frame: CGRect.zero, type: .intimatePhoto)
        //: return v
        return v
        //: }()
    }()
...
extension ReductionHalfDocument {
    //: func crushBtnTool(modle: TalkingMomentModel) {
    func behindToday(modle: SubsequentSkip) {
        ...
        if modle.sex == nil {
            ...
            //: bottomView.isHidden = modle.sex==QuantityeractionDelay.share.loginUserMode.sex
            bottomView.isHidden = modle.sex == QuantityeractionDelay.share.loginUserMode.sex
        }
