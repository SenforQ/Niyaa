...
fileprivate let noti_directlyUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_loading_lose" :*/
fileprivate let app_orangeMedalKey:String = "img_lproduction action ready drop"
fileprivate let main_securityString:String = "pop each from listng_los"
fileprivate let main_careStr:String = "E"
...
class StructureView: UICollectionViewCell {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: noti_directlyUrl.reversed(), encoding: .utf8)!)
    }
...
extension StructureView {
    ...
    private func device() {
        ...
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(enhance))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(execute)).then {
...
extension StructureView {
    ...
    func errBeauty(imgModel: UntilInstruction) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(app_orangeMedalKey.prefix(5)) + "oadi" + String(main_securityString.suffix(6)) + main_careStr.lowercased())), options: [.keepCurrentImageWhileLoading]) { result in
                //: self.recoverSubviews()
                self.master()
            }
        }
    ...
    func following(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(app_orangeMedalKey.prefix(5)) + "oadi" + String(main_securityString.suffix(6)) + main_careStr.lowercased())), options: [.keepCurrentImageWhileLoading]) { result in
            //: self.recoverSubviews()
            self.master()
        }
    }
...
extension StructureView: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension StructureView {
    ...
    func master() {
        ...
        var newH = dataTabUrl * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > const_rebuildWithinId {
            //: newH = ScreenHeight
            newH = const_rebuildWithinId
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * const_rebuildWithinId / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (dataTabUrl - newW) / 2.0, y: (const_rebuildWithinId - newH) / 2)
