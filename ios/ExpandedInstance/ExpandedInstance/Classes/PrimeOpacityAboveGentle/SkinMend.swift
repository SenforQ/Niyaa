...
fileprivate let constFrequencyUrl:[UInt8] = [0xc6,0xc1,0xc6,0xdb,0x87,0xcc,0xc0,0xcb,0xca,0xdd,0x95,0x86,0x8f,0xc7,0xce,0xdc,0x8f,0xc1,0xc0,0xdb,0x8f,0xcd,0xca,0xca,0xc1,0x8f,0xc6,0xc2,0xdf,0xc3,0xca,0xc2,0xca,0xc1,0xdb,0xca,0xcb]

private func hostDeal(assert num: UInt8) -> UInt8 {
    return num ^ 175
}

/*: "icon_male_default" :*/
fileprivate let user_introPath:[UInt8] = [0x85,0x8f,0x83,0x82,0xb3,0x81,0x8d,0x80,0x89,0xb3,0x88,0x89,0x8a,0x8d,0x99,0x80,0x98]

/*: "icon_female_default" :*/
fileprivate let mainCoordinateDirectValue:[Character] = ["i","c","o","n","_","f","e"]
fileprivate let userIncreasedCeilingData:String = "womanl"
fileprivate let show_siblingSparkId:[Character] = ["e","_","d","e","f","a","u","l","t"]

/*: "level_%@" :*/
fileprivate let main_publisherSophisticatedValue:[Character] = ["l","e","v","e","l","_","%","@"]
...
class SkinMend: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constFrequencyUrl.map{hostDeal(assert: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .alongsideOk(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .alongsideOk(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension SkinMend {
    ...
    func roundYesterdayAround(model: ContainKey) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.embraceByBold())
        ...
        let imgStr = model.sex == CommonAmong.male.rawValue ? String(bytes: user_introPath.map{$0^236}, encoding: .utf8)! : (String(mainCoordinateDirectValue) + userIncreasedCeilingData.replacingOccurrences(of: "woman", with: "ma") + String(show_siblingSparkId))
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(main_publisherSophisticatedValue)), model.level)
