...
/*: "Cancel" :*/
fileprivate let appTemporaryName:[Character] = ["C","a","n","c","e","l"]

/*: "Post" :*/
fileprivate let constLeastThoroughTitle:[Character] = ["P","o","s","t"]

/*: "Upload failed" :*/
fileprivate let appGapId:[Character] = ["U","p","l","o"]
fileprivate let data_curveValue:String = "bound providerad failed"

/*: "imgUrl" :*/
fileprivate let kPinKey:String = "imgUrltransition infrastructure"

/*: "videoUrl" :*/
fileprivate let app_thatPath:String = "vsocialde"

/*: "content" :*/
fileprivate let k_techniqueString:String = "conmusicenmusic"

/*: "location" :*/
fileprivate let k_correctYouName:[Character] = ["l","o","c","a","t"]
fileprivate let kMultipleFrontId:String = "isalen"

/*: "Video review in progress" :*/
fileprivate let dataGrayUrl:String = "Videpast uniform staff visual"
fileprivate let data_missionPath:String = "link train besideiew i"
fileprivate let appRefuseFormat:[Character] = ["n"," ","p","r","o","g","r","e","s","s"]

/*: "Shoot video at least 5 seconds" :*/
fileprivate let const_correctData:String = "path persist live complaint shortShoot"
fileprivate let dataVestFormat:String = "to regulate indeed mp at le"
fileprivate let data_familyStr:[Character] = ["s","e"]
fileprivate let kAlbumKey:String = "shotnds"

/*: "Camera" :*/
fileprivate let userLeadingTeamTarUrl:String = "Cameralite jack bounce re success"

/*: "Photo" :*/
fileprivate let mainInjuryData:[Character] = ["P","h","o","t","o"]

/*: "Video" :*/
fileprivate let kSFormat:String = "Videoprivilege could"
...
// __CLOSE_PRINT__
//
//  DestroyEventKeepDirectlyRounding.swift
//  AbroadTalking
//
...
protocol DiscoverFrom: NSObjectProtocol {
    // 发布成功
    //: func ShareSucceed()
    func pull()
}
...
class DestroyEventKeepDirectlyRounding: ScaleViewController {
    //: weak var delegate: TalkingFreeViewControllerDelegate?
    weak var delegate: DiscoverFrom?
    ...
    private lazy var reqManager: TellCharacter = //: return TellCharacter()
        .init()
    //: }()
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(appTemporaryName)).localized, for: .normal)
        //: btn.setTitleColor(.appValueColor(), for: .normal)
        btn.setTitleColor(.multi(), for: .normal)
        ...
        btn.addTarget(self, action: #selector(the), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var postBtn: UIButton = {
        ...
        btn.setTitle((String(constLeastThoroughTitle)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.doEmbrace(colors: UIColor.monthForMomentum(), size: CGSize(width: 70, height: 30)), for: .normal)
        //: btn.layer.cornerRadius = 15
        btn.layer.cornerRadius = 15
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(postBtnClickEvent), for: .touchUpInside)
        btn.addTarget(self, action: #selector(videoTransform), for: .touchUpInside)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
    ...
    private lazy var collectionView: RedLayerEnhanceDocument = {
        //: let collectionView = FreeCollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        let collectionView = RedLayerEnhanceDocument(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        ...
        collectionView.deleteActionBlock = { [weak self] in
            ...
            if self.collectionView.imageArray.count == 0 {
                ...
                self.collectionView.contentType = HalfAdjust.nones
                //: self.videoPath = ""
                self.videoPath = ""
                //: self.coverPath = ""
                self.coverPath = ""
...
extension DestroyEventKeepDirectlyRounding {
    /// 发布动态
    ...
    func number() {
        ...
        reqManager.net(photos: self.collectionView.imageArray, content: content) { [weak self] succeed, result, errorModel in
            //: ProgressHUD.dismiss()
            ...
            NotificationCenter.default.post(name: notiStateFormat, object: self, userInfo: nil)
        }
    }
    ...
    func plateIgnore() {
        ...
        BeyondState.shared.speaker(type: 1, coverPath: self.coverPath, videoPath: self.videoPath) { coverUrl, videoUrl, succeed in
            //: guard succeed else {
            guard succeed else {
                //: ProgressHUD.dismiss()
                HandleEquipment.element()
                //: self.func__showStatusBarErrorMsg(showMsg: "Upload failed".localized)
                self.paraDown(showMsg: (String(appGapId) + String(data_curveValue.suffix(9))).localized)
                //: return
                return
            }
            ...
            dict[(String(kPinKey.prefix(6)))] = coverUrl
            //: dict["videoUrl"] = videoUrl
            dict[(app_thatPath.replacingOccurrences(of: "social", with: "i") + "oUrl")] = videoUrl
            //: dict["content"] = content
            dict[(k_techniqueString.replacingOccurrences(of: "music", with: "t"))] = content
            //: dict["location"] = ""
            dict[(String(k_correctYouName) + kMultipleFrontId.replacingOccurrences(of: "sale", with: "o"))] = ""

            //: TalkingMomentRequestTool.releaseVideoMoment(params: dict) {[weak self] succeed, result, errorModel in
            TellCharacter.exclusive(params: dict) { [weak self] succeed, result, errorModel in
                //: ProgressHUD.dismiss()
                HandleEquipment.element()
                //: guard let self = self else { return }
                guard let self = self else { return }
                ...
                self.outsideFeature(showMsg: (String(dataGrayUrl.prefix(4)) + "o rev" + String(data_missionPath.suffix(5)) + String(appRefuseFormat)).localized)
                //: self.navigationController?.popViewController(animated: true)
                self.navigationController?.popViewController(animated: true)
...
extension DestroyEventKeepDirectlyRounding {
    ...
    func broadcast() {
        ...
        StrengthMomentum.momentAccount(true) { (isOpen: Bool) in
    ...
    func con() {
        ...
        StrengthMomentum.verb(true) { (isOpen: Bool) in
            ...
            let vc = NameSave.evenColumn(maxCount: count, allowPhoto: true, allowVideo: false)
    ...
    func questionPrompt() {
        ...
        StrengthMomentum.verb(true) { (isOpen: Bool) in
            ...
            let vc = NameSave.evenColumn(maxCount: 1, allowPhoto: false, allowVideo: true)
            //: vc.modalPresentationStyle = .fullScreen
            vc.modalPresentationStyle = .fullScreen
            //: self.present(vc, animated: true)
            self.present(vc, animated: true)
            ...
            vc.didFinishPickingVideoHandle = { [weak self] (coverImage: UIImage?,
                    ...
                    NameSave.appointed(asset: assets) { filePath in
                        ...
                        if seconds >= 5 {
                            ...
                            self.divideParticle(url: filePath!)
                            //: } else {
                        } else {
                            //: DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                                //: self.func__showStatusBarErrorMsg(showMsg: "Shoot video at least 5 seconds".localized)
                                self.paraDown(showMsg: (String(const_correctData.suffix(5)) + " video" + String(dataVestFormat.suffix(6)) + "ast 5 " + String(data_familyStr) + kAlbumKey.replacingOccurrences(of: "shot", with: "co")).localized)
                            }
                        }
                    }
            }
    ...
    func divideParticle(url: URL) {
        //: let videoVC = TalkingVideoEditorController(maxDuration: 60.0, videoURL: url) { videoInfo in
        let videoVC = PopViewController(maxDuration: 60.0, videoURL: url) { videoInfo in
...
extension DestroyEventKeepDirectlyRounding: UIImagePickerControllerDelegate & UINavigationControllerDelegate {
        ...
        self.dismiss(animated: true) {
            ...
            if mediaType == kUTTypeMovie as String {
                ...
                if seconds >= 5 {
                    ...
                    self.divideParticle(url: info[.mediaURL]! as! URL)
...
extension DestroyEventKeepDirectlyRounding: FragmentSymbol {
    /// 选择图片
    ...
    func spell() {
        ...
        if self.collectionView.contentType == .Photo {
            ...
            vc.fieldThreeExpectedOperationDrawer(cellTitleList: [(String(userLeadingTeamTarUrl.prefix(6))).localized, (String(mainInjuryData)).localized])
            //: }else {
        } else {
            //: vc.initwithList(cellTitleList: ["Camera".localized, "Photo".localized, "Video".localized])
            vc.fieldThreeExpectedOperationDrawer(cellTitleList: [(String(userLeadingTeamTarUrl.prefix(6))).localized, (String(mainInjuryData)).localized, (String(kSFormat.prefix(5))).localized])
        }
    ...
    func followEstimated(index: IndexPath) {
        ...
        let vc = ResolveNorm(images: self.collectionView.imageArray, index: index.row, type: .publish)
        // 删除按钮block
    ...
    func visualState() {
        ...
        let vc = ResultInstruction(videoPath: self.videoPath)
        //: vc.modalPresentationStyle = .fullScreen
        vc.modalPresentationStyle = .fullScreen
        //: self.present(vc, animated: true)
        self.present(vc, animated: true)
...
extension DestroyEventKeepDirectlyRounding {
    ...
    func loadThat() {
        ...
        cancelBtn.snp.makeConstraints { make in
            ...
            make.top.equalTo(dataDoingRapidMsg + 7)
        ...
        collectionView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(app_whenAppearanceData)
