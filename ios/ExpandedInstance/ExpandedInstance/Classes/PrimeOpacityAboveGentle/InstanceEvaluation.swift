...
/*: "MF:LiveChatWelMsg" :*/
fileprivate let main_busyManageName:[Character] = ["M","F",":","L","i"]
fileprivate let data_judgePath:String = "veChatbean male between weak"
fileprivate let mainDetailData:String = "WelMsgprint restriction pad ley"

/*: "MF:LiveChatPrizeMsg" :*/
fileprivate let notiShapeMessage:[Character] = ["M","F",":","L","i","v","e","C","h","a","t","P","r","i","z","e","M"]
fileprivate let noti_countCableStr:String = "foundation"

/*: "MF:LiveChatAttentionMsg" :*/
fileprivate let constSureLabMsg:String = "reserve copy distinctive identity supporterMF:L"
fileprivate let main_mountPageStr:[Character] = ["A","t","t","e","n","t","i","o","n","M","s","g"]

/*: "#BAFBFF" :*/
fileprivate let dataAcrossUrl:[Character] = ["#","B","A","F","B","F","F"]

/*: "uid" :*/
fileprivate let noti_afterValue:String = "uiprior"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let app_appearButtonKey:String = "<at>@beside publish"
fileprivate let k_chooseMsg:String = "[\\S\\"

/*: "Sent " :*/
fileprivate let app_cleanParentFormat:String = "Sent subtle advertising accessible"

/*: "#FFA4CF" :*/
fileprivate let show_seaMinuteMessage:[Character] = ["#","F","F","A","4","C"]
fileprivate let show_boldString:String = "f"

/*: "Here comes " :*/
fileprivate let app_cooperativeMmTitle:[Character] = ["H","e","r","e"," ","c","o","m","e"]
fileprivate let kContextId:String = "succeed warn request conversations "

/*: "#FFE386" :*/
fileprivate let const_dimensionStr:[Character] = ["#"]
fileprivate let userSortPath:[Character] = ["F","F","E","3","8","6"]

/*: "followed the anchor" :*/
fileprivate let showSustainableId:String = "foddlow"
fileprivate let userWillBlankString:String = "size sea anchor"

/*: "level_ :*/
fileprivate let main_frameworkUrl:String = "level_below consequence plain less"

/*: "icon_lounge" :*/
fileprivate let const_shareString:[Character] = ["i","c","o","n","_","l","o","u","n","g"]
fileprivate let notiRecoverVisibleUrl:String = "gate"
...
class InstanceEvaluation: NSObject {
    ...
    func sophisticated(model: FindCharacter) -> FindCharacter {
        //: self.model = model
        self.model = model
        /// 普通文本消息,礼物消息
        ...
        } else if self.model.MsgExtension == (String(main_busyManageName) + String(data_judgePath.prefix(6)) + String(mainDetailData.prefix(6))) {
            //: caculateWelMsg()
            gapHandle()
            /// 中奖消息
            //: } else if self.model.MsgExtension == "MF:LiveChatPrizeMsg" {
        } else if self.model.MsgExtension == (String(notiShapeMessage) + noti_countCableStr.replacingOccurrences(of: "foundation", with: "sg")) {
        ...
        } else if self.model.MsgExtension == (String(constSureLabMsg.suffix(4)) + "iveChat" + String(main_mountPageStr)) {
            //: caculAttionMsg()
            atCurve()
        }
        //: return self.model
    ...
    func inner() {
        ...
        let nameAttr = formDoing(text: firstStr, font: UIFont.wantStream(fontSize: 14), color: UIColor(hex: (String(dataAcrossUrl)))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
        ...
        if self.model.msgInfo?.messageType == 1 {
            ...
            let content = formDoing(text: self.model.msgInfo?.content ?? "", font: UIFont.wantStream(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            let atUid = jumps?[(noti_afterValue.replacingOccurrences(of: "prior", with: "d"))] as? Int ?? 0
            //: let text = self.model.msgInfo?.content ?? ""
            let text = self.model.msgInfo?.content ?? ""
            //: let regex = try! NSRegularExpression(pattern: "<at>@[\\S\\s]+?</at>", options: [])
            let regex = try! NSRegularExpression(pattern: (String(app_appearButtonKey.prefix(5)) + k_chooseMsg + "s]+?</at>"), options: [])
            ...
            if matchRange?.length ?? 0 > 9 {
                ...
                let subStr = String.deliveryTo(str: text, from: (matchRange?.location ?? 0) + 4, to: (matchRange?.length ?? 0) - 5)
                //: let contentStr = String.substringByUnicodeIndex(str: text, from: (matchRange?.length ?? 0), to: text.utf16.count )
                let contentStr = String.deliveryTo(str: text, from: (matchRange?.length ?? 0), to: text.utf16.count)
                ...
                let atName = formDoing(text: subStr, font: UIFont.wantStream(fontSize: 14), color: UIColor(hex: (String(dataAcrossUrl)))!, isTap: true, atUid: "\(atUid)")
                ...
                let content = formDoing(text: contentStr, font: UIFont.wantStream(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            let sentAttr = formDoing(text: (String(app_cleanParentFormat.prefix(5))).localized, font: UIFont.wantStream(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            if let toUser = self.model.toUser { // 送礼接收人
                ...
                let nickNameAttr = formDoing(text: "\(toUser.nickname) ", font: UIFont.alongsideOk(type: .Medium, fontSize: 14), color: UIColor(hex: (String(show_seaMinuteMessage) + show_boldString.uppercased()))!, isTap: true, atUid: String(toUser.uid))
                //: nameAttr.append(nickNameAttr)
                nameAttr.append(nickNameAttr)
            }
            //: let giftnameStr = changeGiftName(giftmodel: self.model.gift!)
            let giftnameStr = tooDown(giftmodel: self.model.gift!)
            ...
            let giftAttr = formDoing(text: giftnameStr, font: UIFont.alongsideOk(type: .Medium, fontSize: 14), color: UIColor(hex: (String(show_seaMinuteMessage) + show_boldString.uppercased()))!, isTap: false, atUid: nil)
            ...
            let imag = self.model.gift?.giftImg?.policyCityDateLimited(with: size)
            ...
            let giftIcon = NSMutableAttributedString.yy_attachmentString(withContent: imag, contentMode: .center, attachmentSize: size, alignTo: UIFont.wantStream(fontSize: 14), alignment: .center)
            //: giftIcon.yy_paragraphStyle = paragraphStyle()
            giftIcon.yy_paragraphStyle = rounding()
            ...
            let numAttr = self.formDoing(text: "*\(self.model.gift?.num ?? 0)", font: UIFont.wantStream(fontSize: 14), color: UIColor(hex: (String(show_seaMinuteMessage) + show_boldString.uppercased()))!, isTap: false, atUid: nil)
    ...
    func mention(model: FindCharacter) -> FindCharacter {
        //: self.model = model
        self.model = model
        // 内容
        //: let paraStyle = paragraphStyle()
        ...
        let content = formDoing(text: self.model.msgInfo?.content ?? "", font: UIFont.wantStream(fontSize: 14), color: UIColor(hex: (String(dataAcrossUrl)))!, isTap: false, atUid: nil)
    ...
    func gapHandle() {
        ...
        let strnor = (String(app_cooperativeMmTitle) + String(kContextId.suffix(2))).localized
        ...
        let content = formDoing(text: strnor, font: UIFont.wantStream(fontSize: 14), color: UIColor(hex: (String(const_dimensionStr) + String(userSortPath)))!, isTap: false, atUid: nil)
        ...
        let attachText = formDoing(text: strc, font: UIFont.wantStream(fontSize: 14), color: UIColor(hex: (String(dataAcrossUrl)))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
    ...
    func rePair() {
        ...
        let content = formDoing(text: firstStr, font: UIFont.wantStream(fontSize: 14), color: UIColor.white, isTap: true, atUid: String(self.model.user?.uid ?? 0))
        //: textView.append(content)
        textView.append(content)
        //: self.model.msgAttribText = textView
        self.model.msgAttribText = textView
    ...
    func atCurve() {
        ...
        let strnor = (showSustainableId.replacingOccurrences(of: "odd", with: "ol") + "ed the" + String(userWillBlankString.suffix(7))).localized
        //: let strc =  "\(self.model.user?.nickname ?? "") "
        let strc = "\(self.model.user?.nickname ?? "") "
        ...
        let content = formDoing(text: strnor, font: UIFont.wantStream(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
        ...
        let nameText = formDoing(text: strc, font: UIFont.wantStream(fontSize: 14), color: UIColor(hex: (String(dataAcrossUrl)))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
    ...
    func rounding() -> NSMutableParagraphStyle {
    ...
    func formDoing(text: String, font: UIFont, color: UIColor, isTap: Bool, atUid: String?) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = rounding()
        ...
        if isTap == true {
            ...
            attribute.yy_setTextHighlight(range, color: color, backgroundColor: .clear, userInfo: nil) { containerView, text, range, rect in
                ...
                /// 不能@自己
                //: if atUid != QuantityeractionDelay.share.loginUserMode.userID {
                if atUid != QuantityeractionDelay.share.loginUserMode.userID {
    ...
    func tip(attribText: NSMutableAttributedString) {
        ...
        let maxWidth = mainInstanceDespiteMaterialTitle - 16
    ...
    private func tooDown(giftmodel: DarkLost) -> String {
...
extension InstanceEvaluation {
    /// 获取弹幕富文本图片
    //: func getImagesAttributedString() -> NSMutableAttributedString {
    func yellow() -> NSMutableAttributedString {
        ...
        levelImgV.image = UIImage.transferAndJoinSmall(name: (String(main_frameworkUrl.prefix(6))) + "\(self.model.user?.level ?? 1)")
        //: let levelAttr = getAttachText(imageView: levelImgV)
        let levelAttr = functional(imageView: levelImgV)
        ...
        if self.model.user?.loungePlus == true {
            ...
            vipImgV.image = UIImage.transferAndJoinSmall(name: (String(const_shareString) + notiRecoverVisibleUrl.replacingOccurrences(of: "gate", with: "e")))
            //: let vipAttr = getAttachText(imageView: vipImgV)
            let vipAttr = functional(imageView: vipImgV)
    ...
    func functional(imageView: UIImageView) -> NSMutableAttributedString {
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView.image, contentMode: .scaleToFill, attachmentSize: imageView.size, alignTo: UIFont.botLocal(fontSize: 12), alignment: .center)
        //: attachText.yy_paragraphStyle = paragraphStyle()
        attachText.yy_paragraphStyle = rounding()
        //: return attachText
        return attachText
    }
}
