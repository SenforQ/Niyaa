...
/*: "icon_randow_user_bg" :*/
fileprivate let noti_onAnotherName:[Character] = ["i","c","o","n","_","r","a","n","d"]
fileprivate let kQuantityryUrl:String = "help accelerateow_"
fileprivate let showHoneyNothingData:String = "user_bgrow seek tab"
...
class ModelSendView: UIView {
    // MARK: - UI Components

    //: private let leftAvatarView = AvatarView()
    private let leftAvatarView = FixedGem()
    //: private let centerAvatarView = AvatarView()
    private let centerAvatarView = FixedGem()
    //: private let rightAvatarView = AvatarView()
    private let rightAvatarView = FixedGem()
    ...
    private func bassVoice() {
        ...
        centerBgView.image = UIImage.transferAndJoinSmall(name: (String(noti_onAnotherName) + String(kQuantityryUrl.suffix(3)) + String(showHoneyNothingData.prefix(7))))
    }

    //: private func updateAvatarDisplay() {
    private func beforePropertyDestination() {
...
extension ModelSendView {
    // MARK: - Public Methods

    //: func configure(avatars: [String]) {
    func fractionFor(avatars: [String]) {
...
class FixedGem: UIView {
    ...
    private func clusterDistribution() {
        //: imageView.setUrlImage(urlStr: self.imageurl ?? "")
        imageView.okUponLock(urlStr: self.imageurl ?? "")
    }
}
