...
fileprivate let showReFormat:[UInt8] = [0x3d,0x42,0x3d,0x48,0xfc,0x37,0x43,0x38,0x39,0x46,0xe,0xfd,0xf4,0x3c,0x35,0x47,0xf4,0x42,0x43,0x48,0xf4,0x36,0x39,0x39,0x42,0xf4,0x3d,0x41,0x44,0x40,0x39,0x41,0x39,0x42,0x48,0x39,0x38]

fileprivate func modelResident(addition num: UInt8) -> UInt8 {
...
/*: "icon_chat_start" :*/
fileprivate let dataConstraintUrl:String = "due accessicon_c"
fileprivate let dataAreaSuccessfulInjuryPath:[Character] = ["h","a","t","_","s","t","a","r","t"]

/*: "#752FE9" :*/
fileprivate let main_originHarassmentId:String = "#752Fwritten strength"
fileprivate let k_canvasRoundingFormat:String = "Eaccessible"

/*: "#F5F5F5" :*/
fileprivate let data_itKey:String = "#F5F5appeal automatically"
fileprivate let appBetterCrystalString:[Character] = ["F","5"]

/*: "btn_feed_dianzan_nor" :*/
fileprivate let show_divideBehaviorData:[UInt8] = [0x7c,0x8e,0x88,0x79,0x80,0x7f,0x7f,0x7e,0x79,0x7e,0x83,0x7b,0x88,0x94,0x7b,0x88,0x79,0x88,0x89,0x8c]

fileprivate func titleMin(gray num: UInt8) -> UInt8 {
...
/*: "btn_feed_reply_nor" :*/
fileprivate let dataGroupFormat:[Character] = ["b","t","n","_","f","e","e","d","_"]
fileprivate let showAffectString:[Character] = ["r","e","p"]
fileprivate let constExistValue:[Character] = ["l","y","_","n","o","r"]
...
class ProfileExecute: UITableViewCell {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showReFormat.map{modelResident(addition: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var playView: UIImageView = {
        ...
        img.image = UIImage.transferAndJoinSmall(name: (String(dataConstraintUrl.suffix(6)) + String(dataAreaSuccessfulInjuryPath)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var atNickNameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(main_originHarassmentId.prefix(5)) + k_canvasRoundingFormat.replacingOccurrences(of: "accessible", with: "9")))
    ...
    private lazy var timeLab: UILabel = {
        ...
        lab.font = UIFont.wantStream(fontSize: 12)
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var mainContentView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(data_itKey.prefix(5)) + String(appBetterCrystalString)))
...
extension ProfileExecute {
    ...
    func allow(_ model: WindowResult) {
        ...
        previewImgView.okUponLock(urlStr: model.imageUrl)
        ...
        let typeImgStr = (model.type == 0) ? String(bytes: show_divideBehaviorData.map{titleMin(gray: $0)}, encoding: .utf8)! : (String(dataGroupFormat) + String(showAffectString) + String(constExistValue))
        //: statusIcon.image = UIImage.BundleImageNamed(name: typeImgStr)
        statusIcon.image = UIImage.transferAndJoinSmall(name: typeImgStr)
...
extension ProfileExecute {
    ...
    private func drawing() {
        ...
        nickNameLab.snp.makeConstraints { make in
            ...
            make.width.lessThanOrEqualTo((dataTabUrl - 128) / 2)
        }
