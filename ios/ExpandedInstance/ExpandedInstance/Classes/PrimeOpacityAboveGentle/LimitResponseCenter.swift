...
/*: "#BAFBFF" :*/
fileprivate let constNormString:[Character] = ["#","B","A","F","B","F","F"]
...
class LimitResponseCenter: NSObject {
    ...
    func proThen(model: MethodDrawer) -> MethodDrawer {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func infoSwitchly() {
        ...
        let name = streamCenter(text: firstStr, font: UIFont.wantStream(fontSize: 14), color: UIColor(hex: (String(constNormString)))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = streamCenter(text: self.model.content, font: UIFont.wantStream(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        boxListener(attribText: self.model.msgAttribText)
    }

    //: func caculateTransComment() {
    func mainForSecureAttach() {
        //: let paraStyle = paragraphStyle()
        let paraStyle = awayAcross()
        //: paraStyle.lineSpacing = 3.0
        paraStyle.lineSpacing = 3.0 // 行间距
        ...
        let content = streamCenter(text: self.model.transContent, font: UIFont.wantStream(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func awayAcross() -> NSMutableParagraphStyle {
    ...
    func streamCenter(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = awayAcross()
        //: return attribute
        return attribute
    }
    ...
    func boxListener(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = appConsumeHandleId - 36
