...
fileprivate let app_feedbackName:[UInt8] = [0xd7,0xd0,0xd7,0xca,0x96,0xdd,0xd1,0xda,0xdb,0xcc,0x84,0x97,0x9e,0xd6,0xdf,0xcd,0x9e,0xd0,0xd1,0xca,0x9e,0xdc,0xdb,0xdb,0xd0,0x9e,0xd7,0xd3,0xce,0xd2,0xdb,0xd3,0xdb,0xd0,0xca,0xdb,0xda]

private func feedVisitAd(detail num: UInt8) -> UInt8 {
    return num ^ 190
}
...
class ReductionText: ScaleViewController {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: RoundingStatusInsert!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = RoundingStatusInsert(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: app_feedbackName.map{feedVisitAd(detail: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class RoundingStatusInsert: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: dataTabUrl, height: const_rebuildWithinId * 0.6)
    ...
    func floorRespective(corners: UIRectCorner, radius: CGFloat) {
...
extension RoundingStatusInsert: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source: UIViewController) -> UIPresentationController? {
        ...
        let res = ExceptController(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension RoundingStatusInsert: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            vertical(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            weekDoing(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func vertical(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func weekDoing(transitionContext: UIViewControllerContextTransitioning) {
...
class ExceptController: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: RoundingStatusInsert!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: RoundingStatusInsert, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func recto() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(account))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: dataTabUrl, height: const_rebuildWithinId)
