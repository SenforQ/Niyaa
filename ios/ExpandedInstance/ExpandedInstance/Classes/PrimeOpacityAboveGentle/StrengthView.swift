...
/*: "cellID" :*/
fileprivate let dataConvertStyleId:String = "integration around appointed writecellID"

/*: "init(coder:) has not been implemented" :*/
fileprivate let show_currentEvaluationName:[UInt8] = [0x4,0x9,0x4,0xf,0xc3,0xfe,0xa,0xff,0x0,0xd,0xd5,0xc4,0xbb,0x3,0xfc,0xe,0xbb,0x9,0xa,0xf,0xbb,0xfd,0x0,0x0,0x9,0xbb,0x4,0x8,0xb,0x7,0x0,0x8,0x0,0x9,0xf,0x0,0xff]

fileprivate func pendingInstruction(purple num: UInt8) -> UInt8 {
...
/*: "Video Call" :*/
fileprivate let mainReductionMessage:[Character] = ["V","i","d","e","o"]
fileprivate let show_litePerId:String = " Callkey answer"

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let appLostTitle:String = "Video return stack until"
fileprivate let data_vestStr:String = "please draw observe must%@ Po"
fileprivate let notiFixedName:[Character] = ["i","n","t","s","/","M","i","n",")"]

/*: "Voice Call" :*/
fileprivate let showZoneData:String = "Voicetoward subtle define front"

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let dataReserveName:[Character] = ["V","o","i","c","e"," ","C","a","l","l"," ","(","%","@"," ","P","o","i","n","t","s"]
fileprivate let user_sablePresentName:[Character] = ["/","M","i","n",")"]

/*: "#F8F8F8" :*/
fileprivate let notiTingStr:String = "#F8F8F8tailed bag"

/*: "Cancel" :*/
fileprivate let k_seekCornerMsg:[Character] = ["C","a","n","c","e","l"]

/*: "#F5F5F5" :*/
fileprivate let userAnotherName:String = "#F5F5F5during dilemma"

/*: "icon_lounge" :*/
fileprivate let data_handleTitle:[Character] = ["i","c","o","n","_","l","o","u"]
fileprivate let const_linkAccountCommentUrl:[Character] = ["n","g","e"]
...
class StrengthView: UIView {
    ...
    private var type: CouldCanvas = .Unknown // 当前展示类型
    ...
    var cellTitleColor = UIColor.thinOf()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.alongsideOk(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.appValueDetailColor()
    var cancelColor = UIColor.underCountensityLanguage()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.alongsideOk(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.multiSince()
    /// 标题字体大小
    //: var titleFont = UIFont.pingfangMediumFont(fontSize: 16)
    var titleFont = UIFont.wantStream(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (String(dataConvertStyleId.suffix(6)))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + constCountervalMaterialMsg) : constCountervalMaterialMsg
    }
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: show_currentEvaluationName.map{pendingInstruction(purple: $0)}, encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func fieldThreeExpectedOperationDrawer(cellTitleList: [String]!) {
        // 初始化
    ...
    func petEffect(type: CouldCanvas, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if QuantityeractionDelay.share.appStatus == TotalervalObserve.normal.rawValue {
                ...
                if QuantityeractionDelay.share.loginUserMode.sex == CommonAmong.male.rawValue {
                    ...
                    let videoAttrStr = String.suspend(VideoPrice: Int(videoPrice ?? "0") ?? 0)
                    //: let voiceAttrStr = String.setVoicePriceStr(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    let voiceAttrStr = String.appDepth(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    ...
                    let attributes = [.foregroundColor: UIColor.multiSince(), .font: UIFont.alongsideOk(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(mainReductionMessage) + String(show_litePerId.prefix(5))).localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(appLostTitle.prefix(6)) + "Call (" + String(data_vestStr.suffix(5)) + String(notiFixedName)).underConnection(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(showZoneData.prefix(5)) + " Call").localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(dataReserveName) + String(user_sablePresentName)).underConnection(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(mainReductionMessage) + String(show_litePerId.prefix(5))).localized, (String(showZoneData.prefix(5)) + " Call").localized]
            }
        }
    ...
    public func programSay() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: const_rebuildWithinId - tableViewHight, width: dataTabUrl, height: tableViewHight), style: .plain)
        ...
        tableView.register(InsideFlow.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.asAffect(width: dataTabUrl,
...
extension StrengthView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? InsideFlow
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = InsideFlow(style: .default, reuseIdentifier: cellID)
        }

        //: if QuantityeractionDelay.share.loginUserMode.sex == Gender.male.rawValue,
        if QuantityeractionDelay.share.loginUserMode.sex == CommonAmong.male.rawValue,
           //: QuantityeractionDelay.share.loginUserMode.loungePlus == true,
           QuantityeractionDelay.share.loginUserMode.loungePlus == true,
           //: type == .Call {
           type == .Call
        {
        ...
        cell!.titleLab.font = cellTitleFont
        //: if self.type == .Call && QuantityeractionDelay.share.appStatus == AppSkinStatus.normal.rawValue {
        if self.type == .Call && QuantityeractionDelay.share.appStatus == TotalervalObserve.normal.rawValue {
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: dataTabUrl, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: dataTabUrl, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (String(notiTingStr.prefix(7))))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: dataTabUrl, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((String(k_seekCornerMsg)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(thoroughOne), for: .touchUpInside)
        //: return footerView
        return footerView
    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: dataTabUrl, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: dataTabUrl - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: dataTabUrl, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(userAnotherName.prefix(7))))
...
class InsideFlow: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.transferAndJoinSmall(name: (String(data_handleTitle) + String(const_linkAccountCommentUrl)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(userAnotherName.prefix(7))))
        //: return lab
        return lab
        //: }()
    }()
