...
/*: "live_ended" :*/
fileprivate let data_hideWrittenName:String = "crucial lose her short recognizelive_en"
fileprivate let appPerformValue:String = "publisherd"

/*: "The live broadcast has ended" :*/
fileprivate let user_picStr:[Character] = ["T","h","e"," ","l","i","v","e"," ","b","r","o","a","d","c","a"]
fileprivate let const_nowTitle:String = "analyze numb hundred server falsest has"

/*: "bth_live_male_phone_nor" :*/
fileprivate let appCoverAltogetherMessage:String = "bth_libegin agree please gender audience"
fileprivate let app_marginTitle:String = "shadow priority cut discount bettere_pho"
fileprivate let notiLimitedName:[Character] = ["n","e","_","n","o","r"]

/*: "floatingScreen" :*/
fileprivate let dataHerName:[Character] = ["f","l","o","a","t","i","n","g","S","c","r","e","e","n"]

/*: "You have been muted" :*/
fileprivate let appMinuteTabShouldName:[Character] = ["Y","o","u"," ","h","a","v","e"," ","b","e","e","n"," ","m","u","t"]
fileprivate let user_telescopeName:String = "eta"

/*: "You have entered the other party's blacklist" :*/
fileprivate let mainCapableSameName:[UInt8] = [0x74,0x73,0x69,0x6c,0x6b,0x63,0x61,0x6c,0x62,0x20,0x73,0x27,0x79,0x74,0x72,0x61,0x70,0x20,0x72,0x65,0x68,0x74,0x6f,0x20,0x65,0x68,0x74,0x20,0x64,0x65,0x72,0x65,0x74,0x6e,0x65,0x20,0x65,0x76,0x61,0x68,0x20,0x75,0x6f,0x59]

/*: "Live room exception, please return to retry, if multiple failures please contact the staff" :*/
fileprivate let noti_borderGoPath:[UInt8] = [0x74,0x51,0x4e,0x5d,0x18,0x4a,0x57,0x57,0x55,0x18,0x5d,0x40,0x5b,0x5d,0x48,0x4c,0x51,0x57,0x56,0x14,0x18,0x48,0x54,0x5d,0x59,0x4b,0x5d,0x18,0x4a,0x5d,0x4c,0x4d,0x4a,0x56,0x18,0x4c,0x57,0x18,0x4a,0x5d,0x4c,0x4a,0x41,0x14,0x18,0x51,0x5e,0x18,0x55,0x4d,0x54,0x4c,0x51,0x48,0x54,0x5d,0x18,0x5e,0x59,0x51,0x54,0x4d,0x4a,0x5d,0x4b,0x18,0x48,0x54,0x5d,0x59,0x4b,0x5d,0x18,0x5b,0x57,0x56,0x4c,0x59,0x5b,0x4c,0x18,0x4c,0x50,0x5d,0x18,0x4b,0x4c,0x59,0x5e,0x5e]

private func normWarn(attribute num: UInt8) -> UInt8 {
    return num ^ 56
}

/*: "isCalling" :*/
fileprivate let appFinishEffMsg:[Character] = ["i","s","C","a","l","l","i","n","g"]
...
class DuringInformation: ScaleViewController {
    //: var type = LiveEnterType.userDetail
    var type = DirectChain.userDetail // 入口
    ...
    override func viewDidLoad() {
        ...
        WillProCommon.enablee().delegate = self
    ...
    private lazy var videoCallManager: GroupStatus = {
        ...
        m.uid = String(RedWith.atPerform().liveRoomModel.streamerInfo.uid)
        //: return m
        return m
        //: }()
    }()
    ...
    private lazy var stopLiveBgView: UIView = {
        ...
        let icon = UIImageView(image: UIImage.transferAndJoinSmall(name: (String(data_hideWrittenName.suffix(7)) + appPerformValue.replacingOccurrences(of: "publisher", with: "de"))))
        //: bg.addSubview(icon)
        bg.addSubview(icon)
        //: icon.snp.makeConstraints { make in
        icon.snp.makeConstraints { make in
            //: make.top.equalTo(StatusBarHeight+actualWidth(w: 205))
            make.top.equalTo(dataDoingRapidMsg + actualWidth(w: 205))
        ...
        lab.text = (String(user_picStr) + String(const_nowTitle.suffix(6)) + " ended").localized
        //: bg.addSubview(lab)
        bg.addSubview(lab)
    ...
    private lazy var bottomView: BarDivide = {
        //: let view = TalkingLiveRoomBottomView(frame: .zero)
        let view = BarDivide(frame: .zero)
        //: view.refreshBottomView(TalkingLiveManager.shared().liveRoomModel)
        view.counterruptRefresh(RedWith.atPerform().liveRoomModel)
        //: view.delegate = self
        view.delegate = self
        //: return view
        return view
    ...
    private lazy var commInputView: SumBackground = {
        //: let text = TalkingLiveRoomInputView.init()
        let text = SumBackground()
        //: text.delegate = self
        text.delegate = self
        //: text.isHidden = true
        text.isHidden = true
    ...
    private lazy var danmuView: RestrictionView = {
        //: let view = TalkingDanmuMsgListTableView.init()
        let view = RestrictionView()
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var callBtn: UIButton = {
        ...
        btn.setImage(UIImage.transferAndJoinSmall(name: (String(appCoverAltogetherMessage.prefix(6)) + "ve_mal" + String(app_marginTitle.suffix(5)) + String(notiLimitedName))), for: .normal)
        //: btn.addTarget(self, action: #selector(callButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(cleanCreate), for: .touchUpInside)
    ...
    private lazy var giftEffectView: CellView = {
        //: let effectView = TalkingPrivateChatAnimatView.init()
        let effectView = CellView()
    ...
    private lazy var giftTrackView: FragmentPublish = {
        //: let trackView = TalkingGiftTrackView.init()
        let trackView = FragmentPublish()
    ...
    private lazy var floatScreenView: RangeOrientation = { // 飘屏
        //: let v = TalkingFloatScreenView()
        let v = RangeOrientation()
        //: v.isHidden = (QuantityeractionDelay.share.appStatus == AppSkinStatus.special.rawValue)
        v.isHidden = (QuantityeractionDelay.share.appStatus == TotalervalObserve.special.rawValue)
        //: return v
        return v
        //: }()
    }()
...
extension DuringInformation {
    ...
    @objc private func orange() {
        ...
        self.player.startLivePlay(RedWith.atPerform().liveRoomModel.livePlayUrl, type: TX_Enum_PlayType.PLAY_TYPE_LIVE_FLV)
    }
    ...
    func among() {
        ...
        WillProCommon.refuse(groupId: RedWith.atPerform().liveRoomModel.chatGroupId, announcementMsg: RedWith.atPerform().liveRoomModel.notification)

        //: self.bottomView.toUid = String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid)
        self.bottomView.toUid = String(RedWith.atPerform().liveRoomModel.streamerInfo.uid)
    }
    ...
    func fastProduceer() {
        ...
        V2TIMManager.sharedInstance().quitGroup(RedWith.atPerform().liveRoomModel.chatGroupId, succ: {
            //: }, fail: { code, str in
        }, fail: { code, str in
            //: })
        })
...
extension DuringInformation {
    ...
    @objc func bounceCycle() {
        ...
        danmuView.snp.updateConstraints { make in
            ...
            make.bottom.equalTo(bottomView.snp.top).offset(-(const_rebuildWithinId - notiGiftEnterData - bottomView.height - constCountervalMaterialMsg))
        }
    }
    ...
    @objc func name() {
        ...
        for vc in self.children {
            ...
            if vc.isKind(of: BarViewController.self) || vc.isKind(of: MovementSkirt.self) {
                //: ishave += 1
                ishave += 1
            }
        }
...
extension DuringInformation {
    ...
    @objc private func cleanCreate() {
        //: var videoStr = TalkingLiveManager.shared().liveRoomModel.videoPrice
        var videoStr = RedWith.atPerform().liveRoomModel.videoPrice
        //: var voiceStr = TalkingLiveManager.shared().liveRoomModel.voicePrice
        var voiceStr = RedWith.atPerform().liveRoomModel.voicePrice
        //: if QuantityeractionDelay.share.loginUserMode.loungePlus {
        if QuantityeractionDelay.share.loginUserMode.loungePlus {
            //: videoStr = TalkingLiveManager.shared().liveRoomModel.videoVIPPrice
            videoStr = RedWith.atPerform().liveRoomModel.videoVIPPrice
            //: voiceStr = TalkingLiveManager.shared().liveRoomModel.voiceVIPPrice
            voiceStr = RedWith.atPerform().liveRoomModel.voiceVIPPrice
        }
        //: videoCallManager.showVideoCallMenu(videoPrice: "\(videoStr)", voicePrice: "\(voiceStr)")
        videoCallManager.isMarkOf(videoPrice: "\(videoStr)", voicePrice: "\(voiceStr)")
        ...
        case .match:
            ...
            k_playShadowValue.behindImplement(eventID: constImpactMsg, toUid: String(RedWith.atPerform().liveRoomModel.streamerInfo.uid))
        ...
        case .userDetail:
            ...
            k_playShadowValue.behindImplement(eventID: constDeliveryStr, toUid: String(RedWith.atPerform().liveRoomModel.streamerInfo.uid))
    ...
    func afterEnable(animated: Bool = true) {
        //: self.topView.stopTimer()
        self.topView.back()
        //: req_quitGroup()
        fastProduceer() // 退出群聊
        //: TalkingLiveManager.destroy()
        RedWith.thresholdDownAgainst() // 销毁单例
        //: TalkingDanmuManager.shared().delegate = nil
        WillProCommon.enablee().delegate = nil
        ...
        guard let currentVC = moreHide() else { return }
    ...
    @objc private func doSimultaneously(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let dict = userinfo["floatingScreen"]
        let dict = userinfo[(String(dataHerName))]
        //: if let model = TalkingFloatingScreenModel.deserialize(from: dict as? Dictionary) {
        if let model = DisabledModel.deserialize(from: dict as? Dictionary) {
            //: self.floatScreenView.didReceive(model)
            self.floatScreenView.conversationBy(model)
        }
    }
}
...
extension DuringInformation: ResponsePath {
    //: func func__commentBtnClick() {
    func pastAppear() {
        //: if Int(Date().timeIntervalSince1970) < TalkingLiveManager.shared().liveRoomModel.muteExpireAt {
        if Int(Date().timeIntervalSince1970) < RedWith.atPerform().liveRoomModel.muteExpireAt {
            //: self.func__showStatusBarErrorMsg(showMsg: "You have been muted".localized)
            self.paraDown(showMsg: (String(appMinuteTabShouldName) + user_telescopeName.replacingOccurrences(of: "ta", with: "d")).localized)
            //: return
            return
        }
        /// 拉起弹幕评论
        //: commInputView.updatePlaceholder()
        commInputView.outVariable()
    }
}
...
extension DuringInformation: MapBrushRate {
    ...
    func sufficient(Msg: FindCharacter) {
        ...
        danmuView.includeNowJumpResistance(msgModel: Msg)
    }
    ...
    func board(pushUid: String?) {
        ...
        let view = CountoSkin(frame: self.view.frame, uid: pushUid ?? "")
        //: view.delegate = self
        view.delegate = self
        //: view.show()
        view.secure()
    ...
    func heroBasic(nickName: String, atUid: String?) {
        ...
        commInputView.instead(toUid: atUid ?? "", nickName: nickName)
    }
...
extension DuringInformation {
    ...
    func info(Msg: FindCharacter) {
        //: if giftTrackView.giftSendItem.count == 0 {
        if giftTrackView.giftSendItem.count == 0 {
            //: let arrM = NSMutableArray.init()
            let arrM = NSMutableArray()
            //: for model in TalkingChatGiftManager.share.getSendingItems() {
            for model in KindChain.share.gapFormatBlueBox() {
                //: arrM.add(model)
                arrM.add(model)
            }
            //: giftTrackView.giftSendItem = arrM as! [TalkingGiftNumArrModel]
            giftTrackView.giftSendItem = arrM as! [ThresholdWrite]
        }
        ...
        if giftModel != nil {
            ...
            giftTrackView.joinBy(model: giftModel!)
            //: giftEffectView.addGiftAnimatModelArr(modelArr: [giftModel!])
            giftEffectView.customFast(modelArr: [giftModel!])
        }
    }
}
...
extension DuringInformation: ImmediateDrawer {
    ...
    func throughWithinPicture(heightToBottom: CGFloat) {
        ...
        danmuView.snp.updateConstraints { make in
            ...
            let height = heightToBottom == 0 ? 0 : -(heightToBottom - bottomView.height + commInputView.height - constCountervalMaterialMsg)
    ...
    func yesterdaySlide(msgStr: String, toUid: String?) {
        ...
        WillProCommon.hardInfo(groupId: RedWith.atPerform().liveRoomModel.chatGroupId, message: msgStr, toUid: toUid) { succeed, result, errorModel in
            //: if succeed {
            if succeed {}
        }
    }
...
extension DuringInformation: V2TIMGroupListener {
    /// 被主播拉黑后，禁言退出房间
    //: func onMemberInfoChanged(_ groupID: String!, changeInfoList: [V2TIMGroupMemberChangeInfo]!) {
    func onMemberInfoChanged(_ groupID: String!, changeInfoList: [V2TIMGroupMemberChangeInfo]!) {
        //: if changeInfoList.first?.userID == QuantityeractionDelay.share.loginUserMode.userID {
        if changeInfoList.first?.userID == QuantityeractionDelay.share.loginUserMode.userID {
            //: let toastStr = "You have entered the other party's blacklist".localized
            let toastStr = String(bytes: mainCapableSameName.reversed(), encoding: .utf8)!.localized
...
extension DuringInformation {
    ...
    @objc private func tagToTransform() {
...
extension DuringInformation {
    ...
    @objc func someTheme(note: Notification) {
...
extension DuringInformation: TXLivePlayListener {
    ...
    func onPlayEvent(_ evtID: Int32, withParam param: [AnyHashable: Any]!) {
        ...
        { // 观众侧拉流失败，退出直播间
            ...
            self.paraDown(showMsg: String(bytes: noti_borderGoPath.map{normWarn(attribute: $0)}, encoding: .utf8)!.localized)
            //: popCurrentViewController()
            afterEnable()
        }
    }
...
extension DuringInformation {
    ...
    private func model() {
        ...
        self.view.backgroundColor = .thinOf()
    ...
    private func announcementEnable() {
        ...
        danmuView.snp.makeConstraints { make in
            ...
            make.height.equalTo(user_includeFormat)
            //: make.width.equalTo(MsgTableViewWidth)
            make.width.equalTo(mainInstanceDespiteMaterialTitle)
        }
        ...
        giftTrackView.snp.makeConstraints { make in
            ...
            make.bottom.equalTo(bottomView.snp.top).offset(-user_includeFormat)
        }
    }
    ...
    func listen() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(someTheme(note:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(tagToTransform),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(orange),
                                               //: name: LIVE_RECIVE_STARTLIVE_NOTIFICATION,
                                               name: noti_colorString,
                                               //: object: nil)
                                               object: nil)
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(doSimultaneously(notification:)),
                                               //: name: LIVE_RECIVE_FLOATSCREEN_NOTIFICATION,
                                               name: constEnterMessage,
                                               //: object: nil)
                                               object: nil)
        ...
        UpZone.shared.rx
            ...
            .observeWeakly(Bool.self, (String(appFinishEffMsg)))
        ...
        //: TalkingSocketManager.shared.rx
        UpZone.shared.rx
            .observeWeakly(Bool.self, "agileImageSkewy")
            //: .subscribe(onNext: { [weak self] (value) in
            .subscribe(onNext: { [weak self] value in
                //: guard let self = self else { return }
                guard let self = self else { return }
