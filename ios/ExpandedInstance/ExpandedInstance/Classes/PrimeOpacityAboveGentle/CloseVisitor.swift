...
fileprivate let noti_clubKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "all" :*/
fileprivate let appSecularDoingString:String = "adivide"
...
class CloseVisitor: LocateMight {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: DespiteEnhance?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: noti_clubKey.reversed(), encoding: .utf8)!)
    }
...
extension CloseVisitor {
    ...
    override func panel(with data: CleanShared) {
        ...
        self.giftData = data as? DespiteEnhance
        ...
        self.giftImgView.okUponLock(urlStr: imgPreview)
        //: self.giftContentLab.attributedText = giftData.attributedString
        self.giftContentLab.attributedText = giftData.attributedString
        //: if giftData.msgModel.toUser.uid == "all" { // 送礼所有人
        if giftData.msgModel.toUser.uid == (appSecularDoingString.replacingOccurrences(of: "divide", with: "ll")) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.transferAndJoinSmall(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.okUponLock(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
