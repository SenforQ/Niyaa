...
/*: "serialQueue" :*/
fileprivate let app_heartMsg:String = "senowa"

/*: "fromUids" :*/
fileprivate let kPresentationData:[UInt8] = [0x39,0x2d,0x30,0x32,0xa,0x36,0x3b,0x2c]

//: Declare String End

// __DEBUG__
// __CLOSE_PRINT__
//  FinishAssociated.swift
//  AbroadTalking
//
...
public class FinishAssociated: NSObject {
    ...
    private static var _instance: FinishAssociated?
    //: class func shared() -> DBUserInfoManager {
    class func belowSession() -> FinishAssociated {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = FinishAssociated()
            // 添加退出账号通知
            //: NotificationCenter.default.addObserver(_instance!,
            NotificationCenter.default.addObserver(_instance!,
                                                   //: selector: #selector(accountLogout),
                                                   selector: #selector(broadLikeCancelMaterial),
    ...
    @objc private func broadLikeCancelMaterial() {
        //: DBUserInfoManager.destroy()
        FinishAssociated.blueNumber()
    }
}
...
extension FinishAssociated {
    /// 获取用户消息
    /// - Parameters:
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([BetweenInput]?, Error?) -> Void) {
    class func composition(with userIdsArr: [String], completion: @escaping ([BetweenInput]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if FinishAssociated.belowSession().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(FinishAssociated.belowSession().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            FinishAssociated.belowSession().needReqUidsSet = nil
        }
    ...
    class func off(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (app_heartMsg.replacingOccurrences(of: "now", with: "ri") + "lQueue"))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    ...
                    AgainstListen.materialAbout(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
    ...
    public class func image(message: V2TIMMessage) -> PanMarker {
        //: let key = TUISocialChatManager.checkPrimaryKey(withMsgModel: message)
        let key = ChainSample.discoverUniform(withMsgModel: message)
        //: var cacheWrap = WCDBVoiceMsgTable.db_getVoiceMsg(with: key)
        var cacheWrap = PanMarker.clearWrite(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            //: let info = TUISocialChatManager.packageDict(withTXMessage: message)
            let info = ChainSample.tweenGreet(withTXMessage: message)
            //: cacheWrap = WCDBVoiceMsgTable.db_inserVoiceMsgToDB(info)
            cacheWrap = PanMarker.deployBottom(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension FinishAssociated {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> BetweenInput? {
    class func resumeQuery(targetId: String) -> BetweenInput? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return OutsideMode.chapterActivity(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: BetweenInput?) {
    class func radioImmediate(with userInfo: BetweenInput?) {
        ...
        let model = FinishAssociated.resumeQuery(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        OutsideMode.cosBelow(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: mainPathUrl, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func netGraphic(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = FinishAssociated.resumeQuery(targetId: targetId) else { return }
    ...
    class func innerWill(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = FinishAssociated.resumeQuery(targetId: targetId) else { return }
    ...
    class func nearTo(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        OutsideMode.scanByProgram(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        FinishAssociated.belowSession().reqedUidsSet.remove(targetUid)
    }
}
...
extension FinishAssociated {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([BetweenInput]?, Error?) -> Void) {
    private class func per(with userIdsArr: [String], completion: @escaping ([BetweenInput]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = OutsideMode.findUsed(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if FinishAssociated.belowSession().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            FinishAssociated.databaseSaveer(with: noDBUidArr)
        }
    }
    ...
    private class func databaseSaveer(with userIdsArr: [String]) {
        ...
        FinishAssociated.belowSession().reqedUidsSet = FinishAssociated.belowSession().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: TalkingChatRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        AgainstListen.cropAttach(param: [String(bytes: kPresentationData.map{$0^95}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                FinishAssociated.belowSession().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            FinishAssociated.slow(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: BetweenInput] = [:]
            var retInfoDict: [String: BetweenInput] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: mainPathUrl, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [BetweenInput]) {
    private class func slow(userInfoArr: [BetweenInput]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            OutsideMode.cosBelow(aWrap)
        }
    }
}
