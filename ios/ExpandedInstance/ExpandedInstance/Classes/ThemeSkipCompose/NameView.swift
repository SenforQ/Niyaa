...
/*: "#AAAAAA" :*/
fileprivate let userIgnoreMsg:String = "vid tar#AAAAA"
fileprivate let user_currencyFormat:String = "identify"

/*: " No more than 10 characters" :*/
fileprivate let show_lineMutterPath:[Character] = [" ","N","o"," ","m","o","r","e"," "]
fileprivate let noti_movieMessage:String = "illegala"
fileprivate let constShPath:[Character] = ["n"," ","1","0"," ","c","h","a","r","a","c","t","e","r","s"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let k_rootPurpleStr:[UInt8] = [0x98,0x9d,0x98,0xa3,0x57,0x92,0x9e,0x93,0x94,0xa1,0x69,0x58,0x4f,0x97,0x90,0xa2,0x4f,0x9d,0x9e,0xa3,0x4f,0x91,0x94,0x94,0x9d,0x4f,0x98,0x9c,0x9f,0x9b,0x94,0x9c,0x94,0x9d,0xa3,0x94,0x93]

fileprivate func albumEquipment(welcome num: UInt8) -> UInt8 {
...
class NameView: UIView {
    ...
    private var alertConfig: SignatureAssociate
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.font = UIFont.alongsideOk(type: .Semibold, fontSize: 20)
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    private lazy var inputTView: GenerateView = {
        ...
        input.placeholderColor = .init(hex: (String(userIgnoreMsg.suffix(6)) + user_currencyFormat.replacingOccurrences(of: "identify", with: "A"))) ?? UIColor.gray
        //: input.placeholder = " No more than 10 characters".localized
        input.placeholder = (String(show_lineMutterPath) + noti_movieMessage.replacingOccurrences(of: "illegal", with: "th") + String(constShPath)).localized
        ...
        input.backgroundColor = .boardImplement()
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.multiSince().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(reportCur), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        rightBtn.setBackgroundImage(UIImage.doEmbrace(colors: UIColor.monthForMomentum(), size: CGSize(width: 267, height: 50)), for: .normal)
        ...
        rightBtn.setBackgroundImage(UIImage.doEmbrace(colors: UIColor.independentInsideRemain(), size: CGSize(width: 267, height: 50)), for: .disabled)
        ...
        rightBtn.addTarget(self, action: #selector(minimizeBy), for: .touchUpInside)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: k_rootPurpleStr.map{albumEquipment(welcome: $0)}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func cellWithoutMountField(title: String, isTouch: Bool) {
        ...
        if !isTouch {
            ...
            rightBtn.setBackgroundImage(UIImage.transferAndJoinSmall(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.BundleImageNamed(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.transferAndJoinSmall(name: alertConfig.rightBackgroundImage).withTintColor(.multiSince())
...
extension NameView: UITextViewDelegate {
    //: func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
    func outCriticalPara(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
    ...
    func beautyPressureCoordinatorReceive(_ textField: UITextField) -> Bool {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > alertConfig.lengthLimit {
            ...
            if textNum > alertConfig.lengthLimit && alertConfig.lengthLimit > 0 {
                ...
                textView.text = res(index: alertConfig.lengthLimit, text: textContent)
            }
        }
    }

    //: private func string_prefix(index: Int, text: String) -> String {
    private func res(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
