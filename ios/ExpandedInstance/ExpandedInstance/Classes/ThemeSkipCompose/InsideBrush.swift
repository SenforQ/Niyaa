...
fileprivate let user_feedString:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Add more details of yourself" :*/
fileprivate let show_spotFractionId:[Character] = ["A","d","d"," ","m","o","r","e"," ","d","e","t","a","i","l","s"," ","o","f"," ","y","o","u","r","s","e"]
fileprivate let showFloorEffUrl:String = "rem"

/*: "#999999" :*/
fileprivate let notiMyCountervalId:[Character] = ["#","9","9","9","9","9"]
fileprivate let appRenderKey:String = "9"

/*: "0/500" :*/
fileprivate let notiHoneyUrl:String = "boy/5boyboy"

/*: "Tip:Don't post content that induces others to send gifts or other money-relatsed content." :*/
fileprivate let user_extensiveMsg:[UInt8] = [0x2e,0x74,0x6e,0x65,0x74,0x6e,0x6f,0x63,0x20,0x64,0x65,0x73,0x74,0x61,0x6c,0x65,0x72,0x2d,0x79,0x65,0x6e,0x6f,0x6d,0x20,0x72,0x65,0x68,0x74,0x6f,0x20,0x72,0x6f,0x20,0x73,0x74,0x66,0x69,0x67,0x20,0x64,0x6e,0x65,0x73,0x20,0x6f,0x74,0x20,0x73,0x72,0x65,0x68,0x74,0x6f,0x20,0x73,0x65,0x63,0x75,0x64,0x6e,0x69,0x20,0x74,0x61,0x68,0x74,0x20,0x74,0x6e,0x65,0x74,0x6e,0x6f,0x63,0x20,0x74,0x73,0x6f,0x70,0x20,0x74,0x27,0x6e,0x6f,0x44,0x3a,0x70,0x69,0x54]
...
class InsideBrush: UITableViewCell {
    //: let limitCount = 500
    let limitCount = 500
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(searcher(notification:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(tick(notification:)), name: UIResponder.keyboardWillShowNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder aDecoder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: user_feedString.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(show_spotFractionId) + showFloorEffUrl.replacingOccurrences(of: "rem", with: "lf")).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var numberLB: UILabel = {
        ...
        LB.textColor = UIColor(hex: (String(notiMyCountervalId) + appRenderKey.capitalized))
        //: LB.font = UIFont.pingfangFont(type: .Regular, fontSize: 14)
        LB.font = UIFont.alongsideOk(type: .Regular, fontSize: 14)
        //: LB.text = "0/500"
        LB.text = (notiHoneyUrl.replacingOccurrences(of: "boy", with: "0"))
    ...
    lazy var textView: SmallView = {
        ...
        //: let sign = QuantityeractionDelay.share.loginUserMode.signature
        let sign = QuantityeractionDelay.share.loginUserMode.signature
        //: textView.text = sign
        textView.text = sign
        //: textView.placeholder = "Tip:Don't post content that induces others to send gifts or other money-relatsed content.".localized
        textView.placeholder = String(bytes: user_extensiveMsg.reversed(), encoding: .utf8)!.localized
        //: textView.placeholderColor = UIColor.appValueDetailColor()
        textView.placeholderColor = UIColor.underCountensityLanguage()
...
extension InsideBrush {
    //: @objc func keyboardWillBeHidden(notification: NSNotification) {
    @objc func searcher(notification: NSNotification) {
    ...
    @objc func tick(notification: NSNotification) {
...
extension InsideBrush: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > limitCount {
            ...
            if textNum > limitCount && limitCount > 0 {
                ...
                textView.text = replace(index: limitCount, text: textContent)
            }
        }
        //: numberLB.text =  "\(textView.text.count)/\(limitCount)"
        numberLB.text = "\(textView.text.count)/\(limitCount)"
    ...
    private func replace(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
