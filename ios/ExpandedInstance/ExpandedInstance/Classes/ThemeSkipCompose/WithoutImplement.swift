...
class WithoutImplement: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: ToleranceLast?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.thePermanent()
            }
        }
    ...
    func outWritten() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.theMinimize(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.boardImplement()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.thinOf(), NSAttributedString.Key.font: UIFont.alongsideOk(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.theMinimize(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.thinOf(), NSAttributedString.Key.font: UIFont.alongsideOk(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.thinOf()
...
extension WithoutImplement: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            PromiseFinishSample.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
...
extension WithoutImplement: UIGestureRecognizerDelegate {
