...
struct SubsequentSkip: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct ProcessorMargin: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
struct FieldPer: HandyJSON {
    ...
    var status: ImageCreateer?
}

///
//: enum MomentVideoStatus: String {
enum ImageCreateer: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension SubsequentSkip {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func sourceByStage(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(MessageStage.sort(text: content!, size: CGSize(width: dataTabUrl - 54, height: CGFloat(MAXFLOAT)), font: UIFont.alongsideOk(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == HalfAdjust.Photo.rawValue || momentType == HalfAdjust.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
