...
class ReloadImplementation: UILabel {
    ...
    //: private func setup() {
    private func serverMore() {
        //: translatesAutoresizingMaskIntoConstraints = false
        translatesAutoresizingMaskIntoConstraints = false
        //: layer.backgroundColor = UIColor.msgTipsColor().cgColor
        layer.backgroundColor = UIColor.yearMiss().cgColor
        //: textColor = .white
        textColor = .white
        //: font = UIFont.pingfangFont(type: .Medium, fontSize: 11)
        font = UIFont.alongsideOk(type: .Medium, fontSize: 11)
...
class GrainPurple: UILabel {
    ...
    func subsequentAlong(isTop:Bool) {
        //: if isTop {
        if isTop {
