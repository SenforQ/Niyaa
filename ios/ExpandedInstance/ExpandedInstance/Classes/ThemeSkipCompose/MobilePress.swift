...
fileprivate let const_occurReaderValue:[UInt8] = [0xd6,0xd1,0xd6,0xcb,0x97,0xdc,0xd0,0xdb,0xda,0xcd,0x85,0x96,0x9f,0xd7,0xde,0xcc,0x9f,0xd1,0xd0,0xcb,0x9f,0xdd,0xda,0xda,0xd1,0x9f,0xd6,0xd2,0xcf,0xd3,0xda,0xd2,0xda,0xd1,0xcb,0xda,0xdb]
...
class MobilePress: UITableViewCell {
    //: lazy var menuImageView: UIImageView = self.makeMenuImageView()
    lazy var menuImageView: UIImageView = self.underProjection()
    //: lazy var menuTitleLabel: UILabel = self.makeMenuTitleLable()
    lazy var menuTitleLabel: UILabel = self.dataEarlyMonth()
    ...
    var dropMenuStyle: LockThroughName? {
        //: didSet {
        didSet {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: const_occurReaderValue.map{$0^191}, encoding: .utf8)!)
    }
    ...
    private func underProjection() -> UIImageView {
    ...
    private func dataEarlyMonth() -> UILabel {
    ...
    private func anPlus() {
        ...
        lineLayer.strokeColor = UIColor.robotBy(230, 230, 230, 1.0).cgColor
