...
fileprivate let dataOperateTeakString:[UInt8] = [0x2a,0x2d,0x2a,0x37,0x6b,0x20,0x2c,0x27,0x26,0x31,0x79,0x6a,0x63,0x2b,0x22,0x30,0x63,0x2d,0x2c,0x37,0x63,0x21,0x26,0x26,0x2d,0x63,0x2a,0x2e,0x33,0x2f,0x26,0x2e,0x26,0x2d,0x37,0x26,0x27]

private func readingCollect(tailed num: UInt8) -> UInt8 {
    return num ^ 67
}
...
enum ResultPhoneHardBrownStrengthBlock: Int {
    //: case TimeGap = 0
...
@objc protocol LengthSineSkip: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func behavior()

    //: @objc optional func endScroll()
    @objc optional func destroyFilter()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class ErrorServiceView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(purchaseLimited))
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: dataOperateTeakString.map{readingCollect(tailed: $0)}, encoding: .utf8)!)
    }
...
extension ErrorServiceView {
    ...
    func childLanguage() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(selected), userInfo: nil, repeats: true)
    }
...
extension ErrorServiceView {
    ...
    func sophisticatedByPull(msgModel: MethodDrawer?) {
    ...
    func successAgainst() {
        ...
        if AllHeight > showImageUrl {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < showImageUrl {
                ...
                self.msgTableView.height = showImageUrl
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = showImageUrl - AllHeight
        ...
        quantityEvaluation(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func quantityEvaluation(animated: Bool) {
    ...
    func monsterByBasic(inPending: Bool) {
...
extension ErrorServiceView: UITableViewDelegate, UITableViewDataSource {
    ...
    func smart(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.destroyFilter?()
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(appAdvancedKey)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = CouldContrast.toleranceProjection(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension ErrorServiceView {
    ...
    func create() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: appConsumeHandleId, height: showImageUrl)
    }
}
