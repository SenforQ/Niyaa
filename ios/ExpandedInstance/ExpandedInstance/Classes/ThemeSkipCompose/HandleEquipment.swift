...
fileprivate let userReflectPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
let constThinMicBoxData = 80.0
//: let kProgressHUD_cornerRadius = 14.0
let constClearPath = 14.0
//: let kProgressHUD_alpha        = 0.9
let main_associatedFormat = 0.9
//: let kBackgroundView_alpha     = 0.6
let mainInsteadRedString = 0.6
//: let kAnimationInterval        = 0.2
let kEnhanceMessage = 0.2
//: let kTransformScale           = 0.9
let mainFindMessage = 0.9

//: open class ProgressHUD: UIView {
open class HandleEquipment: UIView {
    //: required public init?(coder: NSCoder) {
    public required init?(coder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: userReflectPath.reversed(), encoding: .utf8)!)
    }

    //: static var shared = ProgressHUD()
    static var shared = HandleEquipment()
    ...
    class func faceSpot() {
        //: show(superView: nil)
        sinceFollow(superView: nil)
    }

    //: class func show(superView: UIView?) {
    class func sinceFollow(superView: UIView?) {
        //: if superView != nil {
        if superView != nil {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                //: ProgressHUD.shared.frame = superView!.bounds
                HandleEquipment.shared.frame = superView!.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                HandleEquipment.shared.activityIndicator.center = HandleEquipment.shared.center
                //: superView!.addSubview(ProgressHUD.shared)
                superView!.addSubview(HandleEquipment.shared)
            }
            //: } else {
        } else {
            //: DispatchQueue.main.async {
    ...
    private func purpleBy() {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: kEnhanceMessage) {
                ...
                HandleEquipment.shared.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.bounds = CGRect(x: 0, y: 0, width: constThinMicBoxData, height: constThinMicBoxData)
        //: indicator.center = self.center
        indicator.center = self.center
        //: indicator.backgroundColor = .black
        indicator.backgroundColor = .black
        //: indicator.layer.cornerRadius = kProgressHUD_cornerRadius
        indicator.layer.cornerRadius = constClearPath
...
extension HandleEquipment {
    //: class func toast(_ str: String?) {
    class func spreadWith(_ str: String?) {
        //: toast(str, showTime: 1)
        mutterEnable(str, showTime: 1)
    }

    //: class func toast(_ str: String?, showTime: CGFloat) {
    class func mutterEnable(_ str: String?, showTime: CGFloat) {
        ...
        VersusQuit.further().addSubview(titleLab)
        ...
        titleLab.center = VersusQuit.further().center
