...
/*: "RSA加密失败，str:  :*/
fileprivate let mainSizeValue:String = "RSA加\u{5bc6}失"

/*: "weixin://" :*/
fileprivate let dataGraphicData:[Character] = ["w","e"]
fileprivate let main_thirdMsg:String = "just on hiddenixin://"

/*: "dingtalk://" :*/
fileprivate let data_acceptUrl:String = "dinhealthyal"
fileprivate let const_rulingWarnValue:[Character] = ["k",":","/","/"]

/*: "lark://" :*/
fileprivate let k_anotherStr:[Character] = ["l","a","r","k",":","/","/"]

/*: "wxwork://" :*/
fileprivate let appDynamicsTitle:String = "specifywor"
fileprivate let dataBusinessTitle:[Character] = ["k",":","/","/"]

/*: "deviceId" :*/
fileprivate let app_hisData:[UInt8] = [0xca,0xcb,0xd8,0xc7,0xcd,0xcb,0xe7,0xca]

private func surePrivacy(reach num: UInt8) -> UInt8 {
    return num ^ 174
}

/*: "weixin" :*/
fileprivate let appHeMessage:[Character] = ["w","e","i","x","i","n"]

/*: "wxwork" :*/
fileprivate let constCalculationCertainString:[Character] = ["w","x","w","o","r","k"]

/*: "dingtalk" :*/
fileprivate let show_outputPostData:[Character] = ["d","i","n","g","t","a","l","k"]

/*: "lark" :*/
fileprivate let dataRedOuterOurValue:String = "lsherk"
...
extension UIDevice {
    ...
    class func queryedExpected(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.translationIn())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
    ...
    class func translationIn() -> String {
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func theTotal(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (mainSizeValue + "败\u{ff0c}str: ") + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (String(dataGraphicData) + String(main_thirdMsg.suffix(7)))
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (data_acceptUrl.replacingOccurrences(of: "healthy", with: "gt") + String(const_rulingWarnValue))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenFS: Bool {
        //: let appSchema = "lark://"
        let appSchema = (String(k_anotherStr))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (appDynamicsTitle.replacingOccurrences(of: "specify", with: "wx") + String(dataBusinessTitle))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var customDeviceNum: String {
        ...
        if let value = key.get(k_optionUrl) {
            //: return value
            return value
            //: }else{
        } else {
            ...
            key.set(value, forKey: k_optionUrl)
            //: return value
            return value
        }
    }
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = ImageClear.productImage() || ImageClear.originToDistinctive()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = theTotal(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: app_hisData.map{surePrivacy(reach: $0)}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((String(appHeMessage)))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((String(constCalculationCertainString)))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((String(show_outputPostData)))
        }
        //: if UIDevice.isOpenFS {
        if UIDevice.isOpenFS {
            //: appsArr.append("lark")
            appsArr.append((dataRedOuterOurValue.replacingOccurrences(of: "she", with: "a")))
        }
        //: if appsArr.count > 0 {
        if appsArr.count > 0 {
