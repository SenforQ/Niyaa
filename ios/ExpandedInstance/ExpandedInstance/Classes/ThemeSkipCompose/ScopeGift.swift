...
fileprivate let main_loopUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Reply" :*/
fileprivate let k_beforeMsg:String = "Replyaway arrangement"

/*: "#F95151" :*/
fileprivate let user_editValue:[Character] = ["#","F","9","5","1"]
fileprivate let notiMainMsg:String = "5accept"
...
let app_stackLicenseUrl = (dataDoingRapidMsg + 80.0)
//: class TalkingMessageNotificationView: UIView {
class ScopeGift: UIView {
    //: var startDismissBlock: ((_ model: TalkingConversationModel) -> Void)?
    var startDismissBlock: ((_ model: SpeedModel) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: ScopeGift) -> Void)? // 完成移除视图Block
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: main_loopUrl.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .alongsideOk(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .alongsideOk(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(k_beforeMsg.prefix(5))).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(user_editValue) + notiMainMsg.replacingOccurrences(of: "accept", with: "1"))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension ScopeGift {
    ...
    func smartDraft(_ model: SpeedModel?) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 5) {
                    ...
                    self.betweenToday(needAlpha: false)
                }
            }
        }
    }
    ...
    func betweenToday(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -app_stackLicenseUrl
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func host() {
        ...
        if currModel.chatType == .service {
            ...
            //: TellEnd.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            TellEnd.share.broadcastResign(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func will(gesture: UISwipeGestureRecognizer) {
...
extension ScopeGift {
    ...
    func withPad() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.image = UIImage.primaryEnable()
            //: iconBorder.isHidden = true
            iconBorder.isHidden = true
        }

        //: msgShowLabel.attributedText = TalkingChatListTableCell.handleShowC2CDetailTextOfMessage(model: currModel)
        msgShowLabel.attributedText = PointPet.systemTransfer(model: currModel)
    }
    ...
    private func date() {
        ...
        self.frame = CGRect(x: 0, y: -app_stackLicenseUrl, width: dataTabUrl, height: app_stackLicenseUrl)
