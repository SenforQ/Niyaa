...
fileprivate let notiEarlyGroupPath:[UInt8] = [0x20,0x73,0x69,0x20,0x73,0x75,0x74,0x61,0x74,0x53,0x6e,0x6f,0x69,0x74,0x61,0x7a,0x69,0x72,0x6f,0x68,0x74,0x75,0x61,0x20,0x6e,0x6f,0x69,0x74,0x61,0x63,0x6f,0x6c]

/*: "get location failed. error: :*/
fileprivate let show_commandTitle:String = "available large take man fromget l"
fileprivate let kLengthPath:String = "n faspeed way plus near"
fileprivate let noti_thanName:String = "sound net busy answer mutter. erro"
fileprivate let mainProvisionData:String = "r:leave recommend"
...
class PlateTellDelete: NSObject {
    ...
    var locationType: DestroyPauseBorder?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: PlateTellDelete?
    //: class func shared() -> TalkingLocationManager {
    class func direction() -> PlateTellDelete {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = PlateTellDelete()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func broadFlow(block: @escaping Locationblock) {
    ...
    func customEnable(block: @escaping Authblock) {
    ...
    func barDimension() -> DestroyPauseBorder {
    ...
    func subtleIgnore() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: notiEarlyGroupPath.reversed(), encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension PlateTellDelete: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(show_commandTitle.suffix(5)) + "ocatio" + String(kLengthPath.prefix(4)) + "iled" + String(noti_thanName.suffix(6)) + String(mainProvisionData.prefix(2))) + "\(error.localizedDescription)")
    }
}
