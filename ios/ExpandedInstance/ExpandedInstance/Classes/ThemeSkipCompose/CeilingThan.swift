...
fileprivate let const_drawerData:String = "TSysred mouse recto eh"
fileprivate let data_regularProFormat:String = "earn boy become shot tailsageC"
fileprivate let notiBlendMsg:[Character] = ["u","s","e","I","d"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let dataDefineTitle:[UInt8] = [0x4b,0x50,0x4b,0x56,0xa,0x45,0x51,0x46,0x47,0x54,0x1c,0xb,0x2,0x4a,0x43,0x55,0x2,0x50,0x51,0x56,0x2,0x44,0x47,0x47,0x50,0x2,0x4b,0x4f,0x52,0x4e,0x47,0x4f,0x47,0x50,0x56,0x47,0x46]

fileprivate func notAcross(appearance num: UInt8) -> UInt8 {
...
/*: "icon_talk_notic" :*/
fileprivate let appElementAppearId:String = "icon_identify nobody tool"
fileprivate let showReparationOptionMsg:[Character] = ["t","a","l","k","_","n","o","t","i","c"]
...
@objc public protocol StorageTween: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: CeilingThan)
    func startPick(cell: CeilingThan)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: CeilingThan)
    func priceThumb(cell: CeilingThan)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: CeilingThan)
    func spendConstant(cell: CeilingThan)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: CeilingThan)
    func efficiencyStep(cell: CeilingThan)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: CeilingThan)
    func dayFound(cell: CeilingThan)
}

//: @objcMembers public class CeilingThan: TCommonTableViewCell {
@objcMembers public class CeilingThan: InsertSection {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\FlexBridge.h
     */
    //: public var messageData: FlexBridge?
    public var messageData: FlexBridge?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(const_drawerData.prefix(4)) + "temMes" + String(data_regularProFormat.suffix(5)) + "ell_Re" + String(notiBlendMsg))

    /**
     *  协议委托
     *  负责实现 StorageTween 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: StorageTween?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: dataDefineTitle.map{notAcross(appearance: $0)}, encoding: .utf8)!)
    }
    ...
    override public func panel(with data: CleanShared) {
        //: super.fill(with: data)
        super.panel(with: data)
        //: let newData: FlexBridge = data as! FlexBridge
        let newData: FlexBridge = data as! FlexBridge
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.that()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(impressionMemory(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.wantStream(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(underDoing(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(noticeLab(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.transferAndJoinSmall(name: (String(appElementAppearId.prefix(5)) + String(showReparationOptionMsg)))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(ovalInStyle(recognizer:)))
...
//: extension CeilingThan {
extension CeilingThan {
    //: @objc func onSelectMessageAvatar(recognizer: UIGestureRecognizer) {
    @objc func impressionMemory(recognizer: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.efficiencyStep(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func noticeLab(recognizer: UIGestureRecognizer) {
