...
fileprivate let dataAdjustmentPlainMessage:String = "icon_hidden already do nous city"
fileprivate let data_ignoreId:String = "_kongpurchase local click err"
fileprivate let main_rowMessage:String = "mergeult"

/*: "page" :*/
fileprivate let appPartString:[UInt8] = [0x6b,0x7a,0x7c,0x7e]

private func blueAutomaticShape(respective num: UInt8) -> UInt8 {
    return num ^ 27
}

/*: "uid" :*/
fileprivate let dataDisplayStageCookieMessage:[UInt8] = [0x8,0x14,0x19]

private func numbDesign(plate num: UInt8) -> UInt8 {
    return num ^ 125
}

/*: "list" :*/
fileprivate let kHoneyMessage:[Character] = ["l","i","s","t"]
...
class LineFlow: ScaleViewController {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [ImageTail]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: k_postMsg, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(CountoEquipment.self, forCellWithReuseIdentifier: CountoEquipment.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.transferAndJoinSmall(name: (String(dataAdjustmentPlainMessage.prefix(5)) + "kong" + String(data_ignoreId.prefix(5)) + "_def" + main_rowMessage.replacingOccurrences(of: "merge", with: "a")))
...
extension LineFlow {
    ...
    private func nearAssociated() {
        //: let params = ["page": pageIndex, "uid": userInfoModel.uid] as [String: Any]
        let params = [String(bytes: appPartString.map{blueAutomaticShape(respective: $0)}, encoding: .utf8)!: pageIndex, String(bytes: dataDisplayStageCookieMessage.map{numbDesign(plate: $0)}, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        TellCharacter.basicEarly(params: params) { [weak self] succeed, result, errorModel in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(String(kHoneyMessage))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = SubsequentSkip.deserialize(from: dict as? Dictionary) {
...
extension LineFlow: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CountoEquipment.className(), for: indexPath) as! CountoEquipment
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = BulletCycleViewController(model: detailModel)
...
extension LineFlow: JXPagingViewListViewDelegate {
...
extension LineFlow {
    ...
    func reportDrawer(_ userModel: PutPath) {
