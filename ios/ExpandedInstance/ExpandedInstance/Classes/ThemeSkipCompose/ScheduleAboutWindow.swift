...
/*: "position.x" :*/
fileprivate let noti_helloData:String = "POSI"
fileprivate let const_commandPath:[Character] = ["t","i","o","n",".","x"]

/*: "slideIn" :*/
fileprivate let const_unlessTitle:String = "slideInactor launch square rate seek"

/*: "slideOut" :*/
fileprivate let main_exMessage:String = "remain"
fileprivate let userBetweenPath:[Character] = ["l","i","d","e","O","u","t"]
...
class ScheduleAboutWindow: NSObject {
    //: static let shared = GiftFloatViewManager()
    static let shared = ScheduleAboutWindow()
    //: private var giftArr = [SocialBroadcastModel]()
    private var giftArr = [WithoutResume]()
    ...
    private lazy var giftView: BottomView = {
        //: let v = GiftFloatView()
        let v = BottomView()
        //: MacroDefine.getWindow().addSubview(v)
        VersusQuit.further().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension ScheduleAboutWindow {
    /// 添加礼物
    //: private func addGift(gift: SocialBroadcastModel) {
    private func originalFragment(gift: WithoutResume) {
        //: self.synchronized(self) {
        self.todayByRequest(self) {
            //: self.giftArr.append(gift)
            self.giftArr.append(gift)
            //: self.showNextGift(index: 0)
            self.aroundAppearance(index: 0)
        }
    }

    //: public func synchronized<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    public func todayByRequest<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    ...
    private func outsideMarkPick(_ model: WithoutResume) {
        ...
        let isRTL = CoordinatorExtra.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : dataTabUrl + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = dataTabUrl / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? dataTabUrl + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (noti_helloData.lowercased() + String(const_commandPath)))
        ...
        self.giftView.layer.add(slideIn, forKey: (String(const_unlessTitle.prefix(7))))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (noti_helloData.lowercased() + String(const_commandPath)))
            ...
            self.giftView.layer.add(slideOut, forKey: (main_exMessage.replacingOccurrences(of: "remain", with: "s") + String(userBetweenPath)))
...
extension ScheduleAboutWindow {
    ...
    func found() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.sumensityAlong(noti:)),
                                               //: name: USER_RECIEVED_GIFT_NOTIFICATION,
                                               name: showLibraryString,
                                               //: object: nil)
                                               object: nil)
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.mergeCap),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.skirtJack),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.exitStage),
    ...
    @objc private func sumensityAlong(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<WithoutResume>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != HeapEvolution.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        }
        //: let currUserType = QuantityeractionDelay.share.appUserConfigMode.userType
        let currUserType = QuantityeractionDelay.share.appUserConfigMode.userType
        // 过滤接口异常
        ...
        // 非特殊用户 不展示 不同地区用户礼物
        //: if currUserType != 0, gift.userCountryType != QuantityeractionDelay.share.appUserConfigMode.userCountryType {
        if currUserType != 0, gift.userCountryType != QuantityeractionDelay.share.appUserConfigMode.userCountryType {
            //: return
            return
        }
        ...
        self.originalFragment(gift: gift)
    }

    /// 应用退到后台
    //: @objc private func enterBackground() {
    @objc private func mergeCap() {
        //: self.stopWhenDismiss()
        self.indicatorCustom()
    }

    /// 应用进入前台
    //: @objc private func enterForeground() {
    @objc private func skirtJack() {
        //: self.activeWhenShow()
        self.combineAccount()
    }
