...
fileprivate let constThemeRoutePath:[UInt8] = [0x14,0x13,0x14,0x9,0x55,0x1e,0x12,0x19,0x18,0xf,0x47,0x54,0x5d,0x15,0x1c,0xe,0x5d,0x13,0x12,0x9,0x5d,0x1f,0x18,0x18,0x13,0x5d,0x14,0x10,0xd,0x11,0x18,0x10,0x18,0x13,0x9,0x18,0x19]

private func nothingAdvertising(loan num: UInt8) -> UInt8 {
    return num ^ 125
}

/*: "icon_chat_lock" :*/
fileprivate let user_threadPath:[Character] = ["i","c","o","n","_","c","h","a","t","_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let noti_equalShouldValue:String = "icalongn"
fileprivate let data_contextFatalTitle:String = "urncalculationd"

/*: "icon_chat_burned_shan" :*/
fileprivate let show_pictureUrl:String = "icon_chabig architecture"
fileprivate let constImplementationMessage:String = "total mutual frame print revenuet_bu"
fileprivate let const_flexibleContextMsg:String = "rned_ex please"

/*: "Already burned" :*/
fileprivate let userBeyondUrl:[Character] = ["A","l","r","e","a","d","y"," ","b","u","r"]
fileprivate let dataUnableValue:String = "nthorough"
...
class FactoryEven: LocateMight {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: constThemeRoutePath.map{nothingAdvertising(loan: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.transferAndJoinSmall(name: (String(user_threadPath)))
    ...
    lazy var burnedImagV: ArrayButton = {
        ...
        imgV.setBackgroundImage(UIImage.transferAndJoinSmall(name: (noti_equalShouldValue.replacingOccurrences(of: "along", with: "o") + "_chat_b" + data_contextFatalTitle.replacingOccurrences(of: "calculation", with: "e"))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.transferAndJoinSmall(name: (String(show_pictureUrl.prefix(8)) + String(constImplementationMessage.suffix(4)) + String(const_flexibleContextMsg.prefix(5)) + "shan")), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(userBeyondUrl) + dataUnableValue.replacingOccurrences(of: "thorough", with: "ed")).localized, for: .normal)
        //: imgV.setTitleColor(.white, for: .normal)
        imgV.setTitleColor(.white, for: .normal)
        //: imgV.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 16)
        imgV.titleLabel?.font = UIFont.botLocal(fontSize: 16)
        //: return imgV
        return imgV
        //: }()
    }()
...
extension FactoryEven {
    //: override func fill(with data: TCommonCellData) {
    override func panel(with data: CleanShared) {
        //: super.fill(with: data)
        super.panel(with: data)
        //: guard let imageCellData = data as? AppearanceAddLock else { return }
        guard let imageCellData = data as? AppearanceAddLock else { return }
        ...
        let intimatePhoto: PlateUniformModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            scriptChart(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            scriptChart(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: AppearanceAddLock, urlStr: String?, isMosaic: Bool) {
    func scriptChart(data: AppearanceAddLock, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.size(image, occasion: 90)
                        self.photoView.image = UIImage.size(image, occasion: 90)
                        //: } else {
                    } else {
            ...
            factoryInsideEase(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.okUponLock(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func factoryInsideEase(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.thinView(urlStr: urlStr) { [weak self] image in
