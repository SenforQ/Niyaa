...
fileprivate let appPetValue:[UInt8] = [0xa,0xd,0xa,0x17,0x4b,0x0,0xc,0x7,0x6,0x11,0x59,0x4a,0x43,0xb,0x2,0x10,0x43,0xd,0xc,0x17,0x43,0x1,0x6,0x6,0xd,0x43,0xa,0xe,0x13,0xf,0x6,0xe,0x6,0xd,0x17,0x6,0x7]

/*: "#999999" :*/
fileprivate let data_projectStr:String = "#999999"

/*: "0/20" :*/
fileprivate let userItsName:String = "0/20"

/*: "Nickname" :*/
fileprivate let app_outputId:String = "Nicknameput immediate"

/*: "enter your name" :*/
fileprivate let constYetKey:String = "subntsub"
fileprivate let main_easyMessage:[Character] = ["r"," ","n","a","m","e"]
...
class IndexObserve: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(alive(notification:)), name: UIResponder.keyboardWillHideNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder aDecoder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: appPetValue.map{$0^99}, encoding: .utf8)!)
    }
    ...
    lazy var numberLB: UILabel = {
        ...
        LB.textColor = UIColor(hex: (data_projectStr.capitalized))
        //: LB.font = UIFont.pingfangFont(type: .Regular, fontSize: 14)
        LB.font = UIFont.alongsideOk(type: .Regular, fontSize: 14)
        //: LB.text = "0/20"
        LB.text = (userItsName.capitalized)
    ...
    lazy var titleLB: UILabel = {
        ...
        LB.text = (String(app_outputId.prefix(8))).localized
    ...
    lazy var nameTF: UITextField = {
        ...
        tf.font = UIFont.alongsideOk(type: .Medium, fontSize: 16)
        //: tf.textAlignment = .center
        tf.textAlignment = .center
        //: tf.attributedPlaceholder = NSAttributedString.init(string: "enter your name".localized, attributes: [
        tf.attributedPlaceholder = NSAttributedString(string: (constYetKey.replacingOccurrences(of: "sub", with: "e") + "r you" + String(main_easyMessage)).localized, attributes: [
            //: NSAttributedString.Key.foregroundColor: UIColor.init(hex: "#999999")!])
            NSAttributedString.Key.foregroundColor: UIColor(hex: (data_projectStr.capitalized))!])
        //: tf.delegate = self
        tf.delegate = self
        //: tf.returnKeyType = UIReturnKeyType.done
        tf.returnKeyType = UIReturnKeyType.done
        //: tf.text = QuantityeractionDelay.share.loginUserMode.nickname
        tf.text = QuantityeractionDelay.share.loginUserMode.nickname
        //: numberLB.text = "\(QuantityeractionDelay.share.loginUserMode.nickname?.count ?? 0)/20"
        numberLB.text = "\(QuantityeractionDelay.share.loginUserMode.nickname?.count ?? 0)/20"
        //: return tf
        return tf
        //: }()
...
extension IndexObserve {
    //: @objc func keyboardWillBeHidden(notification: NSNotification) {
    @objc func alive(notification: NSNotification) {
...
extension IndexObserve: UITextFieldDelegate {
