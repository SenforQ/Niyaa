...
fileprivate let show_rawInsertName:[UInt8] = [0x49,0x4e,0x49,0x54,0x8,0x43,0x4f,0x44,0x45,0x52,0x1a,0x9,0x0,0x48,0x41,0x53,0x0,0x4e,0x4f,0x54,0x0,0x42,0x45,0x45,0x4e,0x0,0x49,0x4d,0x50,0x4c,0x45,0x4d,0x45,0x4e,0x54,0x45,0x44]

private func compareUpper(radio num: UInt8) -> UInt8 {
    return num ^ 32
}

/*: "Not yet~" :*/
fileprivate let app_presentTitle:[Character] = ["N","o","t"," ","y","e","t"]
fileprivate let main_masterPath:[Character] = ["~"]

/*: "#8C7AFF" :*/
fileprivate let show_errorString:String = "arc explicit response#8C7A"
fileprivate let main_connectionString:String = "objectobject"
...
class SuccessBackgroundLimit: UIView {
    //: var type = 0
    var type = 0
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: show_rawInsertName.map{compareUpper(radio: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(CoordinatorSpell.self, forCellReuseIdentifier: CoordinatorSpell.className())
    ...
    lazy var tableHeaderView = FieldView(frame: CGRect(x: 0, y: 0, width: dataTabUrl, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = StartTitle().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(app_presentTitle) + String(main_masterPath)).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(show_errorString.suffix(5)) + main_connectionString.replacingOccurrences(of: "object", with: "F")))
...
extension SuccessBackgroundLimit: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: CoordinatorSpell.className(), for: indexPath) as! CoordinatorSpell
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: ProfileView.className(), for: indexPath) as! ProfileView
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.weeklyPrimary(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.fluid(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: CoordinatorSpell.className()) as! CoordinatorSpell
...
extension SuccessBackgroundLimit: JXSegmentedListContainerViewListDelegate {
...
extension SuccessBackgroundLimit {
    //: private func setupSubviews() {
    private func totalegrityFor() {
