...
/*: "&sr=6" :*/
fileprivate let dataCellKey:[Character] = ["&","s","r","=","6"]
...
enum GlobalOccasion: String, CaseIterable {
...
class OnCalculate: NSObject {
    //: class func getFullUrl(urlType: WebUrlType)-> String {
    class func occurThird(urlType: PlatformVolume) -> String {
        //: var url = QuantityeractionDelay.share.appConfigMode.urlH5Domain
        var url = QuantityeractionDelay.share.appConfigMode.urlH5Domain
        //: switch urlType {
        switch urlType {
        //: case .RechargeFullPage:
        case .RechargeFullPage:
            //: url.append(QuantityeractionDelay.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : QuantityeractionDelay.share.appUserConfigMode.fullRechargePage)
            url.append(QuantityeractionDelay.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : QuantityeractionDelay.share.appUserConfigMode.fullRechargePage)

        //: break
        //: case .RechargeHalfPage:
        case .RechargeHalfPage:
            //: url.append(QuantityeractionDelay.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : QuantityeractionDelay.share.appUserConfigMode.halfRechargePage)
            url.append(QuantityeractionDelay.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : QuantityeractionDelay.share.appUserConfigMode.halfRechargePage)

        //: case .RechargeToVideoInitivHalfPage:
        case .RechargeToVideoInitivHalfPage:
            //: let newstr = QuantityeractionDelay.share.appUserConfigMode.halfRechargePage.appending("&sr=6")
            let newstr = QuantityeractionDelay.share.appUserConfigMode.halfRechargePage.appending((String(dataCellKey)))
            //: url.append(QuantityeractionDelay.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
            url.append(QuantityeractionDelay.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
        //: default:
        default:
