...
fileprivate let noti_healthyStr:String = "设\u{7f6e}普通"
fileprivate let app_reduceLevelKey:[Character] = ["颜","色","时"," ","请","使","用","R","G","B","空","间","\u{7684}","颜","\u{8272}","值"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let show_appearId:String = "设置选中\u{72b6}"
fileprivate let noti_eachMsg:[Character] = ["态","的","\u{6587}","字","颜","色","时"," ","请","\u{4f7f}"]
fileprivate let noti_weekMessage:String = "\u{7528}RGB\u{7a7a}间"

/*: "init(coder:) has not been implemented" :*/
fileprivate let k_beforeKitStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "设置的下标不合法!!" :*/
fileprivate let k_hundredItsKey:[Character] = ["设","置","的","\u{4e0b}","标","不"]
fileprivate let appBuildUrl:String = "合法!!"

/*: "Bags" :*/
fileprivate let app_cancelString:String = "nature lockBags"
...
open class EquivalentComponent: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: GreenAdvanced
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.result(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((noti_healthyStr + "状\u{6001}的\u{6587}字" + String(app_reduceLevelKey)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.result(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((show_appearId + String(noti_eachMsg) + noti_weekMessage + "的颜色值"))
        }
    ...
    fileprivate func result(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: GreenAdvanced, titles: [String]) {
    ...
    @objc func vidOnPressure(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? MarginLabel else { return }
    ...
    public required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: k_beforeKitStr.reversed(), encoding: .utf8)!)
    }
}
...
public extension EquivalentComponent {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func beforeTar(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (String(k_hundredItsKey) + appBuildUrl.capitalized))
...
//: extension ScrollSegmentView {
extension EquivalentComponent {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func except() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = MarginLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.vidOnPressure(_:)))
            ...
            if title == (String(app_cancelString.suffix(4))).localized {
    ...
    fileprivate func likeIndicator() {
        // 设置Label位置
    ...
    fileprivate func reserve() {
        ...
        if let firstLabel = labelsArray[0] as? MarginLabel {
    ...
    public func manage(_ currentIndex: Int) {
