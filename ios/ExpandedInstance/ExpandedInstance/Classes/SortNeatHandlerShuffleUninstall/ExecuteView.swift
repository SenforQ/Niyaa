...
fileprivate let show_buttSpecifyTitle:[UInt8] = [0xd8,0xdd,0xd8,0xe3,0x97,0xd2,0xde,0xd3,0xd4,0xe1,0xa9,0x98,0x8f,0xd7,0xd0,0xe2,0x8f,0xdd,0xde,0xe3,0x8f,0xd1,0xd4,0xd4,0xdd,0x8f,0xd8,0xdc,0xdf,0xdb,0xd4,0xdc,0xd4,0xdd,0xe3,0xd4,0xd3]

fileprivate func goingTension(fragment num: UInt8) -> UInt8 {
...
/*: "icon_chat_miss" :*/
fileprivate let const_canTailTitle:[Character] = ["i","c","o","n","_"]
fileprivate let show_mustData:[Character] = ["c","h","a","t"]
fileprivate let show_domainOperatePath:String = "end early stop job_miss"

/*: "icon_chat_yes" :*/
fileprivate let user_insteadUrl:[Character] = ["i","c","o","n","_","c","h"]
fileprivate let show_verticalString:[Character] = ["a","t","_","y","e","s"]
...
class ExecuteView: LocateMight {
//    var transBlock: (() -> Void)?

    //: var textData: OwnerChart?
    var textData: OwnerChart?
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: show_buttSpecifyTitle.map{goingTension(fragment: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        label.font = UIFont.botLocal(fontSize: 16)
        //: return label
        return label
        //: }()
    }()
...
extension ExecuteView {
    ...
    override func panel(with data: CleanShared) {
        ...
        //: if let temTextData = data as? OwnerChart {
        if let temTextData = data as? OwnerChart {
            ...
            if imgPreview.count > 0, temTextData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
                ...
                giftImagV.okUponLock(urlStr: imgPreview)
            }
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != VersusQuit.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.corner() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.transferAndJoinSmall(name: (String(const_canTailTitle) + String(show_mustData) + String(show_domainOperatePath.suffix(5))))
                ...
                self.transStatusImagV.image = UIImage.transferAndJoinSmall(name: (String(user_insteadUrl) + String(show_verticalString)))
            }
    ...
    func me() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(containerAdd(tap:)))
        ...
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == VersusQuit.noneConsume())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            finishDismissTechnique()
...
//            }
    ...
    @objc func crucialFor(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func exceptSpot() {
        //: self.delegate?.onReplyMessage(cell: self)
        self.delegate?.dayFound(cell: self)
    }
}
