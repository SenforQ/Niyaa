...
protocol CurrentChart: AnyObject {
...
class ExclusiveViewController: UIViewController {
    ...
    weak var delegate: CurrentChart?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        var height = VersusQuit.holderProcessor() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= user_chainFormat
        }
        ...
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: VersusQuit.lawmakerExtensive(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(MacroDefine.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(VersusQuit.lawmakerExtensive()), height: 0)
        ...
        scrollView.semanticContentAttribute = CoordinatorExtra.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        ...
        var rect = CGRect(x: 15, y: topDistance, width: VersusQuit.lawmakerExtensive()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: MacroDefine.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: VersusQuit.lawmakerExtensive()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = CoordinatorExtra.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(beautyBy), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = MacroDefine.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = VersusQuit.lawmakerExtensive() / Double(self.titleItems.count)
            //: } else {
            } else {
            ...
            let x = CoordinatorExtra.shared.direction == .leftToRight ? originX:(VersusQuit.lawmakerExtensive()-originX-titleWidth)
...
extension ExclusiveViewController {
    ...
    private func accessAdjustment(withIndex index: Int) {
        ...
        let origin_x = CGFloat(index) * VersusQuit.lawmakerExtensive()
        ...
        vc.view.frame = CGRect(x: origin_x, y: 0, width: VersusQuit.lawmakerExtensive(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func voiceWithout(_ button: UIButton) {
    ...
    func questionEmbrace(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= MacroDefine.getScreenWidth() {
        if titleScrollView.contentSize.width <= VersusQuit.lawmakerExtensive() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - MacroDefine.getScreenWidth() * 0.5
        var offset_x = sender.center.x - VersusQuit.lawmakerExtensive() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        ...
        let maxOffset = titleScrollView.contentSize.width - VersusQuit.lawmakerExtensive()
...
extension ExclusiveViewController: UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let index = Int(scrollView.contentOffset.x / VersusQuit.lawmakerExtensive())
        ...
        let scaleR = scrollView.contentOffset.x / VersusQuit.lawmakerExtensive() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
