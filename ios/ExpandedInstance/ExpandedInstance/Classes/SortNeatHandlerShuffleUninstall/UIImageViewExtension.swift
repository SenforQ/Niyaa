...
/*: "add_header_cookies" :*/
fileprivate let mainPairBeyondMsg:String = "attach border scope spanadd_"
fileprivate let mainBeforePath:String = "r_cookiesword provision sibling after"
...
public extension UIImageView {
    ...
    @objc func okUponLock(urlStr: String,
    ...
    internal func viaCluster(urlStr: String,
                             //: placeImg: UIImage = UIImage.placeImgSquare(),
                             placeImg: UIImage = UIImage.embraceByBold(),
    ...
    @objc func sinceFind(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: showGroupFlexibleFlowName.occur(), with: show_whiteTitle)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.beyondGap())
    }
    ...
    @objc func thinView(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: ImageClear.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(mainPairBeyondMsg.suffix(4)) + "heade" + String(mainBeforePath.prefix(9))))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.embraceByBold(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
