...
fileprivate let const_writtenAttachStr:[UInt8] = [0x9b,0x9c,0x9b,0x86,0xda,0x91,0x9d,0x96,0x97,0x80,0xc8,0xdb,0xd2,0x9a,0x93,0x81,0xd2,0x9c,0x9d,0x86,0xd2,0x90,0x97,0x97,0x9c,0xd2,0x9b,0x9f,0x82,0x9e,0x97,0x9f,0x97,0x9c,0x86,0x97,0x96]

private func exerciseCritical(commit num: UInt8) -> UInt8 {
    return num ^ 242
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let main_judgeConvertUrl:String = "btnJQmode target communicate product"
fileprivate let dataPhoneTitle:String = "pi actual_eti"
fileprivate let data_linkMessage:String = "cab_shour marker inmate"
fileprivate let const_midChannelId:String = "ADOW"

/*: "Edit" :*/
fileprivate let appClearId:String = "Editrecording implementation reparation past answer"

/*: "Finish" :*/
fileprivate let noti_taCustomerFormat:String = "Finishtier content"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let main_choiceMsg:[Character] = ["b","t","n","Y","D"]
fileprivate let noti_processorUrl:String = "secure clean super brick destroyo2AS_"
fileprivate let dataLoopAcrossMessage:String = "stop like_play"

/*: "Cancel" :*/
fileprivate let const_clubString:String = "Cancelgray regulate priority out"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let noti_designFormat:[Character] = ["b","g","Q","k","R","T","b","F","_","o","e","d","i","v","_","t","o","a","s"]
fileprivate let app_blankFunKey:String = "uniform"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let constTaStatusStr:[UInt8] = [0x1f,0x23,0x2e,0x6b,0x3d,0x22,0x2f,0x2e,0x24,0x6b,0x27,0x2e,0x25,0x2c,0x3f,0x23,0x6b,0x38,0x23,0x24,0x3e,0x27,0x2f,0x6b,0x29,0x2e,0x6b,0x6e,0xb,0x66,0x6e,0xb,0x6b,0x38,0x2e,0x28,0x24,0x25,0x2f,0x38,0x65,0x6b,0x1b,0x27,0x2e,0x2a,0x38,0x2e,0x6b,0x22,0x25,0x3f,0x2e,0x39,0x28,0x2e,0x3b,0x3f,0x6b,0x22,0x3f]
...
typealias VideoEditBlock = (_ videoInfo: RouteFactory?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let kOntoOutputString: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class PopViewController: ScaleViewController {
    ...
    private var slider: MarkerView?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = RouteFactory() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = ResultSceneEvolution.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.agileImageSkewy == true {
        if UpZone.shared.agileImageSkewy == true { // 正在音视频通话，静音播放
    ...
    init(style: ResultSceneEvolution = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (dataTabUrl - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: const_writtenAttachStr.map{exerciseCritical(commit: $0)}, encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = .black
        //: self.settingPlayer()
        self.canvasAndHomeMagnitudeerested()
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.transferAndJoinSmall(name: (String(main_judgeConvertUrl.prefix(5)) + "bEAB" + String(dataPhoneTitle.suffix(4)) + "hw_k" + String(data_linkMessage.prefix(6)) + const_midChannelId.lowercased())), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = CoordinatorExtra.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(beyondFilter), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(appClearId.prefix(4))).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.multiSince(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.botLocal(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = CoordinatorExtra.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(noti_taCustomerFormat.prefix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.wantStream(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.multiSince()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(aboveBackground), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.transferAndJoinSmall(name: (String(main_choiceMsg) + String(noti_processorUrl.suffix(5)) + "oediv" + String(dataLoopAcrossMessage.suffix(5)))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(noDespite(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(const_clubString.prefix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(failEntity), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.transferAndJoinSmall(name: (String(noti_designFormat) + app_blankFunKey.replacingOccurrences(of: "uniform", with: "t"))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: constTaStatusStr.map{$0^75}, encoding: .utf8)!.underConnection(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: LayerFormatView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = LayerFormatView(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension PopViewController {
    ...
    private func overPad(animated: Bool = true) {
    ...
    @objc func bagPinion() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: dataTabUrl - 2 * 26, height: const_rebuildWithinId - 46 - 127 - constCountervalMaterialMsg)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = dataTabUrl / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: const_rebuildWithinId, width: dataTabUrl, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = MarkerView(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: kOntoOutputString)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - constCountervalMaterialMsg)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: EnvironmentMark) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func aboveBackground() {
        ...
        if self.videoEditHandler != nil {
            ...
            BeyondState.shared.resumeBy(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.overPad()
    ...
    @objc func noDespite(sender: UIButton) {
...
extension PopViewController {
    ...
    @objc private func constantSumensity() {
        //: self.player?.isMuted = true
        self.player?.isMuted = true
    }
}
...
extension PopViewController {
    ...
    private func canvasAndHomeMagnitudeerested() {
        //: guard self.videoURL != nil else { return }
        guard self.videoURL != nil else { return }
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(ovalCount), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.duringProcessor(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func anOutput() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + constCountervalMaterialMsg)
        }
    ...
    private func visibleDilemma() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(completeNor),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(constantSumensity),
