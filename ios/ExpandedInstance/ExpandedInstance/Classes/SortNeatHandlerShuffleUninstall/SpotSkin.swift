...
/*: "call_bgm" :*/
fileprivate let k_defineEfficiencyFormat:[Character] = ["c","a","l","l","_","b","g","m"]

/*: "call_response_bgm" :*/
fileprivate let app_calledMsg:[Character] = ["c","a","l","l","_","r","e","s","p","o","n","s","e","_","b","g","m"]

/*: "invitePop_call_bgm" :*/
fileprivate let appWithoutUrl:[Character] = ["i","n","v","i","t","e","P","o","p","_","c","a"]
fileprivate let noti_secondResumeString:String = "ll_bgmattribute period"
...
class SpotSkin: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = HandleDivide.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func paint(type: HandleDivide) -> SpotSkin {
...
extension SpotSkin {
    ...
    private func viewPriority(notify: Notification) {
    ...
    private func attributeMy(notify: Notification) {
    ...
    private func libraryPrior() -> Bool {
    ...
    private func joinAir() {
        //: var fileName = "call_bgm"
        var fileName = (String(k_defineEfficiencyFormat))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(app_calledMsg))
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (String(appWithoutUrl) + String(noti_secondResumeString.prefix(6)))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = PrepareSpecify.default.sh(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension SpotSkin: TXVodPlayListener {
