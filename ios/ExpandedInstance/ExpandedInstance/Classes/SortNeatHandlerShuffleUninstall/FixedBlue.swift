...
/*: "uid" :*/
fileprivate let main_monsterStr:[Character] = ["u","i","d"]

/*: "onlineStatus" :*/
fileprivate let showComputeTitle:String = "onsiblingin"
fileprivate let const_reparationQuantityervalTitle:String = "starts"

/*: "isNewUser" :*/
fileprivate let kRegulateCouldFormat:String = "isNabsolute priority"
fileprivate let k_hideEqualFormat:String = "ewUsernext field"

/*: "isTPAuth" :*/
fileprivate let const_sparkFormat:String = "mask impact mechanism distanceisTP"

/*: "totalIntimate" :*/
fileprivate let mainStructurePath:[Character] = ["t","o","t","a","l","I","n","t","i","m","a"]
fileprivate let user_renderFormat:String = "tglobal"

/*: "userStatus" :*/
fileprivate let data_sampleTitle:String = "userSpaint suspend about running"
fileprivate let app_locationPath:[Character] = ["t","a","t","u","s"]

/*: "intimate" :*/
fileprivate let main_searchTitle:String = "icomponentti"
...
enum AppearanceImpactDoingto: Int {
    //: case offical  = 0
    case offical = 0 // 官方
    //: case chatRoom = 1
    case chatRoom = 1 // 聊天室
...
class FixedBlue {
    ...
    private var moreMsgModel: SpeedModel = //: return SpeedModel.getPlaceholderModel(chatType: .moreMsg)
        .shadowMargin(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func jobHide() {
        //: self.officialArr = WithinMargin.shared.getOfficialList()
        self.officialArr = WithinMargin.shared.convert()
        //: self.topArr = WithinMargin.shared.topConvList
        self.topArr = WithinMargin.shared.topConvList
        //: self.normalArr = WithinMargin.shared.norConvList
        self.normalArr = WithinMargin.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        space()
    ...
    func difficultStrength(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: WithinMargin.shared.getChatRoomData { succeed in
        WithinMargin.shared.evolution { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = WithinMargin.shared.chatRoomList
                self.chatRoomArr = WithinMargin.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func spreadObserve(indexPath: IndexPath) -> SpeedModel? {
    ...
    func environmentDown(model: SpeedModel?) -> SpeedModel? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: BetweenInput? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: BetweenInput? = FinishAssociated.resumeQuery(targetId: targetId)
    ...
    func terms(conversationList: [SpeedModel], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        FinishAssociated.composition(with: tempArr) { _, error in
    ...
    func noAlbumSignal(conversationModel: SpeedModel) {
    ...
    func loadMagnitudeSmall(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: WithinMargin.shared.func__getConversationList { arr, leftMore in
        WithinMargin.shared.labDownDraw { arr, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [SpeedModel] = //: return Array<SpeedModel>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [SpeedModel] = //: return Array<SpeedModel>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [SpeedModel] = //: return Array<SpeedModel>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [SpeedModel] = //: return Array<SpeedModel>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [SpeedModel] = //: return Array<SpeedModel>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [SpeedModel] = //: return Array<SpeedModel>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [SpeedModel] = //: return Array<SpeedModel>()
        .init()
    //: }()
}
...
extension FixedBlue {
    ...
    func space() {
        //: self.moreMsgArr = WithinMargin.shared.moreMsgList
        self.moreMsgArr = WithinMargin.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func tillBound(indexPath: IndexPath) -> SpeedModel? {
    ...
    func fractionIn() -> SpeedModel {
        ...
        moreMsgModel.unreadCount = makeInKit()
...
extension FixedBlue {
    ...
    func underRobot(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        FinishAssociated.off(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(String(main_monsterStr))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(showComputeTitle.replacingOccurrences(of: "sibling", with: "l") + "eStat" + const_reparationQuantityervalTitle.replacingOccurrences(of: "start", with: "u"))].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(kRegulateCouldFormat.prefix(3)) + String(k_hideEqualFormat.prefix(6)))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(const_sparkFormat.suffix(4)) + "Auth")].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(String(mainStructurePath) + user_renderFormat.replacingOccurrences(of: "global", with: "e"))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(data_sampleTitle.prefix(5)) + String(app_locationPath))].int ?? 1
                    ...
                    self.housing(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func increasedWith(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(main_monsterStr))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(showComputeTitle.replacingOccurrences(of: "sibling", with: "l") + "eStat" + const_reparationQuantityervalTitle.replacingOccurrences(of: "start", with: "u"))] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(kRegulateCouldFormat.prefix(3)) + String(k_hideEqualFormat.prefix(6)))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(data_sampleTitle.prefix(5)) + String(app_locationPath))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(data_sampleTitle.prefix(5)) + String(app_locationPath))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension FixedBlue {
    ...
    func average() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in WithinMargin.shared.norConvList {
        for model in WithinMargin.shared.norConvList {
            //: if model.totalIntimate >= QuantityeractionDelay.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= QuantityeractionDelay.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.environmentDown(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in WithinMargin.shared.topConvList {
        for model in WithinMargin.shared.topConvList {
            //: if model.totalIntimate >= QuantityeractionDelay.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= QuantityeractionDelay.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func enableLibrary(convModel: SpeedModel, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func affect(indexPath: IndexPath) -> SpeedModel? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func housing(uid: String, intimate: Int) {
        ...
        FinishAssociated.netGraphic(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func provideEnable(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(main_monsterStr))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(main_searchTitle.replacingOccurrences(of: "component", with: "n") + "mate")] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
