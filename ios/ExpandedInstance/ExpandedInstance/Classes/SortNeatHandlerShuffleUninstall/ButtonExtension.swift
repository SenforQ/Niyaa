...
/*: "recharge" :*/
fileprivate let notiEnablelyFormat:String = "purple"
fileprivate let userIndependentKey:[Character] = ["c","h","a","r","g","e"]

/*: "recharge_ :*/
fileprivate let showMessageString:[Character] = ["r","e","c","h","a","r","g","e","_"]

/*: ".png" :*/
fileprivate let notiColonialMsg:[Character] = [".","p","n","g"]

/*: .png" :*/
fileprivate let dataSablePath:String = ".pngevery style mouse"

/*: ".jpg" :*/
fileprivate let k_stageCareMessage:String = ".jpgentity bad"

/*: .jpg" :*/
fileprivate let user_indicatorName:[Character] = [".","j","p","g"]
...
enum FrequencyDenseOption: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class ArrayButton: UIButton {
    ...
    var imageAlignment: FrequencyDenseOption = .left
...
extension UIButton {
    ...
    func installTypical(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        calledFor(interval: interval, topOffset: 0)
    }
    ...
    func calledFor(interval: CGFloat, topOffset: CGFloat) {
    ...
    func aboveAcross(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func graphChain(color: UIColor, forState: UIControl.State) {
    ...
    func totalegrationAdmin(urlStr: String,
                            //: placeImg: UIImage? = UIImage.placeImgSquare(),
                            placeImg: UIImage? = UIImage.embraceByBold(),
                            ...
                            loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: showGroupFlexibleFlowName.occur(), with: show_whiteTitle)
    ...
    func alongWhoRecto(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((notiEnablelyFormat.replacingOccurrences(of: "purple", with: "re") + String(userIndependentKey))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (notiEnablelyFormat.replacingOccurrences(of: "purple", with: "re") + String(userIndependentKey)), with: (String(showMessageString)) + "\(appMarkerResultValue)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.totalegrationAdmin(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(notiColonialMsg))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(notiColonialMsg)), with: "_\(appMarkerResultValue)" + (String(dataSablePath.prefix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(k_stageCareMessage.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(k_stageCareMessage.prefix(4))), with: "_\(appMarkerResultValue)" + (String(user_indicatorName)))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.totalegrationAdmin(urlStr: newUrl, placeImg: nil)
    }
