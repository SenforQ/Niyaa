...
fileprivate let userPostName:[UInt8] = [0x81,0x86,0x81,0x9c,0xc0,0x8b,0x87,0x8c,0x8d,0x9a,0xd2,0xc1,0xc8,0x80,0x89,0x9b,0xc8,0x86,0x87,0x9c,0xc8,0x8a,0x8d,0x8d,0x86,0xc8,0x81,0x85,0x98,0x84,0x8d,0x85,0x8d,0x86,0x9c,0x8d,0x8c]

/*: "#FFD13A" :*/
fileprivate let const_openlyYieldMsg:String = "speaker three#FFD13A"

/*: "gift_gb_umnad_1" :*/
fileprivate let appLowMsg:[Character] = ["g","i","f","t","_","g","b","_","u","m","n","a","d"]
fileprivate let show_uponPath:String = "_1"

/*: "icon_circle_pic_default" :*/
fileprivate let kRemoteKey:[Character] = ["i","c","o","n","_","c","i","r","c","l","e","_","p"]
fileprivate let kPlateMsg:String = "infect hi chapteric_def"

/*: "Open lucky blind-box" :*/
fileprivate let showLightMpTitle:String = "Open own rein brush"
fileprivate let show_tillFoundationThreeTitle:String = "remainck"
fileprivate let kWeightKey:String = "nd-boxbrick blend minimize spread local"

/*: "Sent " :*/
fileprivate let main_starTitle:String = "Sent launch key reserve when"

/*: "PingFangSC-Semibold" :*/
fileprivate let mainRelatedValue:String = "Pingtotal minute independent"
fileprivate let const_groupStr:String = "security denyC-Sem"

/*: "Bombard" :*/
fileprivate let data_submitUrl:String = "Bombardpackage rank owner bean"
...
class ButtonPositionCompare: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = DarkLost()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: userPostName.map{$0^232}, encoding: .utf8)!)
    }
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(const_openlyYieldMsg.suffix(7))))
        //: lb.font = UIFont.pingfangRugularFont(fontSize: 13)
        lb.font = UIFont.botLocal(fontSize: 13)
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var pluralTip: CancelWould = {
    ...
    private lazy var countLb: CancelWould = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = CancelWould()
...
extension ButtonPositionCompare {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func renderMap(model: DarkLost) {
        ...
        self.bgImage.image = UIImage.transferAndJoinSmall(name: (String(appLowMsg) + show_uponPath.capitalized))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.transferAndJoinSmall(name: (String(kRemoteKey) + String(kPlateMsg.suffix(6)) + "ault"))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == BulletPrepare.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(showLightMpTitle.prefix(5)) + show_tillFoundationThreeTitle.replacingOccurrences(of: "remain", with: "lu") + "y bli" + String(kWeightKey.prefix(6))).localized
            //: }else {
        } else {
        ...
        self.avatarImage.viaCluster(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.viaCluster(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(main_starTitle.prefix(5))).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(mainRelatedValue.prefix(4)) + "FangS" + String(const_groupStr.suffix(5)) + "ibold"), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = via(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(mainRelatedValue.prefix(4)) + "FangS" + String(const_groupStr.suffix(5)) + "ibold"), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = via(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.readingThumb(texts: strs, colors: colors, fonts: fonts)
        ...
        self.format(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func format(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(mainRelatedValue.prefix(4)) + "FangS" + String(const_groupStr.suffix(5)) + "ibold"), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = via(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(mainRelatedValue.prefix(4)) + "FangS" + String(const_groupStr.suffix(5)) + "ibold"), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = via(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.robotBy(150, 77, 20, 1), UIColor.white, UIColor.robotBy(150, 77, 20, 1)]
        ...
        attText = attText.readingThumb(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.at(attributedText: attText)
        ...
        countLb.anRewardProgress(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func onWeight(giftMsgModel: DarkLost) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(thePublisher), object: nil)

        //: refreshGiftCount(giftCount: giftMsgModel.comboNum)
        format(giftCount: giftMsgModel.comboNum)
        ...
        self.perform(#selector(thePublisher), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func contactWithOption(model: DarkLost, animate: Bool) {
    ...
    func triggerQuestion() {}

    //: func animationViewToTop(frame: CGRect) {
    func obtainTo(frame: CGRect) {
        //: UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
        UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
            //: self.frame = frame
            self.frame = frame
    ...
    @objc func thePublisher() {
        //: NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(animateHide), object: nil)
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(thePublisher), object: nil)
        //: giftMsgModel = TalkingGiftMsgAnimatModel()
        giftMsgModel = DarkLost()
...
extension ButtonPositionCompare {
    ...
    func via(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension ButtonPositionCompare {
    ...
    private func lessCalled() {
        ...
        font = via(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.readingThumb(texts: [(String(data_submitUrl.prefix(7))).localized], colors: [UIColor.robotBy(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.anRewardProgress(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.at(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.anRewardProgress(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
