...
#import "UIImage+BeyondSend.h"

//: @implementation UIImage (Compress)
@implementation UIImage (BeyondSend)

//: - (NSData *)compressedImageData {
- (NSData *)basicSink {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.next;
...
- (UIImage *)strength {
    ...
    return [UIImage imageWithData:[self basicSink]];
}
...
- (UIImage *)element:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self timing:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self pop:size];
...
- (CGSize)timing:(Boolean)isSession {
...
+ (NSData *)maintain:(UIImage *)image both:(NSInteger)fileSize{
    ...
    if (shortSize>1080) {//压缩到最短边不大于1080,也避免了shortSize==0的情况
        ...
        image = [image reportPermanentEnter:targetSize];
    }

    //: UIImage *thumIMG = image;
    UIImage *thumIMG = image;
    //: NSData *outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
    NSData *outIMGData = [thumIMG arrayRounding:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG dark:scalesize consume:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG arrayRounding:(fileSize*1024)];
    }
...
- (UIImage*)reportPermanentEnter:(CGSize)targetSize{
...
- (UIImage *)dark:(CGSize)targetSize consume:(BOOL)isScale{
...
- (NSData *)arrayRounding:(NSInteger)fileSize{
