...
fileprivate let constRetiredWouldFormat:[UInt8] = [0x8,0xf,0x8,0x15,0x49,0x2,0xe,0x5,0x4,0x13,0x5b,0x48,0x41,0x9,0x0,0x12,0x41,0xf,0xe,0x15,0x41,0x3,0x4,0x4,0xf,0x41,0x8,0xc,0x11,0xd,0x4,0xc,0x4,0xf,0x15,0x4,0x5]

private func pullMinimum(cheap num: UInt8) -> UInt8 {
    return num ^ 97
}

/*: "gift_bg_dm" :*/
fileprivate let noti_throughWorkPath:String = "scan usergift_b"
fileprivate let const_advancedKey:String = "g_dmexpected drag feature"

/*: "#FFF4B5" :*/
fileprivate let mainQueryionRegionStr:String = "scale exist yield#FFF4B5"

/*: "won " :*/
fileprivate let showCertainEnterMessage:[Character] = ["w","o","n"," "]

/*: " coins in " :*/
fileprivate let noti_observeCloudFormat:[Character] = [" ","c","o","i","n","s"," ","i","n"]
fileprivate let mainEquallyValue:String = "push"

/*: "just sent" :*/
fileprivate let data_proGuideMsg:[Character] = ["j","u","s","t"," ","s","e","n"]
fileprivate let main_locateRocketKey:[Character] = ["t"]

/*: "#4120C1" :*/
fileprivate let dataBirthdayString:String = "#412someone"
fileprivate let notiEaseFixedId:String = "read"
...
class BottomView: UIView {
    ...
    private var giftModel = WithoutResume()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constRetiredWouldFormat.map{pullMinimum(cheap: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.transferAndJoinSmall(name: (String(noti_throughWorkPath.suffix(6)) + String(const_advancedKey.prefix(4))))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(mainQueryionRegionStr.suffix(7))))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(mainQueryionRegionStr.suffix(7))))
...
extension BottomView {
    ...
    func recoverFor(_ model: WithoutResume) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.okUponLock(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(showCertainEnterMessage)).localized + "\(giftModel.totalCoin)" + (String(noti_observeCloudFormat) + mainEquallyValue.replacingOccurrences(of: "push", with: " ")).localized + giftModel.gameName
            ...
            giftImgV.okUponLock(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (String(data_proGuideMsg) + String(main_locateRocketKey)).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = greatAspect(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (dataBirthdayString.replacingOccurrences(of: "someone", with: "0") + notiEaseFixedId.replacingOccurrences(of: "read", with: "C1")))!, range: nsRange)
            }
    ...
    private func greatAspect(giftmodel: WithoutResume) -> String {
...
extension BottomView {
    ...
    private func underReport() {
        //: self.frame = CGRect(x: 0, y: StatusBarNavigationBarHeight, width: ScreenWidth, height: 60)
        self.frame = CGRect(x: 0, y: app_whenAppearanceData, width: dataTabUrl, height: 60)
