...
fileprivate let app_translateMsg:[UInt8] = [0x3b,0x3c,0x3b,0x26,0x7a,0x31,0x3d,0x36,0x37,0x20,0x68,0x7b,0x72,0x3a,0x33,0x21,0x72,0x3c,0x3d,0x26,0x72,0x30,0x37,0x37,0x3c,0x72,0x3b,0x3f,0x22,0x3e,0x37,0x3f,0x37,0x3c,0x26,0x37,0x36]

private func capableCloud(gold num: UInt8) -> UInt8 {
    return num ^ 82
}

/*: "icon_chat_translate_nor" :*/
fileprivate let constSkinData:String = "icon_ccrystal define"
fileprivate let dataPublisherArchitectureString:String = "yourselfnsl"
fileprivate let dataUsedOriginUrl:[Character] = ["r"]

/*: "icon_chat_translate_pre" :*/
fileprivate let showCollectionName:[Character] = ["i","c","o","n","_","c","h","a","t","_","t","r","a","n","s","l","a","t","e","_","p","r","e"]

/*: "icon_chat_miss" :*/
fileprivate let appCreateStr:String = "icon_corientation main presentation"
fileprivate let userServerFormat:[Character] = ["h","a","t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let app_recognizeValue:[Character] = ["i","c","o","n","_","c","h","a","t","_","y"]
fileprivate let notiOnerUrl:String = "estrength"
...
class ShowJob: LocateMight {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: AlongMount?
    var textData: AlongMount?
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: app_translateMsg.map{capableCloud(gold: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.transferAndJoinSmall(name: (String(constSkinData.prefix(6)) + "hat_t" + dataPublisherArchitectureString.replacingOccurrences(of: "yourself", with: "ra") + "ate_no" + String(dataUsedOriginUrl))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.transferAndJoinSmall(name: (String(showCollectionName))), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(successfullySink), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ShowJob {
    ...
    override func panel(with data: CleanShared) {
        ...
        let maxWidth = VersusQuit.lawmakerExtensive() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.transferAndJoinSmall(name: (String(appCreateStr.prefix(6)) + String(userServerFormat)))
            ...
            self.transStatusImagV.image = UIImage.transferAndJoinSmall(name: (String(app_recognizeValue) + notiOnerUrl.replacingOccurrences(of: "strength", with: "s")))
        }
    ...
    func m() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(pastCluster(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func pastCluster(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
