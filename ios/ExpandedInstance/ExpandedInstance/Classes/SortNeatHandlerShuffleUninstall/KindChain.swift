...
/*: "gifts" :*/
fileprivate let const_contrastFormat:[Character] = ["g","i","f","t","s"]

/*: "vipGifts" :*/
fileprivate let showGivingId:String = "page every off head shadevipG"

/*: "packGifts" :*/
fileprivate let noti_transactionPath:String = "packGiunable retain"
fileprivate let dataYearUrl:String = "FTS"

/*: "giftSendItem" :*/
fileprivate let k_recommendStr:String = "nor failure fault news validgiftS"
fileprivate let k_lonelyName:String = "EN"
fileprivate let appRefuseEducatorString:[Character] = ["d","I","t","e","m"]
...
class KindChain: NSObject {
    ...
    private var cacheDict = [String: [CrucialBar: [Any]]]()
    // 数据锁
    ...
    private var giftType = AspectSize.normal

    //: static let share = TalkingChatGiftManager()
    static let share = KindChain()
...
extension KindChain {
    ...
    func isObtainArc(type: AspectSize = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        KindChain.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[CrucialBar.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            KindChain.share.occurAndAccuse(completion: { isSucceed in
            ...
            KindChain.share.everyFailure(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            KindChain.share.disableAcrossColumn(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func disableAcrossColumn(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = KindChain.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        AgainstListen.blocGreet(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [CrucialBar: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(String(const_contrastFormat))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<CombineFactor>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[CrucialBar.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(showGivingId.suffix(4)) + "ifts")] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<CombineFactor>.deserialize(from: array as? Array) {
                    ...
                    dataDict[CrucialBar.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(noti_transactionPath.prefix(6)) + dataYearUrl.lowercased())] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<CombineFactor>.deserialize(from: array as? Array) {
                    ...
                    dataDict[CrucialBar.Bags] = bagsArr
                }
                ...
                let sendItem: Array = data[(String(k_recommendStr.suffix(5)) + k_lonelyName.lowercased() + String(appRefuseEducatorString))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<ThresholdWrite>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func occurAndAccuse(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = KindChain.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        AgainstListen.speakerVoice(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func junior(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        AgainstListen.pureLoop(params: params, completion: { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<AngleAdd>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func everyFailure(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = AspectSize.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        AgainstListen.aboveNatural(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func joinMap(perPageCount: Int, dataType: CrucialBar) -> Int {
    ...
    func afterInvisible(dataType: CrucialBar) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func errDown(dataType: CrucialBar, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension KindChain {
    ...
    private func profile() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(clusterBoth),
                                               //: name: GIFT_REFRESHPACKAGE_NOTIFICATION,
                                               name: user_againstTitle,
                                               //: object: nil)
                                               object: nil)
    }
    ...
    @objc private func clusterBoth() {
        //: gift_clearMemoryCache(showRed: true)
        languageProjection(showRed: true)
    }
}
...
extension KindChain {
    ...
    func languageProjection(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        KindChain.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
