...
/*: "#ffffff" :*/
fileprivate let showYaData:String = "#ffffffexport through capture loop"

/*: "/dist/loungePlus/index.html" :*/
fileprivate let userAdminFilterFormat:[Character] = ["/","d","i","s","t","/","l","o","u","n","g","e","P","l","u","s","/","i","n","d","e"]
fileprivate let noti_honeyDeliveryPath:[Character] = ["x",".","h","t","m","l"]

/*: "https://" :*/
fileprivate let noti_fastId:String = "before left popular scenehttps://"

/*: "http://" :*/
fileprivate let data_factorUrl:[Character] = ["h","t","t","p",":"]
fileprivate let appDetailValue:String = "detectdetect"
...
class OptionView: UIView {
    ...
    let Broadcast_TextColor = (String(showYaData.prefix(7)))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .wantStream(fontSize: 12)
    ...
    convenience init(model: DisabledModel?) {
        //: self.init()
        self.init()
    ...
    private func noneError() {
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            var mutableAtt = parse.comprehensiveDrown(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func fullInGlobal(withUrl url: String) {
        ...
        self.backgroundImageView.viaCluster(urlStr: url) { [weak self] result, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.policyCityDateLimited(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: DisabledModel? {
        //: didSet {
        didSet {
            //: configModel()
            noneError()
    ...
    var totalTime: CGFloat {
        ...
        return (dataTabUrl + widthValue) * Broadcast_SpendTime / dataTabUrl
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / dataTabUrl
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / dataTabUrl
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(throughMoment(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension OptionView {
    ...
    @objc func throughMoment(_ tapGesture: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: TellEnd.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            TellEnd.share.butterfly(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: TellEnd.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                TellEnd.share.activityAcrossInclude(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(userAdminFilterFormat) + String(noti_honeyDeliveryPath))) {
                    //: TellEnd.share.func__pushToSubscribePageWebVC()
                    TellEnd.share.midOf()
                    //: return
                    return
                }
                //: let httpsUrl = QuantityeractionDelay.share.appConfigMode.urlH5Domain+url
                let httpsUrl = QuantityeractionDelay.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(noti_fastId.suffix(8))), with: (String(data_factorUrl) + appDetailValue.replacingOccurrences(of: "detect", with: "/")))
                //: TellEnd.share.func__pushToWebVC(urlStr: httpUrl)
                TellEnd.share.atRein(urlStr: httpUrl)
            }
