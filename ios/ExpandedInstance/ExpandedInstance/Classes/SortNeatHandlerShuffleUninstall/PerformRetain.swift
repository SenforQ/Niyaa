...
/*: "/TXCache" :*/
fileprivate let mainAccelerateId:[Character] = ["/","T","X","C","a","c"]
fileprivate let mainWrittenUrl:String = "hdescription"
...
class PerformRetain: NSObject {
    ...
    open weak var delegate: PinHero?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: ConsumeNeed?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = PerformRetain()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(mainAccelerateId) + mainWrittenUrl.replacingOccurrences(of: "description", with: "e"))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(volume), userInfo: nil, repeats: true)
...
extension PerformRetain {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func like(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: showGroupFlexibleFlowName.occur(), with: show_whiteTitle)
    ...
    @objc func volume() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.output(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func reason(renderMode: IsolateFlex) {
    ...
    func graph(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.agileImageSkewy == true {
        if UpZone.shared.agileImageSkewy == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func someFlow() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.advertising(player: self, status: self.playStatus!)
        }
    }
...
extension PerformRetain: TXVodPlayListener {
    ...
    func onPlayEvent(_ player: TXVodPlayer!, event EvtID: Int32, withParam param: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.doArea(player: self, duration: Int(duration), currentTime: Int(currentTime))
