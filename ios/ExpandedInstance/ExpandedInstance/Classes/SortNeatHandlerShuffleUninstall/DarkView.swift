...
fileprivate let app_addressOperationName:[UInt8] = [0x36,0x3b,0x36,0x41,0xf5,0x30,0x3c,0x31,0x32,0x3f,0x7,0xf6,0xed,0x35,0x2e,0x40,0xed,0x3b,0x3c,0x41,0xed,0x2f,0x32,0x32,0x3b,0xed,0x36,0x3a,0x3d,0x39,0x32,0x3a,0x32,0x3b,0x41,0x32,0x31]

fileprivate func nearEager(avoid num: UInt8) -> UInt8 {
...
/*: "iosSvgaEffect" :*/
fileprivate let noti_profileLaunchMessage:[Character] = ["i","o","s","S","v","g","a","E","f","f","e","c"]
fileprivate let noti_besideTitle:String = "available"

/*: "iosEffect" :*/
fileprivate let show_tillString:[Character] = ["i","o","s","E","f","f","e","c","t"]

/*: "entityId" :*/
fileprivate let show_temporaryUrl:[Character] = ["e","n","t","i","t","y","I","d"]
...
class DarkView: UIView {
    ...
    var currentEffectMsgModel: BoldEnable?
    ...
    var animatedView: ObserveView?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: DivideHead?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: app_addressOperationName.map{nearEager(avoid: $0)}, encoding: .utf8)!)
    }
...
extension DarkView {
    ...
    func buttTo(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(noti_profileLaunchMessage) + noti_besideTitle.replacingOccurrences(of: "available", with: "t"))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(show_tillString))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(show_temporaryUrl))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = greatMedal(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func greatMedal(msgModel: Any?) -> Bool {
        ...
        if msgModel is BoldEnable {
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.eventIdentity(giftNum: 1) // 礼物个数设置为1，每次动效显示
            ...
            if giftMsgModel.showType == BulletPrepare.myStery.rawValue || giftMsgModel.showType == BulletPrepare.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: BoldEnable = self.effectMsgModelArray[i] as! BoldEnable
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? BoldEnable())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? BoldEnable(), at: index)
                }
            ...
            frequencyOption(effectMsgModel: effectMsgModel!)
        }
...
extension DarkView {
    ...
    func collectionAm() {
        ...
        self.animatedView?.ceilingCoordinator()
    }
    ...
    func instanceTop() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = once() else { return }
                //: firstShow(effectMsgModel: removeModel)
                frequencyOption(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.deviceRe()
        }
    ...
    func once() -> BoldEnable? {
        ...
        if effectMsgModel is BoldEnable {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! BoldEnable
        ...
        return effectMsgModel as? BoldEnable
    }
}
...
extension DarkView {
    ...
    func frequencyOption(effectMsgModel: BoldEnable) {
        ...
        self.delegate?.consumptionWithoutMenu(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        DenseSpell.shared.strategyShared(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.restriction(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? DrawingRead())
            //: })
        })
    }
    ...
    func restriction(effectMsgModel: BoldEnable, success: Bool, effectModel: DrawingRead) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.explain(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    attentionLock(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func characteristic(effectMsgModel: BoldEnable, effectModel: DrawingRead) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.commandHeap()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.oval()
        ...
        className = DenseSpell.shared.noWait(oldName: className ?? "")
        //: let clsName = PodspecName + "." + className!
        let clsName = k_optionUrl + "." + className!
        ...
        let realClass = AnimatedViewClass as! ObserveView.Type
...
extension DarkView: MarginClean {
    ...
    func momentumAvailable(effectItemView: ObserveView, success: Bool) {
        ...
        if !success {
            ...
            distributionBound(success: success)
        }
        ...
        self.delegate?.agreePerform(effectView: self, obj: effectItemView.effectMsgModel ?? BoldEnable(), success: success)
    }
    ...
    func manageParty(effectItemView: ObserveView) {
        ...
        if effectItemView == self.animatedView {
            ...
            if !self.needStopAll {
                ...
                distributionBound(success: true)
            }
        }
        ...
        self.delegate?.roundingCollection(effectView: self, obj: effectMsgModel!)
    }
}
...
extension DarkView {
    ...
    func hide() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.commandHeap() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.oval() // clean animation data
    ...
    func distributionBound(success: Bool) {
        //: cleanData()
        hide()
