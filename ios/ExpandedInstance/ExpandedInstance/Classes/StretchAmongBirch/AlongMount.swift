...
fileprivate let constSheKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "[未知消息]" :*/
fileprivate let notiWarningString:String = "[未知消intensity]"

/*: "extra" :*/
fileprivate let kBothMsg:String = "rebuildtra"

/*: "#0084FF" :*/
fileprivate let dataFaceSeaKey:String = "appropriate your#0084F"
fileprivate let main_complyName:String = "heap"

/*: "#FFEE4B" :*/
fileprivate let k_giveString:[Character] = ["#","F","F","E","E","4","B"]

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let mainMovementTitle:String = "<at>voice catch daily"
fileprivate let const_jobConFormat:String = "\\s]+?"
fileprivate let notiLapBackgroundKey:[Character] = ["<","/","a","t",">"]

/*: "jumpKey" :*/
fileprivate let const_workKey:String = "photo tween multiple keepjumpKey"

/*: "user" :*/
fileprivate let notiBehaviorTitle:String = "uthemer"

/*: "uid" :*/
fileprivate let constReflectValue:[UInt8] = [0xe7,0xfb,0xf6]

private func maxTemp(cut num: UInt8) -> UInt8 {
    return num ^ 146
}

/*: "#16D073" :*/
fileprivate let const_flexUsPath:String = "relation launch mark resource#16D0"
fileprivate let const_plateKey:String = "load3"

/*: "#F95644" :*/
fileprivate let show_stackData:String = "assistant movie bean deploy#F95644"
...
//: @objcMembers public class AlongMount: TalkingChatMsgBaseCellData {
@objcMembers public class AlongMount: LocalStrength {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constSheKey.reversed(), encoding: .utf8)!)
    }
    ...
    override public var msgModel: SceneModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (notiWarningString.replacingOccurrences(of: "intensity", with: "息"))
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = radio(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func radio(msgInfoModel: WithoutModel) -> NSMutableAttributedString {
        ...
        guard dic[(kBothMsg.replacingOccurrences(of: "rebuild", with: "ex"))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.thinOf() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(dataFaceSeaKey.suffix(6)) + main_complyName.replacingOccurrences(of: "heap", with: "F")))! : UIColor(hex: (String(k_giveString)))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(mainMovementTitle.prefix(4)) + "@[\\S" + const_jobConFormat + String(notiLapBackgroundKey)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(const_workKey.suffix(7)))].stringValue == (notiBehaviorTitle.replacingOccurrences(of: "theme", with: "se")) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: constReflectValue.map{maxTemp(cut: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: constReflectValue.map{maxTemp(cut: $0)}, encoding: .utf8)!: uid]) { [weak self] containerView, text, range, rect in
    ...
    func square() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(const_flexUsPath.suffix(5)) + const_plateKey.replacingOccurrences(of: "load", with: "7")))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(show_stackData.suffix(7))))!
        }
