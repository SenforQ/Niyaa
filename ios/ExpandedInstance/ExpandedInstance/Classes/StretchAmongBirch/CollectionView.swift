...
/*: "#ffffff" :*/
fileprivate let constNumberValueDisableKey:String = "#ffffwritten tine continuing"
fileprivate let kMasterTailedMessage:String = "arrayarray"

/*: "earned" :*/
fileprivate let app_minAspectString:String = "earneanonymous"

/*: "Coins in" :*/
fileprivate let appRecordStr:[Character] = ["C","o","i","n","s"," ","i","n"]

/*: "Error:  :*/
fileprivate let notiScaleTitle:String = "tip"
fileprivate let noti_headDealPath:String = "rror: ad divide"

/*: "#C946D2" :*/
fileprivate let main_calledKey:[Character] = ["#","C","9","4","6","D","2"]

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let showFirstPath:String = "btn_sable existing"
fileprivate let data_mergeReadId:String = "informationoa"
fileprivate let notiStatusAskTitle:String = "own effectserBK"
...
class CollectionView: UIView {
    ...
    private let TextColor = (String(constNumberValueDisableKey.prefix(5)) + kMasterTailedMessage.replacingOccurrences(of: "array", with: "f"))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: AttachRapid?) {
        //: self.init()
        self.init()
    ...
    private func build() {
        ...
        if danumuMsgModel.headPic.count > 0 {
            ...
            headAnimatedImageView.okUponLock(urlStr: danumuMsgModel.headPic)
        }
        //: if danumuMsgModel.bgPic.count > 0 {
        if danumuMsgModel.bgPic.count > 0 {
            //: updateBackground(withUrl: danumuMsgModel.bgPic)
            condition(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (app_minAspectString.replacingOccurrences(of: "anonymous", with: "d"))) {
                ...
                if let range = attrStr.string.range(of: (String(appRecordStr))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.alongsideOk(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (notiScaleTitle.replacingOccurrences(of: "tip", with: "E") + String(noti_headDealPath.prefix(6))) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func condition(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.viaCluster(urlStr: url) { [weak self] result, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.policyCityDateLimited(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: AttachRapid? {
        //: didSet {
        didSet {
            //: configModel()
            build()
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(main_calledKey)))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.transferAndJoinSmall(name: (String(showFirstPath.prefix(4)) + "chat_" + data_mergeReadId.replacingOccurrences(of: "information", with: "fl") + "tS_u" + String(notiStatusAskTitle.suffix(5)) + "_nor"))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(column(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        ...
        l.font = UIFont.wantStream(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
        l.numberOfLines = 1
...
extension CollectionView {
    ...
    @objc func column(_ tapGesture: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = QuantityeractionDelay.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = QuantityeractionDelay.share.appConfigMode.urlH5Domain + gameUrl
        ...
        config.clearBgColor = true
        //: TellEnd.share.func__pushToWebVC(urlStr: httpsUrl, webConfig: config)
        TellEnd.share.atRein(urlStr: httpsUrl, webConfig: config)
    }
}
