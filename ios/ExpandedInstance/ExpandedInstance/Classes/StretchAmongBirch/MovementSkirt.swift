...
fileprivate let show_presentStatStr:[UInt8] = [0xc2,0xc7,0xc2,0xcd,0x81,0xbc,0xc8,0xbd,0xbe,0xcb,0x93,0x82,0x79,0xc1,0xba,0xcc,0x79,0xc7,0xc8,0xcd,0x79,0xbb,0xbe,0xbe,0xc7,0x79,0xc2,0xc6,0xc9,0xc5,0xbe,0xc6,0xbe,0xc7,0xcd,0xbe,0xbd]

fileprivate func supporterPerform(exactly num: UInt8) -> UInt8 {
...
fileprivate let k_popInstanceKey:[Character] = ["Y","o","u","\'","v","e"," ","g","o","t"," ","n","o"," ","m","e","s","s","a","g"]
fileprivate let showHalfKey:[Character] = ["e"]
fileprivate let userAgainRemMsg:[Character] = [" ","y","e","t","."]

/*: "icon_kong_kong_default" :*/
fileprivate let noti_rootAmongTitle:String = "ifollowo"
fileprivate let k_eagerSpendValue:[Character] = ["n","_","k","o","n","g","_","k","o","n","g","_","d","e","f","a","u","l","t"]

/*: "Cancel" :*/
fileprivate let data_techniqueMessage:String = "Cancedestroy yourself"
fileprivate let mainCurriculumMessage:String = "unless"

/*: "uid" :*/
fileprivate let kTeaValue:[UInt8] = [0x64,0x69,0x75]

/*: "btn_message_report_nor" :*/
fileprivate let constDialogKey:String = "btn_calculation carve start male lock"
fileprivate let app_analysisId:String = "mix distinction lonely why tae_repo"
fileprivate let userTurnFrontValue:[Character] = ["r"]
fileprivate let dataCenterId:String = "t_norown pet"

/*: "#FF935D" :*/
fileprivate let appRankMessage:String = "#FF935Dnumb while collect found"

/*: "btn_message_block_nor" :*/
fileprivate let dataPrivacyHealthyKey:[Character] = ["b","t","n","_","m","e","s","s","a","g","e","_","b","l","o","c","k","_","n","o","r"]

/*: "#C179F9" :*/
fileprivate let k_consequenceAppealValue:String = "hide"
fileprivate let userSpanUrl:String = "C17lockFlock"

/*: "btn_message_untop_nor" :*/
fileprivate let appExistValue:[UInt8] = [0x5c,0x6e,0x68,0x59,0x67,0x5f,0x6d,0x6d,0x5b,0x61,0x5f,0x59,0x6f,0x68,0x6e,0x69,0x6a,0x59,0x68,0x69,0x6c]

fileprivate func recognizeInner(pressure num: UInt8) -> UInt8 {
    let value = Int(num) + 6
    if value > 255 {
        return UInt8(value - 256)
    } else {
...
/*: "btn_message_top_nor" :*/
fileprivate let kFogQueryedKey:[Character] = ["b","t","n","_","m"]
fileprivate let notiPendingString:[Character] = ["e","s","s","a","g","e","_","t","o","p","_","n","o","r"]

/*: "#8A79F9" :*/
fileprivate let const_sureReadString:[Character] = ["#","8","A","7","9","F","9"]

/*: "btn_message_delete_nor" :*/
fileprivate let constAdvancedTitle:String = "btn_mepattern plus"
fileprivate let show_rapidPath:String = "edit customer last innere_del"
fileprivate let notiRequireMessage:String = "ete_noryear occur since"

/*: "#FF506D" :*/
fileprivate let appErrorValue:String = "#FF506Dnatural visible at secure"
...
class MovementSkirt: ScaleViewController {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: show_presentStatStr.map{supporterPerform(exactly: $0)}, encoding: .utf8)!)
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        super.viewWillAppear(animated)
        //: WithinMargin.shared.func__refreshSysMsg()
        WithinMargin.shared.container()
        // 公共聊天室
        //: self.manager.req_chatRommArr { [weak self] succeed in
        self.manager.difficultStrength { [weak self] succeed in
    ...
    private lazy var tableView: UITableView = {
        ...
        tableView.tillMini { [weak self] in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: self.refreshOnlineStatus(isFirst: true)
            self.duringFoundInput(isFirst: true)
        }
    ...
    private lazy var emptyView: EmptyView = {
        ...
        style.TipsTitle = (String(k_popInstanceKey) + String(showHalfKey) + String(userAgainRemMsg)).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (noti_rootAmongTitle.replacingOccurrences(of: "follow", with: "c") + String(k_eagerSpendValue))
    ...
    private lazy var topItemView: ConvertResolveCenter = {
        //: let view = TalkingChatListTopItemView.init()
        let view = ConvertResolveCenter()
        //: view.delegate = self
        view.delegate = self
...
extension MovementSkirt {
    /// 获取会话列表【增量返回】
    //: func getConversationList() {
    func haveInFade() {
        //: self.manager.req_getConversationList { [weak self] isFinish in
        self.manager.loadMagnitudeSmall { [weak self] isFinish in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + timeInterval) {
                ...
                self.duringFoundInput(isFirst: isFirst)
            }
        }
    }
    ...
    func duringFoundInput(isFirst: Bool) {
        //: self.manager.getConversationListOnlineStatus(isFirst: isFirst) { [weak self] succeed in
        self.manager.underRobot(isFirst: isFirst) { [weak self] succeed in
    ...
    func cornerLast(listData: [SpeedModel]) {
        // 获取用户信息
        //: if listData.count > 0 {
        if listData.count > 0 {
            //: self.manager.getUserInfoListFromCache(conversationList: listData) { [weak self] in
            self.manager.terms(conversationList: listData) { [weak self] in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.reloadData()
                self.jumpBe()
...
extension MovementSkirt {
    ...
    func maleCurrency(indexPath: IndexPath, conversationModel: SpeedModel) {
        ...
        var type: BlackLock = .normalList
        //: if conversationModel.isPinned { type = .topList }
        if conversationModel.isPinned { type = .topList }
        //: WithinMargin.shared.func__removeConversation(targetId: conversationModel.targetId, dataType: type)
        WithinMargin.shared.calendar(targetId: conversationModel.targetId, dataType: type)
    ...
    func bit(conversationModel: SpeedModel) {
        //: if !WithinMargin.shared.func__checkCanOperateList() { return }
        if !WithinMargin.shared.servicePic() { return }
        //: let isTop = !conversationModel.isPinned
        let isTop = !conversationModel.isPinned
        //: WithinMargin.shared.func__setConversationTop(convModel: conversationModel, isTop: isTop)
        WithinMargin.shared.random(convModel: conversationModel, isTop: isTop)

        //: if curType == .intimate {
        if curType == .intimate {
            //: self.manager.top_intimateData(convModel: conversationModel, isTop: isTop)
            self.manager.enableLibrary(convModel: conversationModel, isTop: isTop)
            //: self.tableView.reloadData()
            self.tableView.reloadData()
        }
    }
    ...
    func depth(indexPath: IndexPath, conversationModel: SpeedModel) {
        // 拉黑/解除拉黑
        ...
        TimingVersus.noEntityGuide(title: nil,
                                       ...
                                       leftBtnTitle: (String(data_techniqueMessage.prefix(5)) + mainCurriculumMessage.replacingOccurrences(of: "unless", with: "l")).localized,
                                       ...
                                       rightBtnTitle: "OK".localized)
        {
            ...
            ShouldBridge.sayStructure(uid: conversationModel.userID, isBlack: true) { succeed, result, errorModel in
                ...
                self.maleCurrency(indexPath: indexPath, conversationModel: conversationModel)
                //: NotificationCenter.default.post(name: LIVE_BLOCK_USER_NOTIFICATION,
                NotificationCenter.default.post(name: const_materialUrl,
                                                //: object: nil,
                                                object: nil,
                                                //: userInfo: ["uid": conversationModel.userID])
                                                userInfo: [String(bytes: kTeaValue.reversed(), encoding: .utf8)!: conversationModel.userID])
            }
        }
    }
    ...
    func reading(conversationModel: SpeedModel) {
...
extension MovementSkirt {
    ...
    func destroyNatural() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(sinkStatus(notification:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(pointHiHalf(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(sumerruptRouteBlack(notif:)),
    ...
    @objc func sinkStatus(notification: NSNotification) {
    ...
    @objc private func pointHiHalf(notif: Notification) {
    ...
    @objc private func sumerruptRouteBlack(notif: Notification) {
        //: guard let userInfo = notif.userInfo else { return }
        guard let userInfo = notif.userInfo else { return }
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: if self.manager.update_userIntimate(userInfo: userInfo as! [String: Any]) == true {
            if self.manager.provideEnable(userInfo: userInfo as! [String: Any]) == true {
                //: self.tableView.reloadData()
                self.tableView.reloadData()
            }
        }
...
extension MovementSkirt: OperationRole {
    ...
    func anySustainable(type: QuantityEmpty) {
        //: self.curType = type
        self.curType = type
        //: if self.curType == .All {
        if self.curType == .All {
            ...
            k_playShadowValue.behindImplement(eventID: show_mediumMsg)
        }
    }
}
...
extension MovementSkirt: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if curType == .All {
            ...
            case AppearanceImpactDoingto.offical.rawValue:
            ...
            case AppearanceImpactDoingto.chatRoom.rawValue:
                //: if self.manager.isHalf == true { return 0 }
                if self.manager.isHalf == true { return 0 }
            ...
            case AppearanceImpactDoingto.moreMsg.rawValue:
                //: if self.manager.isHalf == true { return 0 }
                if self.manager.isHalf == true { return 0 }
            ...
            case AppearanceImpactDoingto.top.rawValue:
                //: return self.manager.topArr.count
                return self.manager.topArr.count

            //: case ChatListSectionTyep.normal.rawValue:
            case AppearanceImpactDoingto.normal.rawValue:
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if curType == .All {
            ...
            if indexPath.section == AppearanceImpactDoingto.moreMsg.rawValue {
                //: model = self.manager.req_moreMsgGetPlaceholderModel()
                model = self.manager.fractionIn()
                //: } else {
            } else {
                //: model = self.manager.req_conversationModel(indexPath: indexPath)
                model = self.manager.spreadObserve(indexPath: indexPath)
            }
            //: } else {
        } else {
            //: model = self.manager.req_IntimateConversationModel(indexPath: indexPath)
            model = self.manager.affect(indexPath: indexPath)
        }
    ...
    func tableView(_ tableView: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
        ...
        if curType == .All {
            ...
            guard let model = self.manager.spreadObserve(indexPath: indexPath) else {
                //: return false
                return false
            }
            ...
            guard let model = self.manager.affect(indexPath: indexPath) else {
                //: return false
                return false
            }
            //: guard model.chatType == .private else { return false }
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let reportBtn = UIContextualAction(style: .normal, title: nil) { action, view, completionHandler in
            ...
            self.reading(conversationModel: model!)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let reportBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_report_nor").cgImage {
        if let reportBtnTrashcan = UIImage.transferAndJoinSmall(name: (String(constDialogKey.prefix(4)) + "messag" + String(app_analysisId.suffix(6)) + String(userTurnFrontValue) + String(dataCenterId.prefix(5)))).cgImage {
        ...
        reportBtn.backgroundColor = UIColor(hex: (String(appRankMessage.prefix(7))))
        ...
        let blockBtn = UIContextualAction(style: .normal, title: nil) { action, view, completionHandler in
            ...
            self.depth(indexPath: indexPath, conversationModel: model!)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let blockBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_block_nor").cgImage {
        if let blockBtnTrashcan = UIImage.transferAndJoinSmall(name: (String(dataPrivacyHealthyKey))).cgImage {
        ...
        blockBtn.backgroundColor = UIColor(hex: (k_consequenceAppealValue.replacingOccurrences(of: "hide", with: "#") + userSpanUrl.replacingOccurrences(of: "lock", with: "9")))
        ...
        let topBtn = UIContextualAction(style: .normal, title: nil) { action, view, completionHandler in
            ...
            self.bit(conversationModel: model!)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: let image = model!.isPinned ? "btn_message_untop_nor":"btn_message_top_nor"
        let image = model!.isPinned ? String(bytes: appExistValue.map{recognizeInner(pressure: $0)}, encoding: .utf8)! : (String(kFogQueryedKey) + String(notiPendingString))
        //: if let topBtnTrashcan = UIImage.BundleImageNamed(name: image).cgImage {
        if let topBtnTrashcan = UIImage.transferAndJoinSmall(name: image).cgImage {
            //: topBtn.image = ImageWithoutRender(cgImage: topBtnTrashcan, scale: UIScreen.main.nativeScale, orientation: .up)
            topBtn.image = DarkLike(cgImage: topBtnTrashcan, scale: UIScreen.main.nativeScale, orientation: .up)
        }
        //: topBtn.backgroundColor = UIColor(hex: "#8A79F9")
        topBtn.backgroundColor = UIColor(hex: (String(const_sureReadString)))

        // 删除
        //: let deleteBtn = UIContextualAction(style: .normal, title: nil) { action, view, completionHandler in
        let deleteBtn = UIContextualAction(style: .normal, title: nil) { action, view, completionHandler in
            //: self.deleteChatListCell(indexPath: indexPath, conversationModel: model!)
            self.maleCurrency(indexPath: indexPath, conversationModel: model!)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let deleteBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_delete_nor").cgImage {
        if let deleteBtnTrashcan = UIImage.transferAndJoinSmall(name: (String(constAdvancedTitle.prefix(6)) + "ssag" + String(show_rapidPath.suffix(5)) + String(notiRequireMessage.prefix(7)))).cgImage {
        ...
        deleteBtn.backgroundColor = UIColor(hex: (String(appErrorValue.prefix(7))))
    ...
    func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        ...
        if self.manager.isHalf == false &&
            ...
            section == AppearanceImpactDoingto.offical.rawValue &&
            //: self.manager.officialArr.count > 0 &&
            self.manager.officialArr.count > 0 &&
            //: QuantityeractionDelay.share.appStatus != AppSkinStatus.special.rawValue {
            QuantityeractionDelay.share.appStatus != TotalervalObserve.special.rawValue
        {
            //: return 15.0
            return 15.0
        }
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        if self.curType == .All && section == AppearanceImpactDoingto.offical.rawValue && self.manager.officialArr.count > 0 {
            //: let v = UIView()
            let v = UIView()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if curType == .All {
            //: if indexPath.section == ChatListSectionTyep.moreMsg.rawValue {
            if indexPath.section == AppearanceImpactDoingto.moreMsg.rawValue { // 更多消息
                //: let vc = TalkingMoreMessagesListViewController()
                let vc = DuringFinishSmall()
            ...
            if model.chatType == .group { // 群聊
                ...
                k_playShadowValue.behindImplement(eventID: showWithPropertyUrl, toUid: model.targetId)

                //: } else {
            } else {
                //: if model.targetId == MacroDefine.getXiaoMiID() {
                if model.targetId == VersusQuit.noneConsume() { // 系统消息
                    //: uploadRecord.uploadRecordEvent(eventID: ClickSystemNotificationNoP, toUid: model.targetId)
                    k_playShadowValue.behindImplement(eventID: k_oldMessage, toUid: model.targetId)
                    //: } else {
                } else {
                ...
                let isFrom = self.manager.isHalf ? TailProvider.LiveRoom : TailProvider.Normal
                //: TellEnd.share.func__pushToPriveteChatVC(chatID: model.targetId, isFrom: isFrom) { vc in
                TellEnd.share.broadcastResign(chatID: model.targetId, isFrom: isFrom) { vc in
...
extension MovementSkirt {
    ...
    func comprehensive(model: SpeedModel) {
        //: uploadRecord.uploadRecordEvent(eventID: ClickChatRoomNoP, toUid: model.targetId)
        k_playShadowValue.behindImplement(eventID: dataSpineExerciseValue, toUid: model.targetId)
        //: guard model.onlyOneRoomId.isEmptyString else {
        guard model.onlyOneRoomId.isEmptyString else {
            //: TellEnd.share.func__pushToGroupChat(groupId: model.onlyOneRoomId)
            TellEnd.share.generateerSingle(groupId: model.onlyOneRoomId)
            //: return
            return
        }
...
extension MovementSkirt {
    ...
    private func tempDifficult() {
        //: guard self.manager.isHalf else { return }
        guard self.manager.isHalf else { return }
        //: NotificationCenter.default.post(name: LIVE_HALF_VIEW_SHOW, object: nil, userInfo: nil)
        NotificationCenter.default.post(name: kObjectMessage, object: nil, userInfo: nil)
        //: self.view.frame = CGRect(x: 0, y: ScreenHeight-HalfViewTopMargin, width: ScreenWidth, height: ScreenHeight)
        self.view.frame = CGRect(x: 0, y: const_rebuildWithinId - notiGiftEnterData, width: dataTabUrl, height: const_rebuildWithinId)
        ...
        UIView.animate(withDuration: 0.25) {
            ...
            self.view.frame = CGRect(x: 0,
                                     ...
                                     width: dataTabUrl,
                                     //: height: ScreenHeight)
                                     height: const_rebuildWithinId)
        }
    }

    /// 半屏页消失动画
    //: private func half_dismissViewAnimation() {
    private func towardBuild() {
        //: guard self.manager.isHalf else { return }
        guard self.manager.isHalf else { return }
        //: UIView.animate(withDuration: 0.25) {
        UIView.animate(withDuration: 0.25) {
            ...
            self.view.frame = CGRect(x: 0,
                                     ...
                                     y: const_rebuildWithinId - notiGiftEnterData,
                                     //: width: ScreenWidth,
                                     width: dataTabUrl,
                                     //: height: ScreenHeight)
                                     height: const_rebuildWithinId)
...
extension MovementSkirt {
    ...
    private func painterRefresh() {
        //: if self.manager.isHalf == false {
        if self.manager.isHalf == false { // 全屏
            //: if QuantityeractionDelay.share.appStatus == AppSkinStatus.special.rawValue {
            if QuantityeractionDelay.share.appStatus == TotalervalObserve.special.rawValue { // 审核模式
            ...
            removeCurrVBtn.snp.makeConstraints { make in
                ...
                make.height.equalTo(notiGiftEnterData)
            }
            ...
            contentView.asAffect(width: contentView.width,
