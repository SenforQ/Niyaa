...
/*: "%@ reply %@：%@" :*/
fileprivate let mainJumpStr:String = "%@ rpro rid video"
fileprivate let app_putMsg:[Character] = ["%","@","：","%","@"]
...
extension SecondModel {
    ...
    mutating func tarAttention() {
        //: var tempContent = ""
        var tempContent = ""
        //: if replyUser.count>0 {
        if replyUser.count > 0 {
            //: tempContent = String(format: "%@ reply %@：%@", nickname, replyUser, content)
            tempContent = String(format: (String(mainJumpStr.prefix(4)) + "eply " + String(app_putMsg)), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(MessageStage.sort(text: tempContent, size: CGSize(width: dataTabUrl - 82, height: CGFloat(MAXFLOAT)), font: UIFont.alongsideOk(type: .Regular, fontSize: 14), lineNumber: 0).height)
