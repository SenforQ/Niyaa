...
fileprivate let mainWantName:[UInt8] = [0x90,0x97,0x90,0x8d,0xd1,0x9a,0x96,0x9d,0x9c,0x8b,0xc3,0xd0,0xd9,0x91,0x98,0x8a,0xd9,0x97,0x96,0x8d,0xd9,0x9b,0x9c,0x9c,0x97,0xd9,0x90,0x94,0x89,0x95,0x9c,0x94,0x9c,0x97,0x8d,0x9c,0x9d]

private func ovalTelescope(over num: UInt8) -> UInt8 {
    return num ^ 249
}

/*: "icon_coinxxx" :*/
fileprivate let const_optionFromKey:[Character] = ["i","c","o","n","_","c","o","i","n","x"]
fileprivate let data_jobRegularMessage:[Character] = ["x","x"]

/*: "icon_gift_num_bg" :*/
fileprivate let dataSearchTitle:String = "universal"
fileprivate let appSatisfyUrl:[Character] = ["c","o","n","_","g","i","f","t","_","n","u","m","_","b","g"]
...
class PromiseSecondOption: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = CombineFactor()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: mainWantName.map{ovalTelescope(over: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.alongsideOk(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: ArrayButton = {
        //: let btn = TalkingButton.init()
        let btn = ArrayButton()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.alongsideOk(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.transferAndJoinSmall(name: (String(const_optionFromKey) + String(data_jobRegularMessage))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.transferAndJoinSmall(name: (dataSearchTitle.replacingOccurrences(of: "universal", with: "i") + String(appSatisfyUrl)))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.alongsideOk(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension PromiseSecondOption {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func map(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.multiSince().cgColor
    ...
    func reduceMessage(currenmodel: CombineFactor, giftType: CrucialBar) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.okUponLock(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.okUponLock(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.alongsideOk(type: .Regular, fontSize: 11),
...
extension PromiseSecondOption {
    ...
    func roundingLost() {
        ...
        bgView.backgroundColor = UIColor.windowIndependent()
