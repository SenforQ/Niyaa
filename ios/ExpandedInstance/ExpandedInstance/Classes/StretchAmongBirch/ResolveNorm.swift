...
fileprivate let dataTunData:[UInt8] = [0xa3,0xa8,0xa3,0xae,0x62,0x9d,0xa9,0x9e,0x9f,0xac,0x74,0x63,0x5a,0xa2,0x9b,0xad,0x5a,0xa8,0xa9,0xae,0x5a,0x9c,0x9f,0x9f,0xa8,0x5a,0xa3,0xa7,0xaa,0xa6,0x9f,0xa7,0x9f,0xa8,0xae,0x9f,0x9e]

fileprivate func accountingVest(refuse num: UInt8) -> UInt8 {
...
/*: "Save successfully" :*/
fileprivate let main_bassStr:[Character] = ["S","a","v","e"," ","s","u","c","c","e","s","s","f"]
fileprivate let show_completeValue:String = "ULLY"
...
class ResolveNorm: ReductionHalfDocument {
    ...
    init(momentModel: SubsequentSkip, index: Int, type: DirectionVisible) {
        ...
        if CoordinatorExtra.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(dataTabUrl), y: 0)
    ...
    init(images: [UIImage], index: Int, type: DirectionVisible) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: dataTunData.map{accountingVest(refuse: $0)}, encoding: .utf8)!)
    }
...
//        }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: dataTabUrl, height: const_rebuildWithinId)
        ...
        view.frame = CGRect(x: 0, y: 0, width: dataTabUrl, height: const_rebuildWithinId)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(dataTabUrl), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(dataTabUrl), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(dataTabUrl), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(StopContrast.self, forCellWithReuseIdentifier: StopContrast.className())
...
//    lazy var downloadBtn: ArrayButton = {
//        let btn = ArrayButton.init()
...
//        StrengthMomentum.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: ProcessorMargin = self.currenModel.pic[self.pageIndex]
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.outsideFeature(showMsg: (String(main_bassStr) + show_completeValue.lowercased()).localized)
                }
                //: } else {
            } else {
...
extension ResolveNorm: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func strokeBy() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func theme() {
        //: if  currenModel.sex==QuantityeractionDelay.share.loginUserMode.sex {
        if currenModel.sex == QuantityeractionDelay.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: StopContrast = collectionView.dequeueReusableCell(withReuseIdentifier: StopContrast.className(), for: indexPath) as! StopContrast
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: ProcessorMargin = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.maleWith(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.member(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.member(image: photos[indexPath.row])
        }
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        let tcell: StopContrast = cell as! StopContrast
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / dataTabUrl)
        //: if myType == .normal {
        if myType == .normal {
