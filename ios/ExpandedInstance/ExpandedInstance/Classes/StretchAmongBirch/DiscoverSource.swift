...
fileprivate let user_biologyUrl:[UInt8] = [0xb0,0xb7,0xb0,0xad,0xf1,0xba,0xb6,0xbd,0xbc,0xab,0xe3,0xf0,0xf9,0xb1,0xb8,0xaa,0xf9,0xb7,0xb6,0xad,0xf9,0xbb,0xbc,0xbc,0xb7,0xf9,0xb0,0xb4,0xa9,0xb5,0xbc,0xb4,0xbc,0xb7,0xad,0xbc,0xbd]

private func restrictionTypicalShade(educator num: UInt8) -> UInt8 {
    return num ^ 217
}

/*: "More" :*/
fileprivate let main_characteristicValue:String = "click might shall wed editMore"

/*: "icon_details_arrow" :*/
fileprivate let userYesterdayMsg:String = "ickind"
fileprivate let show_reflectMoreMsg:[Character] = ["s","_","a","r","r","o","w"]

/*: "Post" :*/
fileprivate let dataMysteryCurriculumString:String = "operate masterPost"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let const_hiddenKey:String = "ting zone distribution analyze measurebtn_"
fileprivate let appRenderTitle:String = "crucial photo closed verb chaptermic_s"
fileprivate let show_smallFormat:String = "split"
...
class DiscoverSource: UITableViewCell {
    //: var userId: String = ""
    var userId: String = ""
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: user_biologyUrl.map{restrictionTypicalShade(educator: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLab: UILabel = {
        ...
        lb.font = .alongsideOk(type: .Medium, fontSize: 17)
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .robotBy(0, 0, 0, 0.4)
    ...
    lazy var moreButton: ArrayButton = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .alongsideOk(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(main_characteristicValue.suffix(4))).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (userYesterdayMsg.replacingOccurrences(of: "kind", with: "on") + "_detail" + String(show_reflectMoreMsg))), for: .normal)
    ...
    lazy var pushBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(factoryEx), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension DiscoverSource {
    ...
    func monsterNear(userModel: PutPath) {
        ...
        strA = [(String(dataMysteryCurriculumString.suffix(4))).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.thinOf(), UIColor.multi()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.wantStream(fontSize: 18), UIFont.wantStream(fontSize: 16)]
        ...
        attributedString = attributedString.readingThumb(texts: strA, colors: colorA, fonts: fontA)
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: ImageTail = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.embraceByBold())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    multiple(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.embraceByBold())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    multiple(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.embraceByBold())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    multiple(btn: thirdImgView)
                }
            }
        }
    }
    ...
    func multiple(btn: UIImageView) {
        ...
        let imag = UIImageView(image: UIImage(named: (String(const_hiddenKey.suffix(4)) + "dyna" + String(appRenderTitle.suffix(5)) + "top_no" + show_smallFormat.replacingOccurrences(of: "split", with: "r"))))
...
extension DiscoverSource {
    ...
    private func accessCounterrupt() {
    ...
    private func replacement() {
        ...
        let height = (dataTabUrl - actualWidth(w: 15 * 4)) / 3.0
