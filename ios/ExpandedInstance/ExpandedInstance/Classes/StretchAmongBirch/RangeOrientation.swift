...
fileprivate let notiDrawTitle:[UInt8] = [0xb0,0xb7,0xb0,0xad,0xf1,0xba,0xb6,0xbd,0xbc,0xab,0xe3,0xf0,0xf9,0xb1,0xb8,0xaa,0xf9,0xb7,0xb6,0xad,0xf9,0xbb,0xbc,0xbc,0xb7,0xf9,0xb0,0xb4,0xa9,0xb5,0xbc,0xb4,0xbc,0xb7,0xad,0xbc,0xbd]

private func secretOverEnthusiasm(agree num: UInt8) -> UInt8 {
    return num ^ 217
}

/*: "move" :*/
fileprivate let noti_bassUrl:[Character] = ["m","o","v","e"]
...
class RangeOrientation: UIView {
    ...
    var danumuMsgArray: [DisabledModel] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [OptionView] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<DisabledModel?> = PublishSubject()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiDrawTitle.map{secretOverEnthusiasm(agree: $0)}, encoding: .utf8)!)
    }
    ...
    func conversationBy(_ danumuMsgModel: DisabledModel?) {
    ...
    @objc func tabVersion(_ tapGesture: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(tabVersion(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension RangeOrientation: UIGestureRecognizerDelegate {
    ...
    func offPublisher(for danumuMsgModel: DisabledModel?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = OptionView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (String(noti_bassUrl)))
    ...
    func hiSpecify(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                offPublisher(for: model, isFirst: isFirst)
...
extension RangeOrientation {
    // 添加视图
    //: private func setupSubviews() {
    private func selecterBig() {
