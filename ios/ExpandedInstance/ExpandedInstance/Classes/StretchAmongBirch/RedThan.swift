...
fileprivate let kOfPath:[UInt8] = [0xab,0xb0,0xab,0xb6,0x6a,0xa5,0xb1,0xa6,0xa7,0xb4,0x7c,0x6b,0x62,0xaa,0xa3,0xb5,0x62,0xb0,0xb1,0xb6,0x62,0xa4,0xa7,0xa7,0xb0,0x62,0xab,0xaf,0xb2,0xae,0xa7,0xaf,0xa7,0xb0,0xb6,0xa7,0xa6]

fileprivate func failRid(support num: UInt8) -> UInt8 {
...
/*: "FFD855" :*/
fileprivate let dataMechanismStr:String = "recoverrecoverD855"
...
class RedThan: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - showSmartValue
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: showSmartValue,
                ...
                self.erasePending(startX: self.videoStartX)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: kOfPath.map{failRid(support: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (dataMechanismStr.replacingOccurrences(of: "recover", with: "F")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: user_whiteId, y: 0, width: showSmartValue, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - showSmartValue, y: 0, width: showSmartValue, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.robotBy(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: user_whiteId, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.robotBy(0, 0, 0, 0.6)
...
extension RedThan {
    ...
    func actionMenu() {
        ...
        self.leftSilder.frame = CGRect(x: user_whiteId, y: 0, width: showSmartValue, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - showSmartValue, y: 0, width: showSmartValue, height: self.frame.size.height)
        //: resetIndicator()
        significantSession()
    }

    //: func needShowIndicator(isShow: Bool) {
    func efficiencySchedule(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func erasePending(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - showSmartValue,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - showSmartValue)
    }
}
...
extension RedThan {
    ...
    @objc func faultByWork(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.erasePending(startX: videoStartX)
    ...
    @objc func anPartner(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension RedThan {
    ...
    private func outWarning() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(faultByWork(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(anPartner(gesture:)))
    ...
    private func significantSession() {
    ...
    func duringProcessor(_ xposition: CGFloat) {
