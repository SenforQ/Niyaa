...
fileprivate let kMaxDecreaseData:[UInt8] = [0x4c,0x4b,0x4c,0x51,0xd,0x46,0x4a,0x41,0x40,0x57,0x1f,0xc,0x5,0x4d,0x44,0x56,0x5,0x4b,0x4a,0x51,0x5,0x47,0x40,0x40,0x4b,0x5,0x4c,0x48,0x55,0x49,0x40,0x48,0x40,0x4b,0x51,0x40,0x41]

private func combinedBass(response num: UInt8) -> UInt8 {
    return num ^ 37
}

/*: "nav_back_black_nor" :*/
fileprivate let kRegionMsg:String = "nav_bpure hide prior player"
fileprivate let show_acceptKey:String = "ease provision harassmentlack_nor"

/*: "btn_detail_more" :*/
fileprivate let const_musicData:String = "btn_desupporter related image"
fileprivate let showConversationTitle:String = "tbody"
fileprivate let mainBuildLowerMsg:[Character] = ["i","l","_","m","o","r","e"]

/*: "btn_chat_videocall" :*/
fileprivate let k_componentStr:String = "rank collectbtn_ch"
fileprivate let dataCompleteString:String = "rounddeoca"
fileprivate let show_deliveryName:String = "behaviorbehavior"

/*: "icon_lounge" :*/
fileprivate let data_enjoyMsg:String = "fromo"
fileprivate let notiStackUrl:[Character] = ["n","_","l","o","u","n","g","e"]

/*: "Official" :*/
fileprivate let data_norStr:String = "consume true task him lackOffi"

/*: "Online List" :*/
fileprivate let const_normalToName:String = "consumption increase available visualOnline L"
fileprivate let user_thanMsg:String = "isgraph"

/*: "icon_privateChat_intimate" :*/
fileprivate let user_exclusivePauseUrl:[Character] = ["i","c"]
fileprivate let user_designName:[Character] = ["o","n","_","p","r","i","v","a","t","e","C","h","a","t","_","i","n","t","i","m","a","t","e"]

/*: "#FF60B9" :*/
fileprivate let show_visiblePeopleValue:[Character] = ["#","F","F","6","0"]
fileprivate let app_errorName:String = "Bdistinction"

/*: "Customer Care Center" :*/
fileprivate let dataTitleUrl:[Character] = ["C","u","s","t","o","m","e","r"]
fileprivate let noti_wouldEmbraceMessage:String = "char box spine auto Care"
fileprivate let kRavelId:[Character] = [" ","C","e","n","t","e","r"]

/*: "#48FF1E" :*/
fileprivate let k_themeCosKey:String = "comment"
fileprivate let noti_absMessage:String = "replacement continue don48FF1E"

/*: "Online" :*/
fileprivate let notiDecisionUrl:String = "Onlineown associated favorite"

/*: "#FF4B7F" :*/
fileprivate let k_airName:String = "#FF4B7Ffile exit measure"

/*: "busy" :*/
fileprivate let dataWithinStr:[Character] = ["b","u","s","y"]

/*: "get json error" :*/
fileprivate let k_toolGemHousingString:String = "get jslost press offer even"
fileprivate let k_takeInfectTitle:String = "lessor"

/*: "transform.rotation" :*/
fileprivate let data_recordCalledCustomKey:String = "tranretiredf"
fileprivate let data_networkUrl:String = "iopant"

/*: "transform.scale" :*/
fileprivate let notiDisplayYearId:[Character] = ["t","r","a","n","s","f","o","r","m",".","s","c","a","l","e"]

/*: "zoom&shake" :*/
fileprivate let noti_pureValue:[Character] = ["z","o","o","m","&"]
fileprivate let app_disappearKitKey:[Character] = ["s","h","a","k","e"]
...
class ClusterView: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: kMaxDecreaseData.map{combinedBass(response: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var backButton: UIButton = {
        ...
        let img = UIImage.transferAndJoinSmall(name: (String(kRegionMsg.prefix(5)) + "ack_b" + String(show_acceptKey.suffix(8)))).withRenderingMode(.alwaysTemplate)
    ...
    lazy var moreButton: UIButton = {
        ...
        let img = UIImage.transferAndJoinSmall(name: (String(const_musicData.prefix(6)) + showConversationTitle.replacingOccurrences(of: "body", with: "a") + String(mainBuildLowerMsg))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
    ...
    lazy var videoCallBtn: UIButton = {
        ...
        let img = UIImage.transferAndJoinSmall(name: (String(k_componentStr.suffix(6)) + "at_v" + dataCompleteString.replacingOccurrences(of: "round", with: "i") + show_deliveryName.replacingOccurrences(of: "behavior", with: "l"))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
    ...
    lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.transferAndJoinSmall(name: (data_enjoyMsg.replacingOccurrences(of: "from", with: "ic") + String(notiStackUrl)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(data_norStr.suffix(4)) + "cial").localized, for: .normal)
    ...
    lazy var onlineListBtn: UIButton = {
        ...
        btn.setTitle((String(const_normalToName.suffix(8)) + user_thanMsg.replacingOccurrences(of: "graph", with: "t")).localized, for: .normal)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var onlineStatusV: ArrayButton = {
    ...
    private lazy var myBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(anChain), for: .touchUpInside)
    ...
    private lazy var userBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(ownerTo), for: .touchUpInside)
        //: btn.imageView?.contentMode = .scaleAspectFill
        btn.imageView?.contentMode = .scaleAspectFill
        //: btn.isHidden = true
        btn.isHidden = true
    ...
    private lazy var intimateBtn: UIButton = {
        ...
        btn.setImage(UIImage.transferAndJoinSmall(name: (String(user_exclusivePauseUrl) + String(user_designName))), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(resourceBoard), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var intimateLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (String(show_visiblePeopleValue) + app_errorName.replacingOccurrences(of: "distinction", with: "9")))
    ...
    private lazy var addIntimateLB: UILabel = {
        ...
        lb.font = UIFont.alongsideOk(type: .Semibold, fontSize: 12)
        //: lb.isHidden = true
        lb.isHidden = true
...
extension ClusterView {
    //: func createSubViews() {
    func feedbackSustainable() {
        ...
        if self.chatType == .service {
            ...
            titleLabel.text = (String(dataTitleUrl) + String(noti_wouldEmbraceMessage.suffix(5)) + String(kRavelId)).localized
            //: titleLabel.font = .pingfangFont(type: .Medium, fontSize: 18)
            titleLabel.font = .alongsideOk(type: .Medium, fontSize: 18)
            //: titleLabel.textColor = .appTitleColor()
            titleLabel.textColor = .thinOf()
    ...
    func deploy() {
        ...
        if self.chatType == .service { // 官方客服
            ...
            let offset = CoordinatorExtra.shared.direction == .leftToRight ? 22 : -22
    ...
    ///   - userModel: 用户信息
    ///   - isFrom: 列表控制器入口
    //: func private_updateNavView(userModel: BetweenInput, isFrom: PrivateChatIsFromEnum) {
    func appropriate(userModel: BetweenInput, isFrom: TailProvider) {
        ...
        if userModel.onlineStatus == 1 {
            ...
            let image = UIImage.alwaysAcrossGreen(color: UIColor(hex: (k_themeCosKey.replacingOccurrences(of: "comment", with: "#") + String(noti_absMessage.suffix(6))))!, size: CGSize(width: 8, height: 8))
            //: onlineStatusV.setImage(image.itemHardProject(), for: .normal)
            onlineStatusV.setImage(image.itemHardProject(), for: .normal)
            //: onlineStatusV.setTitle("Online".localized, for: .normal)
            onlineStatusV.setTitle((String(notiDecisionUrl.prefix(6))).localized, for: .normal)
            ...
            let image = UIImage.alwaysAcrossGreen(color: UIColor(hex: (String(k_airName.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineStatusV.setImage(image.itemHardProject(), for: .normal)
            onlineStatusV.setImage(image.itemHardProject(), for: .normal)
            //: onlineStatusV.setTitle("busy".localized, for: .normal)
            onlineStatusV.setTitle((String(dataWithinStr)).localized, for: .normal)
            //: } else {
        } else {
        ...
        titleLabel.textColor = userModel.loungePlus ? .outVideo() : .white
        ...
        var videoCall = userModel.enableVideoCall
        //: if isFrom == .LiveRoom || QuantityeractionDelay.share.appStatus == AppSkinStatus.special.rawValue {
        if isFrom == .LiveRoom || QuantityeractionDelay.share.appStatus == TotalervalObserve.special.rawValue {
            //: videoCall = false
            videoCall = false
        }
        //: if videoCall {
    ...
    /// - Parameters:
    ///   - userModel: 用户信息
    //: func private_updateIntimateNavView(userModel: BetweenInput) {
    func content(userModel: BetweenInput) {
        ...
        userBtn.isHidden = false
        //: myBtn.setUrlImage(urlStr: QuantityeractionDelay.share.loginUserMode.headPic ?? "")
        myBtn.totalegrationAdmin(urlStr: QuantityeractionDelay.share.loginUserMode.headPic ?? "")
        //: userBtn.setUrlImage(urlStr: userModel.headPic)
        userBtn.totalegrationAdmin(urlStr: userModel.headPic)
    ...
    func curveFactory(intimate: Int) {
        //: let str  = "+\(intimate)"
        let str = "+\(intimate)"
        ...
        let attributes = [.foregroundColor: UIColor.white, .font: UIFont.alongsideOk(type: .Semibold, fontSize: 12)] as [NSAttributedString.Key: Any]
        ...
        let url = PrepareSpecify.default.accessibleUp(type: .Private_Chat_intimate)
        //: do {
        do {
            ...
            printLog(message: (String(k_toolGemHousingString.prefix(6)) + "on e" + k_takeInfectTitle.replacingOccurrences(of: "less", with: "rr")))
        }
    }
    ...
    func overIconBuilder(intimate: Int) {
    ...
    @objc func anChain() {
        //: TellEnd.share.func__pushToUserDetailVC(uid: QuantityeractionDelay.share.loginUserMode.userID)
        TellEnd.share.butterfly(uid: QuantityeractionDelay.share.loginUserMode.userID)
    }

    //: @objc func userIconClick() {
    @objc func ownerTo() {
        //: TellEnd.share.func__pushToUserDetailVC(uid: userId)
        TellEnd.share.butterfly(uid: userId)
    }

    //: @objc func intimateBtnClick() {
    @objc func resourceBoard() {
        //: let view = TalkingIntimateInstructionsPopUpView.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight))
        let view = PlatformSendView(frame: CGRect(x: 0, y: 0, width: dataTabUrl, height: const_rebuildWithinId))
        //: view.show()
        view.overFast()
    }
    ...
    func constant() {
        ...
        let roate = CAKeyframeAnimation(keyPath: (data_recordCalledCustomKey.replacingOccurrences(of: "retired", with: "s") + "orm.rotat" + data_networkUrl.replacingOccurrences(of: "pant", with: "n")))
        ...
        let zoom = CAKeyframeAnimation(keyPath: (String(notiDisplayYearId)))
        ...
        videoCallBtn.layer.add(groupAnnimation, forKey: (String(noti_pureValue) + String(app_disappearKitKey)))
    }
}
