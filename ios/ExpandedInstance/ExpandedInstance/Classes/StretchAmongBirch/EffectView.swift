...
fileprivate let constThenMessage:[UInt8] = [0x28,0x2f,0x28,0x35,0x69,0x22,0x2e,0x25,0x24,0x33,0x7b,0x68,0x61,0x29,0x20,0x32,0x61,0x2f,0x2e,0x35,0x61,0x23,0x24,0x24,0x2f,0x61,0x28,0x2c,0x31,0x2d,0x24,0x2c,0x24,0x2f,0x35,0x24,0x25]

private func becomeAdvice(coordinator num: UInt8) -> UInt8 {
    return num ^ 65
}

/*: "bth_live_follow_pre" :*/
fileprivate let const_helpPadPath:String = "action pool distinctionbth_"
fileprivate let user_ridByValue:[Character] = ["_","f","o","l","l"]
fileprivate let data_weeklyValue:String = "age domain interactionow_pre"

/*: "btn_live_mini_nor" :*/
fileprivate let kMultiValue:[Character] = ["b","t","n","_","l","i","v","e","_","m","i","n","i","_","n","o","r"]

/*: "#2DF2FF" :*/
fileprivate let main_libraryMessage:String = "editor along disk in#2DF2FF"

/*: "Live" :*/
fileprivate let kAroundRestorelyTitle:String = "orientation foundationLive"

/*: "removeAttentionUid" :*/
fileprivate let constCyclePath:String = "remtrustve"
fileprivate let constAirPath:String = "my conntionUid"

/*: "attentionUid" :*/
fileprivate let data_againString:[Character] = ["a"]
fileprivate let mainDelicatePortraitString:String = "ttlicense"

/*: "source" :*/
fileprivate let userCurrentlyBeginDistinctiveFormat:String = "blind"
fileprivate let data_yourName:[Character] = ["o","u","r","c","e"]

/*: "num" :*/
fileprivate let mainChallengeKey:[Character] = ["n","u","m"]

/*: "UID: :*/
fileprivate let notiLatBetweenName:String = "what newsUID:"

/*: "btn_live_closed_nor" :*/
fileprivate let kFillPath:String = "BT"
fileprivate let showExistUrl:String = "n_livevisual interval"
fileprivate let kInstallStr:String = "identify from problemed_nor"
...
class EffectView: UIView {
    ...
    weak var weakVC: ScaleViewController?
    // 右侧按钮点击Block
    ...
    private var liveModel = FloorMaterial()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constThenMessage.map{becomeAdvice(coordinator: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(orangeResource), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nickNameLab: UILabel = {
        ...
        lab.font = UIFont.wantStream(fontSize: 11)
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.transferAndJoinSmall(name: (String(const_helpPadPath.suffix(4)) + "live" + String(user_ridByValue) + String(data_weeklyValue.suffix(6)))), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(errorRestore), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.transferAndJoinSmall(name: (String(kMultiValue))), for: .normal)
        ...
        //: btn.isHidden = (QuantityeractionDelay.share.appStatus == AppSkinStatus.special.rawValue && QuantityeractionDelay.share.loginUserMode.sex == Gender.female.rawValue)
        btn.isHidden = (QuantityeractionDelay.share.appStatus == TotalervalObserve.special.rawValue && QuantityeractionDelay.share.loginUserMode.sex == CommonAmong.female.rawValue)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var liveStatusImgV: ArrayButton = {
        ...
        let image = UIImage.alwaysAcrossGreen(color: UIColor(hex: (String(main_libraryMessage.suffix(7))))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.itemHardProject(), for: .normal)
        img.setImage(image.itemHardProject(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(kAroundRestorelyTitle.suffix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.graphChain(color: UIColor.black.withAlphaComponent(0.25), forState: .normal)
...
extension EffectView {
    ...
    @objc func progressWith(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(constCyclePath.replacingOccurrences(of: "trust", with: "o") + "Atte" + String(constAirPath.suffix(8)))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(data_againString) + mainDelicatePortraitString.replacingOccurrences(of: "license", with: "en") + "tionUid")] = uid
            //: dict["source"] = "1"
            dict[(userCurrentlyBeginDistinctiveFormat.replacingOccurrences(of: "blind", with: "s") + String(data_yourName))] = "1"
        }
        //: TalkingChatRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
        AgainstListen.ownerGreen(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
            //: completion?(succeed)
            completion?(succeed)
        }
    }
...
extension EffectView {
    ...
    @objc private func city() {
        ...
        self.onlineListView.compute(chatGroupId: liveModel.chatGroupId)
    }

    /// 获取
    //: @objc private func getOnlineNumData() {
    @objc private func nowScientific() {
        //: TalkingAudienceManager().req_liveRoomOnlineNum(chatGroupId: liveModel.chatGroupId, completion:{ succeed, result, errorModel in
        SkipOriginal().visibleCrucial(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, errorModel in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: let num = json["num"].string
            let num = json[(String(mainChallengeKey))].string
    ...
    private func toolLog() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(nowScientific), userInfo: nil, repeats: true)
    }
...
extension EffectView {
    ...
    func duringStrip(_ liveModel: FloorMaterial) {
        ...
        uidLab.text = (String(notiLatBetweenName.suffix(4))) + "\(liveModel.streamerInfo.uid)"
        ...
        icon.viaCluster(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.fieldIn(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(liveModel.streamerInfo.uid) == QuantityeractionDelay.share.loginUid)
        let isAnchor = (String(liveModel.streamerInfo.uid) == QuantityeractionDelay.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: } else {
            ...
            miniBtn.setImage(UIImage.transferAndJoinSmall(name: (kFillPath.lowercased() + String(showExistUrl.prefix(6)) + "_clos" + String(kInstallStr.suffix(6)))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            custom(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
    ...
    private func custom(isAttention: Bool) {
    ...
    @objc private func errorRestore() {
        //: req_followRequest(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
        progressWith(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.custom(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension EffectView {
    ...
    private func towardIndex() {
        //: self.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 68))
        self.frame = CGRect(x: 0, y: 0, width: dataTabUrl, height: dataDoingRapidMsg + actualWidth(w: 68))
