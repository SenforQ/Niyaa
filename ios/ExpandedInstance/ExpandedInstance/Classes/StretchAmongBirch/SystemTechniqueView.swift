...
/*: "Select a date" :*/
fileprivate let appProperlyKey:String = "manager want alive drawing sSelect "
fileprivate let noti_minimizeKey:String = "a dateused agree"
...
protocol ClearCeiling: NSObjectProtocol {
...
class SystemTechniqueView: HeadView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: ClearCeiling?

    //: override func initPickView() {
    override func colorSignature() {
        //: super.initPickView()
        super.colorSignature()

        //: titleString = "Select a date".localized
        titleString = (String(appProperlyKey.suffix(7)) + String(noti_minimizeKey.prefix(6))).localized
        ...
        reliabilityTranslate(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func reliabilityTranslate(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func activityEvaluation(button: UIButton) {
        //: super.confirmClick(button: button)
        super.activityEvaluation(button: button)
        //: refreshPickViewData()
        anyMerge()
        ...
        if let day = selectDay {
            ...
            delegate?.proClear(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.proClear(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return tillSort(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func tillSort(year: Int, month: Int) -> Int {
