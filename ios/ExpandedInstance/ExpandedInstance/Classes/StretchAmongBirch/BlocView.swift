...
fileprivate let const_awakeAcceptString:[UInt8] = [0x90,0x97,0x90,0x8d,0xd1,0x9a,0x96,0x9d,0x9c,0x8b,0xc3,0xd0,0xd9,0x91,0x98,0x8a,0xd9,0x97,0x96,0x8d,0xd9,0x9b,0x9c,0x9c,0x97,0xd9,0x90,0x94,0x89,0x95,0x9c,0x94,0x9c,0x97,0x8d,0x9c,0x9d]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let k_trustPath:String = "player fresh programbtn_dy"
fileprivate let k_peopleDealSubtleMessage:[Character] = ["n","a","m","i","c","_","s","t","o","p","_","n","o","r"]

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let app_combineFormat:String = "security sumbtnwO"
fileprivate let main_normalLocalStr:String = "ta sustainableblEh_"
fileprivate let user_examineAmongFaceTitle:String = "channel front identify icon affectdiv_"

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let notiOurName:String = "btnCmjelement second typical"
fileprivate let userAdPath:String = "evaluation receivertdj_o"
fileprivate let kBadFormat:[Character] = ["e","d","i","v","_","p","l","a","y"]
...
class BlocView: UIView {
    //: open weak var delegate: InteractionViewDelegate?
    open weak var delegate: ButtChart?
    ...
    override init(frame: CGRect) {
        ...
        self.significantOfPermanent()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: const_awakeAcceptString.map{$0^249}, encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        ...
        btn.setImage(UIImage.transferAndJoinSmall(name: (String(k_trustPath.suffix(6)) + String(k_peopleDealSubtleMessage))), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(called(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        ...
        btn.setImage(UIImage.transferAndJoinSmall(name: (String(app_combineFormat.suffix(5)) + String(main_normalLocalStr.suffix(5)) + "tdj_oe" + String(user_examineAmongFaceTitle.suffix(4)) + "stop")), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.transferAndJoinSmall(name: (String(notiOurName.prefix(6)) + "oS7_" + String(userAdPath.suffix(5)) + String(kBadFormat))), for: .selected)
        //: btn.addTarget(self, action: #selector(playButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(bound(sender:)), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var failImageView: UIImageView = {
        ...
        imag.image = UIImage.transferAndJoinSmall(name: "")
        //: imag.isHidden = true
        imag.isHidden = true
    ...
    private lazy var slider: ToAccessible = {
        ...
        let slid = ToAccessible(frame: CGRect(x: 100, y: 0, width: dataTabUrl - 167, height: 40), sliderWidth: 12, sliderColor: UIColor.white, progressHeight: 2, progressBgColor: UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.3), progressPlayedColor: UIColor.white, progressCachedColor: UIColor.clear, isShowCorner: true)
...
extension BlocView {
    ...
    private func significantOfPermanent() {
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.everyNor(view: self, value: value)
        }
        ...
        self.slider.panEndHandler = { [weak self] value in
            ...
            self.operationTier(status: .StopSeek)
        }
    }
}
