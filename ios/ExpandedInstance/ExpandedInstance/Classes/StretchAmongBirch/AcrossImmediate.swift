...
fileprivate let userSucceedMsg:[UInt8] = [0x3e,0x39,0x3e,0x23,0x7f,0x34,0x38,0x33,0x32,0x25,0x6d,0x7e,0x77,0x3f,0x36,0x24,0x77,0x39,0x38,0x23,0x77,0x35,0x32,0x32,0x39,0x77,0x3e,0x3a,0x27,0x3b,0x32,0x3a,0x32,0x39,0x23,0x32,0x33]

private func dayFresh(tea num: UInt8) -> UInt8 {
    return num ^ 87
}

/*: "Online Users" :*/
fileprivate let noti_romanMessage:[Character] = ["O","n","l","i","n","e"]
fileprivate let mainEstimatedUrl:[Character] = [" ","U","s","e","r","s"]
...
class AcrossImmediate: UIView {
    //: private let contentV_H = (344+kDeviceSafeBottomHeight)
    private let contentV_H = (344 + constCountervalMaterialMsg)
    //: private var DataSource: [TalkingLiveRoomOnlineListModel] = []
    private var DataSource: [CrucialInstead] = []
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: userSucceedMsg.map{dayFresh(tea: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(allowComment), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.font = UIFont.wantStream(fontSize: 20)
        //: lb.textColor = UIColor.appTitleColor()
        lb.textColor = UIColor.thinOf()
        //: lb.textAlignment = .center
        lb.textAlignment = .center
        //: lb.text = "Online Users".localized
        lb.text = (String(noti_romanMessage) + String(mainEstimatedUrl)).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var tableView: UITableView = {
        ...
        table.register(AdvancedChain.self, forCellReuseIdentifier: AdvancedChain.className())
...
extension AcrossImmediate {
    ...
    private func spark() {
        ...
        SkipOriginal().stackScale(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
...
extension AcrossImmediate: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = AdvancedChain.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingLiveRoomOnlineListCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? AdvancedChain
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingLiveRoomOnlineListCell(style: .default, reuseIdentifier: identifier)
            cell = AdvancedChain(style: .default, reuseIdentifier: identifier)
        }
        //: cell?.setCell(model: self.DataSource[indexPath.row], index: indexPath.row)
        cell?.agreeAfter(model: self.DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
...
extension AcrossImmediate {
    ...
    @objc private func allowComment() {
        //: dismiss()
        validEffect()
    }
    ...
    func compute(chatGroupId: String) {
        ...
        self.contentView.frame = CGRect(x: 0, y: const_rebuildWithinId, width: dataTabUrl, height: contentV_H)
        //: reqData()
        spark()
...
extension AcrossImmediate {
    ...
    private func instance() {
        ...
        let style = ScenarioAbout()
        //: emptyView = EmptyView.init(frame: CGRect(x: 0, y: -100, width: ScreenWidth, height: contentV_H+100), style: style)
        emptyView = EmptyView(frame: CGRect(x: 0, y: -100, width: dataTabUrl, height: contentV_H + 100), style: style)
