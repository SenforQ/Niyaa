...
fileprivate let k_receiveNumbMessage:[UInt8] = [0xc4,0xc3,0xc4,0xd9,0x85,0xce,0xc2,0xc9,0xc8,0xdf,0x97,0x84,0x8d,0xc5,0xcc,0xde,0x8d,0xc3,0xc2,0xd9,0x8d,0xcf,0xc8,0xc8,0xc3,0x8d,0xc4,0xc0,0xdd,0xc1,0xc8,0xc0,0xc8,0xc3,0xd9,0xc8,0xc9]

private func buttComposition(salt num: UInt8) -> UInt8 {
    return num ^ 173
}

/*: "bg_medals_podium" :*/
fileprivate let user_stackYaPath:[Character] = ["b","g","_","m","e","d","a","l","s","_"]
fileprivate let k_screenSinceMsg:[Character] = ["p","o","d","i","u","m"]

/*: "icon_rank_first_bg" :*/
fileprivate let noti_pantStr:[Character] = ["i","c","o","n","_","r","a","n"]
fileprivate let data_needString:[Character] = ["k","_","f","i","r"]
fileprivate let data_maxKindValue:String = "natural ruling instead assistst_bg"

/*: "icon_rank_first" :*/
fileprivate let kNativeBarMsg:String = "span lower without she alreadyicon_ra"
fileprivate let const_writeKey:String = "nk_firstdepth auto associated line"

/*: "icon_ammount_points" :*/
fileprivate let user_insertUrl:String = "content daily excited panticon_"
fileprivate let const_onMsg:[Character] = ["p","o","i","n","t","s"]

/*: "icon_lounge" :*/
fileprivate let userPropertyId:[Character] = ["i","c","o","n","_"]
fileprivate let show_analysisString:[Character] = ["l","o","u","n","g","e"]

/*: "icon_rank_coin" :*/
fileprivate let noti_balanceUrl:[Character] = ["i","c","o","n","_","r","a","n","k"]
fileprivate let appControlUrl:[Character] = ["_","c","o","i","n"]

/*: "icon_rank_coin2" :*/
fileprivate let noti_blockResStr:String = "sign dynamics pro easeicon_r"
fileprivate let dataRectoMessage:String = "oinmenu"

/*: "icon_rank_second" :*/
fileprivate let show_ownSpaceLabPath:[Character] = ["i","c","o","n","_","r","a","n","k"]
fileprivate let const_constraintMsg:String = "_secondmultiple guard normal"

/*: "icon_rank_third" :*/
fileprivate let show_problemUrl:String = "closed"
fileprivate let constCityValue:[Character] = ["c","o","n","_","r","a","n","k","_","t","h","i","r","d"]
...
class FieldView: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: k_receiveNumbMessage.map{buttComposition(salt: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.transferAndJoinSmall(name: (String(user_stackYaPath) + String(k_screenSinceMsg)))
    }

    //: lazy var firstView = SocialRankHeaderItemView.init(frame: .zero, rankType: .firstRankType).then {
    lazy var firstView = BlockPathImplementation(frame: .zero, rankType: .firstRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 0
        $0.tag = 0
        //: let tap = UITapGestureRecognizer.init(target: self, action: #selector(registTapAction))
        let tap = UITapGestureRecognizer(target: self, action: #selector(infrastructureTitle))
        //: $0.addGestureRecognizer(tap)
        $0.addGestureRecognizer(tap)
    }

    //: lazy var secondView = SocialRankHeaderItemView.init(frame: .zero, rankType: .secondRankType).then {
    lazy var secondView = BlockPathImplementation(frame: .zero, rankType: .secondRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 1
        $0.tag = 1
    ...
    lazy var thirdView = BlockPathImplementation(frame: .zero, rankType: .thirdRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 2
        $0.tag = 2
...
extension FieldView {
    ...
    private func afterEffect() {
        //: let firstWidth = (ScreenWidth - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        let firstWidth = (dataTabUrl - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (dataTabUrl - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
// MARK: - BlockPathImplementation

//: class SocialRankHeaderItemView: UIView {
class BlockPathImplementation: UIView {
    //: enum SRankHeaderItemType: Int {
    enum ImpressionFirst: Int {
        //: case firstRankType
        case firstRankType
        //: case secondRankType
        case secondRankType
    ...
    public init(frame: CGRect, rankType: ImpressionFirst) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.vest(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.transferAndJoinSmall(name: (String(noti_pantStr) + String(data_needString) + String(data_maxKindValue.suffix(5))))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.transferAndJoinSmall(name: (String(kNativeBarMsg.suffix(7)) + String(const_writeKey.prefix(8))))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .alongsideOk(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = ArrayButton().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.transferAndJoinSmall(name: (String(user_insertUrl.suffix(5)) + "ammount_" + String(const_onMsg))), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .alongsideOk(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.transferAndJoinSmall(name: (String(userPropertyId) + String(show_analysisString)))
    }
}
...
extension BlockPathImplementation {
    //: func updateItemWithModel(model: SocialRankItemModel?, type: Int) {
    func tillSignificant(model: TableModel?, type: Int) {
        ...
        headPic.viaCluster(urlStr: model!.headPic, placeImg: UIImage.fieldIn(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.transferAndJoinSmall(name: (String(noti_balanceUrl) + String(appControlUrl))), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.transferAndJoinSmall(name: (String(noti_blockResStr.suffix(6)) + "ank_c" + dataRectoMessage.replacingOccurrences(of: "menu", with: "2"))), for: .normal)
        }
    }
}
...
extension BlockPathImplementation {
    ...
    func vest(type: ImpressionFirst) {
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.transferAndJoinSmall(name: (String(kNativeBarMsg.suffix(7)) + String(const_writeKey.prefix(8))))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.robotBy(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.transferAndJoinSmall(name: (String(show_ownSpaceLabPath) + String(const_constraintMsg.prefix(7))))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.robotBy(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.transferAndJoinSmall(name: (show_problemUrl.replacingOccurrences(of: "closed", with: "i") + String(constCityValue)))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.robotBy(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
