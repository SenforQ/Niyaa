...
fileprivate let mainTransformData:[UInt8] = [0xa3,0xa8,0xa3,0xae,0x62,0x9d,0xa9,0x9e,0x9f,0xac,0x74,0x63,0x5a,0xa2,0x9b,0xad,0x5a,0xa8,0xa9,0xae,0x5a,0x9c,0x9f,0x9f,0xa8,0x5a,0xa3,0xa7,0xaa,0xa6,0x9f,0xa7,0x9f,0xa8,0xae,0x9f,0x9e]

fileprivate func sendSumro(rate num: UInt8) -> UInt8 {
...
/*: "icon_lounge" :*/
fileprivate let notiSupporterUrl:String = "icon_lblank future don"
fileprivate let showStandardData:[Character] = ["o","u","n","g","e"]

/*: "icon_talk_points" :*/
fileprivate let kCouldGivingData:String = "facecon"
fileprivate let constInsteadFormat:[Character] = ["n","t","s"]

/*: "C6BDFF" :*/
fileprivate let user_firstKey:String = "bottomBDFF"

/*: "#BBBBBB" :*/
fileprivate let noti_circleKey:String = "#BBBBBBcigar min"

/*: "Reply to get points~" :*/
fileprivate let mainYearYieldValue:[Character] = ["R","e","p","l","y"," ","t","o"]
fileprivate let app_departmentSaveId:[Character] = [" ","g","e","t"," ","p","o","i","n","t","s","~"]

/*: "bth_unread_nor" :*/
fileprivate let user_writeId:String = "today"
fileprivate let constCircleTitle:String = "lower unknown primary examineth_un"

/*: "icon_male_default" :*/
fileprivate let main_painterFormat:[UInt8] = [0x74,0x6c,0x75,0x61,0x66,0x65,0x64,0x5f,0x65,0x6c,0x61,0x6d,0x5f,0x6e,0x6f,0x63,0x69]

/*: "icon_female_default" :*/
fileprivate let constCameraFormat:String = "ICON"
fileprivate let showGiftData:String = "e_degravity highlight view threshold others"
fileprivate let show_sayFavoriteTitle:[Character] = ["f","a","u","l","t"]

/*: "+%@ points" :*/
fileprivate let dataSkinWeightTitle:[Character] = ["+","%","@"," ","p","o","i","n","t","s"]

/*: "bth_read_pre" :*/
fileprivate let constMightRidData:String = "bth_rbroadcast self initial scholar background"
...
//: class TalkingChatBaseMsgCell: CeilingThan {
class LocateMight: CeilingThan {
    //: var cellData: TalkingChatMsgBaseCellData?
    var cellData: LocalStrength?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainTransformData.map{sendSumro(rate: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.transferAndJoinSmall(name: (String(notiSupporterUrl.prefix(6)) + String(showStandardData)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var ageLab: UILabel = {
        ...
        label.font = UIFont.wantStream(fontSize: 10)
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var coinIconImg: UIImageView = {
        ...
        imgV.image = UIImage.transferAndJoinSmall(name: (kCouldGivingData.replacingOccurrences(of: "face", with: "i") + "_talk_poi" + String(constInsteadFormat)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    public lazy var coinLabel: UILabel = {
        ...
        label.textColor = UIColor(hex: (user_firstKey.replacingOccurrences(of: "bottom", with: "C6")))
        //: label.isHidden = true
        label.isHidden = true
    ...
    public lazy var replyTipLab: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(noti_circleKey.prefix(7))))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.botLocal(fontSize: 14)
        //: label.text = "Reply to get points~".localized
        label.text = (String(mainYearYieldValue) + String(app_departmentSaveId)).localized
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var isReadImg: UIImageView = {
        ...
        imgV.image = UIImage.transferAndJoinSmall(name: (user_writeId.replacingOccurrences(of: "today", with: "b") + String(constCircleTitle.suffix(5)) + "read_nor"))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension LocateMight {
    ...
    override func panel(with data: CleanShared) {
        ...
        self.cellData = data as? LocalStrength
        ...
        let userModel = newData.msgModel.user
        //: let loungePlus = (QuantityeractionDelay.share.loginUid == String(userModel.uid)) ? QuantityeractionDelay.share.loginUserMode.loungePlus : userModel.loungePlus
        let loungePlus = (QuantityeractionDelay.share.loginUid == String(userModel.uid)) ? QuantityeractionDelay.share.loginUserMode.loungePlus : userModel.loungePlus

        //: self.nameLabel.text = userModel.nickname
        self.nameLabel.text = userModel.nickname
        //: self.nameLabel.textColor = loungePlus ? .userVipColor() : .appValueColor()
        self.nameLabel.textColor = loungePlus ? .outVideo() : .multi()
        //: self.nameLabel.font = UIFont.pingfangMediumFont(fontSize: 13)
        self.nameLabel.font = UIFont.wantStream(fontSize: 13)
        //: let imgStr = userModel.sex == Int(Gender.male.rawValue) ? "icon_male_default" : "icon_female_default"
        let imgStr = userModel.sex == Int(CommonAmong.male.rawValue) ? String(bytes: main_painterFormat.reversed(), encoding: .utf8)! : (constCameraFormat.lowercased() + "_femal" + String(showGiftData.prefix(4)) + String(show_sayFavoriteTitle))
        //: sexIconBtn.setBackgroundImage(UIImage.BundleImageNamed(name: imgStr), for: .normal)
        sexIconBtn.setBackgroundImage(UIImage.transferAndJoinSmall(name: imgStr), for: .normal)
        ...
        self.avatarView.okUponLock(urlStr: userModel.headPic)
        ...
        self.iconBorder.alongWhoRecto(urlStr: userModel.headPicFrame)

        //: let isShowCoin = newData.msgIncome > 0
        let isShowCoin = newData.msgIncome > 0
        //: self.coinIconImg.isHidden = !(isShowCoin && QuantityeractionDelay.share.appStatus == AppSkinStatus.normal.rawValue)
        self.coinIconImg.isHidden = !(isShowCoin && QuantityeractionDelay.share.appStatus == TotalervalObserve.normal.rawValue)
        ...
        self.coinLabel.text = (String(dataSkinWeightTitle)).underConnection(NSNumber(value: Float(newData.msgIncome)))
        //: self.replyTipLab.isHidden = true
        self.replyTipLab.isHidden = true

        // 展示消息是否已读标识
        //: if TalkingPrivateChatManager.isShowReadMsg(cellData: newData) {
        if ThreadSpell.frequencyAgainst(cellData: newData) {
            //: self.isReadImg.isHidden = false
            self.isReadImg.isHidden = false
            //: var readImgStr = "bth_unread_nor"
            var readImgStr = (user_writeId.replacingOccurrences(of: "today", with: "b") + String(constCircleTitle.suffix(5)) + "read_nor")
            // 消息对端是否已读
            //: if TalkingPrivateChatManager.msgIsRead(cellData: newData) {
            if ThreadSpell.connection(cellData: newData) {
                //: readImgStr = "bth_read_pre"
                readImgStr = (String(constMightRidData.prefix(5)) + "ead_pre")
            }
            //: self.isReadImg.image = UIImage.BundleImageNamed(name: readImgStr)
            self.isReadImg.image = UIImage.transferAndJoinSmall(name: readImgStr)
    ...
    override public func updateConstraints() {
        ...
        if let temCellData = self.cellData {
            //: loungePlus = (QuantityeractionDelay.share.loginUid == String(temCellData.msgModel.user.uid)) ? QuantityeractionDelay.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
            loungePlus = (QuantityeractionDelay.share.loginUid == String(temCellData.msgModel.user.uid)) ? QuantityeractionDelay.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
        }
